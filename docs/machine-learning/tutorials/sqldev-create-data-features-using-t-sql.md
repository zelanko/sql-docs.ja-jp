...
ms.locfileid: "81116065"
---
# <a name="lesson-2-create-data-features-using-r-and-t-sql"></a>レッスン 2: R と T-SQL を使用してデータの特徴量を作成する
...
データ探索を何度か行うと、データからインサイトが収集されているので、 "*特徴量エンジニアリング*" に移ることができます。 生データから意味のある特徴量を作成するこのプロセスは、分析モデルを作成するための重要な手順です。
...
2.  この関数が動作することを確認するために、それを利用し、メーター距離が 0 であったが、乗車場所と降車場所が異なっていた乗車の地理的距離を計算できます。
    ...
    ご覧のとおり、メーターによって報告された距離と地理的距離は常に一致しているわけではありません。 そのため、特徴量エンジニアリングが重要となります。 これらの改善されたデータの特徴量を使用して、R を使用して機械学習モデルをトレーニングすることができます。
