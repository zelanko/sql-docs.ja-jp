---
title: "データ シェイプ |Microsoft ドキュメント"
ms.prod: sql-non-specified
ms.technology:
- drivers
ms.custom: 
ms.date: 01/19/2017
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- data shaping [ADO]
ms.assetid: 62bd7dc9-45b5-4ca9-8b52-457325e0ce9e
caps.latest.revision: 9
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: 1ad3b257166308fe2c5a4bd94da5427dc0ca2649
ms.contentlocale: ja-jp
ms.lasthandoff: 09/09/2017

---
# <a name="data-shaping"></a>データ シェイプ
このセクションでは、次のトピックを扱います。  
  
-   [データ シェイプの概要](../../../ado/guide/data/data-shaping-overview.md)  
  
-   [データ ShapingExample](../../../ado/guide/data/data-shaping-example.md)  
  
-   [形状を変更](../../../ado/guide/data/reshaping.md)  
  
-   [孫の集計](../../../ado/guide/data/grandchild-aggregates.md)  
  
-   [パラメーター化コマンドの介在するコンピューティング コマンド](../../../ado/guide/data/parameterized-commands-with-intervening-compute-commands.md)  
  
-   [永続化階層レコード セット](../../../ado/guide/data/persisting-hierarchical-recordsets.md)  
  
-   [データ シェイプに必要なプロバイダー](../../../ado/guide/data/required-providers-for-data-shaping.md)  
  
-   [一般的な図形コマンド](../../../ado/guide/data/shape-commands-in-general.md)  
  
-   [図形の APPEND 句](../../../ado/guide/data/shape-append-clause.md)  
  
-   [図形の COMPUTE 句](../../../ado/guide/data/shape-compute-clause.md)  
  
-   [製造階層レコード セット](../../../ado/guide/data/fabricating-hierarchical-recordsets.md)  
  
-   [階層のレコード セット内の行にアクセスします。](../../../ado/guide/data/accessing-rows-in-a-hierarchical-recordset.md)  
  
-   [図形の正式な文法](../../../ado/guide/data/formal-shape-grammar.md)  
  
-   [Visual Basic アプリケーション関数](../../../ado/guide/data/visual-basic-for-applications-functions.md)
