...
## <a name="server-side-cursors"></a>サーバー側カーソル  
 ...
 ただし、サーバー側カーソルがであるため、注意が-少なくとも一時的に-すべてのアクティブなクライアントについて貴重なサーバー リソースを消費します。 サーバー ハードウェアがすべてのアクティブなクライアントによって要求されたサーバー側カーソルを管理できることを確認するに応じて計画する必要があります。 また、サーバー側カーソルを遅くなることだけ 1 つの行へのアクセスを提供するため、使用可能なバッチのカーソルはありません。  
