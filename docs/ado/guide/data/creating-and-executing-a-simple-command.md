...
## <a name="using-a-command-object"></a>コマンド オブジェクトの使用  
 使用してを単純なコマンドを作成する、**コマンド**オブジェクト、命令を割り当てる必要があります、 **CommandText**のプロパティ、**コマンド**オブジェクトし、適切な値の設定**CommandType**プロパティです。 コマンドを実行するに、開いている接続が割り当てられている必要があります、 **ActiveConnection**のプロパティ、**コマンド**への呼び出しの前に、オブジェクト、 **Execute**メソッド**コマンド**オブジェクト。  
...
## <a name="using-a-recordset-object"></a>レコード セット オブジェクトの使用  
 テキスト文字列と pa としてコマンドを作成することもに、**開く**メソッドを**レコード セット**オブジェクト、コマンドの種類 (adCmdText) と共に実行するためです。 次のコード スニペットでは、これを示しています。  
...
## <a name="using-a-connection-object"></a>接続オブジェクトを使用します。  
 コマンドは、開いている接続オブジェクトでも実行できます。 これで、前のコード例になりますこの。  
