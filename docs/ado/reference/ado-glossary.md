...
## <a name="a"></a>A  
 ...
 別名 (alias)  
 代替名を列または式では、SQL SELECT ステートメント、多くの場合より短い、またはわかりやすいに付与します。 たとえば、次の SELECT ステートメント内の別名は、BobSales:"SalesDB から BobSales として wr 販売を選択する。 DataControl オブジェクトの管理のバインドに列を動的に割り当てるには、エイリアスを使用できます。  
...
## <a name="c"></a>c  
 ...
 COM コンポーネント  
 バイナリ ファイル、.dll、.ocx、いくつかの .exe ファイルなど、オブジェクトを提供するための標準の COM をサポートします。 このようなファイルには、1 つまたは複数のクラス ファクトリ、COM クラス、レジストリ エントリのメカニズム、読み込みコード、およびなどのコードが含まれています。  
