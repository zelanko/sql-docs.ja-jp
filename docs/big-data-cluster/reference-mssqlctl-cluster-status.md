---
title: mssqlctl クラスター状態の参照
titleSuffix: SQL Server big data clusters
description: Mssqlctl クラスター状態コマンドに関する参照記事です。
author: rothja
ms.author: jroth
manager: craigg
ms.date: 05/22/2019
...
ms.locfileid: "65995060"
---
# <a name="mssqlctl-cluster-status"></a>mssqlctl クラスターの状態

[!INCLUDE[tsql-appliesto-ssver15-xxxx-xxxx-xxx](../includes/tsql-appliesto-ssver15-xxxx-xxxx-xxx.md)]

次の記事への参照を提供する、**クラスターの状態**コマンド、 **mssqlctl**ツール。 その他の詳細については**mssqlctl**コマンドを参照してください[mssqlctl 参照](reference-mssqlctl.md)します。

## <a name="commands"></a>コマンド
|     |     |
| --- | --- |
[mssqlctl クラスター状態の表示](#mssqlctl-cluster-status-show) | クラスターの状態が表示されます。
## <a name="mssqlctl-cluster-status-show"></a>mssqlctl クラスター状態の表示
クラスターの状態が表示されます。
```bash
mssqlctl cluster status show 
```
### <a name="examples"></a>使用例
ユーザーがログインしているクラスターの状態。
```bash
mssqlctl cluster status show
```
