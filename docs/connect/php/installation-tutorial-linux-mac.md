...
PHP 7.2 または 7.3 で PECL を含む PHP ドライバーをコンパイルするには、既定よりも最新の GCC が必要です。
```
...
exit
```
PECL の問題によって、アップグレード済みの GCC を使用している場合でも、ドライバーの最新バージョンの正しいインストールが妨げられることがあります。 インストールするには、パッケージをダウンロードし、手動でコンパイルします (pdo_sqlsrv と同様の手順)。
```
