...
> [!NOTE]  
>  以前のバージョンの [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client および SQLOLEDB では、標準の変換が失敗した場合に備えて OLE 変換が実装されていました。 その結果、SQL Server の OLE DB ドライバーによって実行されるいくつかの変換は、OLE DB 仕様によって異なります。  
