...
[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] は一括して設定の変換を行い、問題が発生すると JDBC ドライバーにエラーを返します。 クライアント側の変換の例外し、のみの場合は実行**日付**、**時間**、**タイムスタンプ**、**ブール**、および**文字列**値。
