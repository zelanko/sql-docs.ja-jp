...
> [!NOTE]  
> データベース内のオブジェクトを変更する SQL ステートメントは、データ定義言語 (DDL) ステートメントと呼ばれます。 これらの include ステートメント`CREATE TABLE`、 `DROP TABLE`、 `CREATE INDEX`、および`DROP INDEX`します。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] でサポートされる DDL ステートメントの種類の詳細については、[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)] オンライン ブックを参照してください。
