...
2.  **\<serviceBehaviors>** の下で **mdsWsHttpBehavior** セクションを見つけます。 サービスメタデータ>要素の場合は **、httpGetEnabled を** **true**に設定します。 ** \<**  
  
    > [!NOTE]  
    >  以前はセキュリティで保護されたソケット レイヤー (SSL) と呼ばれていたトランスポート層セキュリティ (TLS) 経由で Web サービスを有効にする場合は、web.config ファイルの**mdsWsHttpBehavior**セクションで**httpsGetEnabled**を**true**に設定します。 また、tls 用にも構成されるように**mdsWsHTTPBinding**を変更し、TLS 以外のセクションをコメントアウトする必要があります。  
