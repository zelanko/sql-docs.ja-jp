 ...
 **-e** _error_log_path_  
 エラー ログ ファイルのフル パスを指定します。 指定されていない場合、既定の場所は* \<drive>*: Server\MSSQL\Log\Errorlog になります。既定のインスタンスとドライブの場合は、名前付きインスタンスの場合は* \<ドライブ>**instance_name*: "/" になります。 **-e** と *error_log_path*の間には空白を入れません。  
