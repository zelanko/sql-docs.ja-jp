...
> [!NOTE]  
>  `TransactionOption` プロパティは、コンテナーによって要求された `IsolationLevel` プロパティの値が適用されるかどうかに影響します。 詳細については、の説明を参照して、 `IsolationLevel` 、トピック「プロパティ[パッケージのプロパティの設定](set-package-properties.md)です。  
