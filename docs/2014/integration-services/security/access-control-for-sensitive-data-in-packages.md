...
## <a name="protection-level-setting-and-the-ssisdb-catalog"></a>保護レベルの設定と SSISDB カタログ  
 ...
 プロジェクト (.ispac ファイル) をエクスポートするかどうか、[!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]サーバ、ファイル システムからは、システムは自動的に保護レベルを変更`EncryptSensitiveWithUserKey`です。 使用してプロジェクトをインポートする場合、 **Integration Services プロジェクトのインポート ウィザード**で[!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)]、 **ProtectionLevel**プロパティに、**プロパティ**ウィンドウ値を示した`EncryptSensitiveWithUserKey`です。  
...
1.  開発中は、既定値に設定するパッケージの保護レベルのままに`EncryptSensitiveWithUserKey`です。 この設定により、開発者のみがパッケージの機密情報を参照できるようになります。 また、`EncryptAllWithUserKey` または `DontSaveSensitive` を使用することもできます。  
