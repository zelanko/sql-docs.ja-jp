...
ms.locfileid: "36178257"
---
# <a name="integration-services-roles-ssis-service"></a>Integration Services のロール (SSIS サービス)
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 次の 3 つの固定データベース レベル ロールが含まれています`db_ssisadmin`、 **db_ssisltduser**、および**db_ssisoperator**パッケージへのアクセスを制御するためです。 保存されているパッケージにのみロールを実装することができます、`msdb`データベース[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]です。 パッケージにロールを割り当てるには、 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]を使用します。 ロールの割り当てに保存、`msdb`データベース。  
 ...
 **Sysssispackages**テーブルに`msdb`に保存されているパッケージを含む[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]です。 詳細については、「[sysssispackages &#40;Transact-SQL&#41;](/sql/relational-databases/system-tables/sysssispackages-transact-sql)」を参照してください。  
...
## <a name="permissions"></a>アクセス許可  
 既定では、アクセス許可、`db_ssisadmin`と**db_ssisoperator**固定データベース レベル ロールと、パッケージを作成したユーザーの一意なセキュリティ識別子が、パッケージ、およびアクセス許可のリーダー ロールに適用`db_ssisadmin`ロールおよびパッケージを作成したユーザーの一意なセキュリティ識別子、ライター ロールに適用します。 ユーザーのメンバーである必要があります、 `db_ssisadmin`、 **db_ssisltduser**、または**db_ssisoperator**ロール パッケージに対する読み取りアクセス権を持つようにします。 ユーザーのメンバーである必要があります、`db_ssisadmin`を書き込みアクセス権を持つようにロール。  
  
## <a name="access-to-packages"></a>パッケージへのアクセス  
 固定データベース レベル ロールは、ユーザー定義ロールと組み合わせて使用されます。 ユーザー定義ロールとは、ユーザーが [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] で作成するロールのことで、権限をパッケージに割り当てるために使用します。 パッケージにアクセスするには、ユーザーは、ユーザー定義ロールおよび関連する [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 固定データベースレベル ロールのメンバーである必要があります。 たとえば、ユーザーのメンバーである場合、 **AuditUsers**をパッケージに割り当てられているユーザー定義のロール、するものメンバーである`db_ssisadmin`、 **db_ssisltduser**、または**作業ssisoperator**ロール パッケージに対する読み取りアクセス権を持つようにします。  
  
 ユーザー定義ロールをパッケージに割り当てていない場合、パッケージへのアクセスは固定データベース レベル ロールによって決定されます。  
  
 ユーザー定義ロールを使用する場合を追加する必要があります、`msdb`データベースにパッケージに割り当てることができます。 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]では、新しいデータベース ロールを作成できます。  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] (MSSQLSERVER サービス) は、データベース エンジンとアクセスに接続できる前に開始する必要があります、`msdb`データベース。  
