...
# <a name="msmq-connection-manager"></a>MSMQ 接続マネージャー
 ...
 MSMQ 接続マネージャーをパッケージに追加するときに[!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]接続マネージャーを実行時に、MSMQ 接続を解決する、接続マネージャーのプロパティを設定する接続マネージャーを追加作成、`Connections`コレクションに、パッケージです。 `ConnectionManagerType`接続マネージャーのプロパティに設定されて`MSMQ`です。  
