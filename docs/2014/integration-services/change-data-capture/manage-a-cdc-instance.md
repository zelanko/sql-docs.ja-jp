...
-   **状態**: CDC インスタンスの現在の状態を示すアイコン。 これらの状態を次に示します。  
    ...
    |![OK](../media/okay.gif "OK")|**[実行中]**: CDC インスタンスが実行されていると、変更レコードが処理します。 次の副状態が利用できます。<br /><br /> **アイドル**:すべての変更レコードが処理され、ターゲット変更テーブルに格納されています。 アクティブなトランザクションはこれ以上ありません。<br /><br /> **処理**:プロセスの中の変更テーブルにまだ書き込まれていない変更レコードがあります。|  
