...
-   **シンボル**既定では、一連の**シンボル**、次の文字が含まれています: `,.;:-"'`~ = (& a)/@.!?() <>:operator[]{}| #* ^ %`. For example, if the symbols are "`()-'"、「(425) 123-4567」の値としてトークン化された [「(",「425」,」)",「123」、"-「,「4567」,」)"]。  
 ...
 **Symbols**  
 パターンの一部として保持する記号の一覧が表示されます。 記号の例として、日付を表す "/"、時刻を表す ":"、電子メール アドレスを表す " @ " などがあります。 既定では、一連の**シンボル**、次の文字が含まれています: `,.;:-"'`~ = (& a)/@.!?() <>:operator[]{}| #* ^ %' です。  
