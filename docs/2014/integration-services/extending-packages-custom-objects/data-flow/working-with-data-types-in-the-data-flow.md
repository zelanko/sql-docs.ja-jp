...
## <a name="mapping-data-types-in-the-data-flow"></a>データ フローでのデータ型のマッピング  
 変換を介して変換元から変換先にデータを移動する際に、<xref:Microsoft.SqlServer.Dts.Runtime.Wrapper.DataType> 列挙で定義された [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] 型と、**System** 名前空間で定義された [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[dnprdnshort](../../../includes/dnprdnshort-md.md)] のマネージ データ型との間で、データ フロー コンポーネントによるデータ型の変換が必要になる場合があります。 また、[!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)] データ型をマネージ型に変換する前に、別の型への変換が必要になる場合もあります。  
