...
ms.locfileid: "36071236"
---
# <a name="review-data-type-mapping-sql-server-import-and-export-wizard"></a>[データ型マッピングの確認]\(SQL Server インポートおよびエクスポート ウィザード)
  使用して、**データ型マッピングの確認**にソース データを転送先との互換性を実行する、ウィザードでは、データ型の変換に関する詳細な情報を確認します。 この情報には、成功すると予測される変換と、エラーまたは切り捨てが発生する可能性がある変換を区別するための視覚的な手掛かりが含まれています。 変換ごとに、ウィザードによって提案された変換を受け入れるかどうかを決定したり、発生したエラーの処理方法を指定したりすることができます。  
 ...
 **レビューのデータ型マッピング**ページから成る、**テーブル** ボックスの一覧、**データ型マッピング**リスト、およびエラー処理オプション。  
  
### <a name="table-list"></a>[テーブル] の一覧  
 上部、**データ型の問題のレビュー**ページは、**テーブル**ソースから宛先に転送するテーブルを一覧表示される一覧。 次の表では、この一覧の列について説明します。  
...
|**変換先**|変換先テーブルの名前を表示します。|  
 ...
 個々 のテーブルの変換情報を表示するには、このテーブルを選択**テーブル**グリッドです。 内の列に、選択したテーブルの変換情報が表示されます、**データ型マッピング グリッド**ページの下部にします。  
  
### <a name="data-type-mapping-list"></a>[データ型マッピング] の一覧  
 下の部分、**データ型の問題のレビュー**ページは、**データ型マッピング** ボックスの一覧です。 このグリッドには、**[テーブル]** の一覧で選択したテーブルの列についての詳細な変換情報が表示されます。 次の表では、この一覧の列について説明します。  
...
|**エラー時**|ウィザードでエラーを処理する方法を指定します。<br /><br /> 使用して、 **On Error (グローバル)** 設定します。<br /><br /> エラーが発生して失敗し、インポート処理またはエクスポート処理を停止します。<br /><br /> エラーを無視します。|  
|**切り捨て時**|ウィザードで切り捨てを処理する方法を指定します。<br /><br /> 使用して、 **切り捨て時 (グローバル)** 設定します。<br /><br /> エラーで失敗して、インポートの停止またはエクスポート プロセス<br /><br /> 切り捨てを無視します。|  
...
-   エラーを無視し、インポート処理またはエクスポート処理を続行します。  
 ...
 この設定を持つすべての変換に適用されます**グローバルを使用**で選択されている、 **On Error**の列、**データ型マッピング** ボックスの一覧。  
...
-   切り捨てを無視し、インポート処理またはエクスポート処理を続行します。  
 ...
 この設定を持つすべての変換に適用されます**グローバルを使用**で選択されている、 **[切り捨て時**の列、**データ型マッピング**] ボックスの一覧です。  
