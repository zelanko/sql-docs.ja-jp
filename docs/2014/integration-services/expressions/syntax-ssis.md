...
## <a name="identifiers"></a>識別子  
 ...
 式には、 [!INCLUDE[ssIS](../../includes/ssis-md.md)] で用意されているシステム変数と、カスタム変数を含めることができます。 式で参照されている場合、変数を含める必要があります、\@プレフィックス。 たとえば、`Counter`を使用して変数を参照\@カウンター。 \@文字変数名の一部でない; のみを示します、式エバリュエーターに識別子が変数であります。 詳しくは、「[識別子 &#40;SSIS&#41;](identifiers-ssis.md)」をご覧ください。  
