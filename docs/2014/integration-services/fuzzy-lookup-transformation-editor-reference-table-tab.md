...
> [!NOTE]  
>  `Exhaustive`と`MaxMemoryUsage`あいまい参照変換のプロパティでは使用できない、**あいまい参照変換エディター**を使用して設定することが、**詳細エディター**です。 さらに、値の 100 より大きい`MaxOutputMatchesPerInput`でのみ指定することができます、**詳細エディター**です。 これらのプロパティの詳細については、「 [変換のカスタム プロパティ](data-flow/transformations/transformation-custom-properties.md)」の「あいまい参照変換」を参照してください。  
