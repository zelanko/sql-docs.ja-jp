...
## <a name="benefits-of-the-buffer-pool-extension"></a>バッファー プール拡張の利点  
 ...
 このような I/O ボトルネックを解決するための一般的な方法は、DRAM を増設するか、または高パフォーマンス SAS スピンドルを追加する方法です。 これらのオプションは効果的ですが、重大な欠点があります。DRAM はデータ ストレージ ドライブよりも高価とスピンドルを追加するのには、ハードウェアの購入経費が増えるし、電力消費量とコンポーネントの障害の可能性の増加によって、運用コストが増加します。  
