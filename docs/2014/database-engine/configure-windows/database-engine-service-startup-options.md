...
## <a name="using-startup-options-for-troubleshooting"></a>トラブルシューティングでのスタートアップ オプションの使用  
 シングル ユーザー モードや最小構成モードなど、一部のスタートアップ オプションは、主にトラブルシューティングの際に使用されます。 **-M**オプションまたは **-f**オプションを使用してトラブルシューティングを行うためにサーバーを起動することは、sqlservr.exe を手動で開始するのと同じように、コマンドラインで行うのが最も簡単です。  
