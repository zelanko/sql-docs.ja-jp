...
2.  [**名前**] ボックスに「 `mmc -a` 」と入力[!INCLUDE[msCoName](../../includes/msconame-md.md)]して、管理コンソールを作成者モードで開きます。  
...
12. [**ファイル**] メニューの [名前を付け**て保存**] をクリックし、ファイル拡張子の`.msc`付いた適切な名前を使用して、目的のフォルダーにファイルを保存します。 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 管理コンソールを閉じます。  
