...
# <a name="optimize-for-ad-hoc-workloads-server-configuration-option"></a>optimize for ad hoc workloads サーバー構成オプション
 ...
 トレース フラグ 8032 は、キャッシュ制限パラメーターを [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] RTM の設定に戻します。これにより、一般に、より大きいキャッシュに対応できるようになります。 頻繁に再利用されるキャッシュエントリがキャッシュに格納されず、[*アドホックワークロードの最適化] サーバー構成オプション*でプランキャッシュの問題を解決できなかった場合に、この設定を使用します。  
