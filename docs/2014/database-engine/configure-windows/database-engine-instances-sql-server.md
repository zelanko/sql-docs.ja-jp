...
ms.locfileid: "36083498"
---
# <a name="database-engine-instances-sql-server"></a>データベース エンジンのインスタンス (SQL Server)
  インスタンス、[!INCLUDE[ssDE](../../includes/ssde-md.md)]のコピーである、`sqlservr.exe`オペレーティング システム サービスとして実行する実行可能ファイルです。 各インスタンスは、いくつかのシステム データベースと、1 つまたは複数のユーザー データベースを管理します。 各コンピューターは、 [!INCLUDE[ssDE](../../includes/ssde-md.md)]の複数のインスタンスを実行できます。 アプリケーションはインスタンスに接続して、インスタンスに管理されているデータベースでの作業を実行します。  
