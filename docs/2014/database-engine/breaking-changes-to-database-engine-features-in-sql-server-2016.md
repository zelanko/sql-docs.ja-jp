---
title: 重大な変更をデータベース エンジンの SQL Server 2014 における機能 |Microsoft ドキュメント
ms.custom: ''
ms.date: 06/13/2017
...
|機能|説明|  
|-------------|-----------------|  
|NEXT という名前の列またはテーブルから選択している|シーケンスでは、ANSI 標準の NEXT VALUE FOR 関数が使用されます。 テーブルまたは列の名前が NEXT で、テーブルまたは列が別名として VALUE があり、ANSI 標準の AS が省略されている場合は、作成されたステートメントにエラーが発生します。 問題を回避するには、ANSI 標準の AS キーワードを含めます。 たとえば、`SELECT NEXT VALUE FROM Table` は `SELECT NEXT AS VALUE FROM Table` に書き直す必要があり、`SELECT Col1 FROM NEXT VALUE` は `SELECT Col1 FROM NEXT AS VALUE` に書き直す必要があります。|  
...
|sp_setapprole と sp_unsetapprole|Cookie`OUTPUT`パラメーター`sp_setapprole`として記載されている現在`varbinary(8000)`な正しいの最大長。 ただし、現在の実装が返されます`varbinary(50)`です。 アプリケーションが継続して予約する必要があります`varbinary(8000)`アプリケーションのサイズの増加、将来のリリースでクッキーの戻り値が正しく動作を継続できるようにします。 詳細については、「[sp_setapprole &#40;Transact-SQL&#41;](/sql/relational-databases/system-stored-procedures/sp-setapprole-transact-sql)」を参照してください。|  
|EXECUTE AS|EXECUTE AS のクッキーの OUTPUT パラメーターは現在、適切な最大長である `varbinary(8000)` としてドキュメントに記載されています。 ただし、現在の実装が返されます`varbinary(100)`です。 アプリケーションが継続して予約する必要があります`varbinary(8000)`アプリケーションのサイズの増加、将来のリリースでクッキーの戻り値が正しく動作を継続できるようにします。 詳細については、「[EXECUTE AS &#40;Transact-SQL&#41;](/sql/t-sql/statements/execute-as-transact-sql)」を参照してください。|  
|sys.fn_get_audit_file 関数|2 つの列 (**user_defined_event_id**と**user_defined_information**) ユーザー定義の監査イベントをサポートするために追加されました。 名前で列を選択しないアプリケーションは、予想よりも多くの列を返す場合があります。 名前で列を選択するか、これらの追加列を受け入れるようにアプリケーションを調整してください。|  
...
|SERVERPROPERTY (‘Edition’)|[!INCLUDE[ssSQL11](../includes/sssql11-md.md)] インスタンスのインストールされている製品のエディション。 このプロパティの値を使用すると、インストールされている製品でサポートされている CPU の最大数など、機能や制限を確認できます。<br /><br /> インストールされている Enterprise のエディションに基づいて、"Enterprise Edition" または "Enterprise Edition: Core-based Licensing" が返されることがあります。 Enterprise のエディションは、[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] の単一のインスタンスによる最大計算容量に基づいて区別されます。 計算容量制限について詳しく[!INCLUDE[ssSQL11](../includes/sssql11-md.md)]を参照してください[Compute Capacity Limits by Edition of SQL Server](../sql-server/compute-capacity-limits-by-edition-of-sql-server.md)です。|  
|CREATE LOGIN|`CREATE LOGIN WITH PASSWORD = '`*パスワード*`' HASHED`によって作成されたハッシュでオプションを使用することはできません[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]7 またはそれ以前です。|  
...
|sys.dm_os_memory_cache_counters|次の列の名前が変更されています。<br /><br /> single_pages_kb になります。 <br />                          pages_kb<br /><br /> multi_pages_kb<br />                           ここでは、: pages_in_use_kb|  
|sys.dm_os_memory_cache_entries|Pages_allocated_count [列] 列には、名前を変更した pages_kb はされました。|  
|sys.dm_os_memory_clerks|列 multi_pages_kb は削除されました。<br /><br /> 列 single_pages_kb 列には、名前を変更した pages_kb はされました。|  
|sys.dm_os_memory_nodes|次の列の名前が変更されています。<br /><br /> single_pages_kb になります。 <br />                            pages_kb<br /><br /> multi_pages_kb になります。 <br />                            foreign_committed_kb|  
|sys.dm_os_memory_objects|以下の列の名前が変更されました。<br /><br /> pages_allocated_count になります。<br />                            pages_in_bytes<br /><br /> max_pages_allocated_count は今すぐ: max_pages_in_bytes|  
|sys.dm_os_sys_info|次の列の名前が変更されています。<br /><br /> physical_memory_in_bytes になります。 <br />                            physical_memory_kb<br /><br /> bpool_commit_target になります。 <br />                            committed_target_kb<br /><br /> bpool_visible はされます。 <br />                            visible_target_kb<br /><br /> virtual_memory_in_bytes になります。 <br />                            virtual_memory_kb<br /><br /> bpool_commited になります。<br />                            committed_kb|  
...
### <a name="sql-clr-data-types-geometry-geography-and-hierarchyid"></a>SQL CLR データ型 (geometry、geography、および hierarchyid)  
 アセンブリ**Microsoft.SqlServer.Types.dll**、バージョン 11.0 に、バージョン 10.0 からアップグレードされたが、空間データ型と hierarchyid 型が含まれています。 このアセンブリを参照するカスタム アプリケーションは、次の条件に該当する場合に失敗します。  
...
-   使用して外部の値をインポート**sql:column**または**sql:variable**です。 これらの方法を使用した場合、無効または部分的なサロゲート ペアが使用される可能性は残ります。  
...
-   **fn:string-長さ**です。 ただし、無効または部分的なサロゲート ペアがでの動作は、引数として渡された場合**文字列長**が定義されていません。  
  
-   **fn:substring**です。  
  
-   **fn: を含む**です。 ただし、部分的なサロゲート ペアが値として渡された場合**含む**整形式のサロゲート ペアに含まれている部分的なサロゲート ペアが検出されたために、予期しない結果を返す可能性があります。  
  
-   **fn:concat**です。 ただし、部分的なサロゲート ペアが値として渡された場合**concat**正しくないサロゲート ペアまたは部分的なサロゲート ペアを生成することができます。  
  
-   比較演算子と**の order by**句。 比較操作には、+、 \<、>、 \<=、> =、 `eq`、 `lt`、 `gt`、 `le`、および`ge`です。  
  
#### <a name="distributed-query-calls-to-a-system-procedure"></a>分散クエリ呼び出しに、システム ストアド プロシージャ  
 ...
 **新しい動作は、既定の名前空間 URI 関数によって異なります。**  
  
 次の機能がデモンストレーションされる場合にのみ上、既定の名前空間 URI に対応する最終的な推奨設定の名前空間は、新しい動作が説明されている[ http://www.w3.org/2005/xpath-functions](http://www.w3.org/2005/xpath-functions)です。 互換性レベルが 110 以上の場合、既定では、[!INCLUDE[ssSQL11](../includes/sssql11-md.md)] は既定の関数名前空間をこの名前空間にバインドします。 ただし、これらの関数は、この名前空間が互換性レベルにかかわらず使用される場合にのみ、新しい動作を示します。  
  
-   **fn:string の長さ**  
  
-   **fn:substring**  
  
##  <a name="KJKatmai"></a> 重大な変更: SQL Server 2008/SQL Server 2008 r2  
...
|機能|説明|  
|-------------|-----------------|  
|プラン表示の XML スキーマ|新しい**SeekPredicateNew**要素は、Showplan XML スキーマ、およびある xsd sequence に追加 (**SqlPredicatesType**) に変換されますが、  **\<xsd:choice >** 項目。 1 つまたは複数の代わりに**SeekPredicate**要素、1 つまたは複数**SeekPredicateNew**要素可能性があります、Showplan XML で表示されます。 この 2 つの要素を同時に指定することはできません。 **SeekPredicate**は保持され、Showplan XML スキーマの下位互換性です。 ただし、作成されたクエリ プランで[!INCLUDE[ssKatmai](../includes/sskatmai-md.md)]含めることができます、 **SeekPredicateNew**要素。 のみを取得するアプリケーションは、 **SeekPredicate**場合、ShowPlanXML/BatchSequence/バッチ/ステートメント/StmtSimple/QueryPlan/RelOp/行わ/SeekPredicates ノードから子が失敗する可能性があります、 **SeekPredicate**要素が存在しません。 いずれかにアプリケーションを書き直し、 **SeekPredicate**または**SeekPredicateNew**このノード内の要素。 詳細については、「」を参照してください。|  
|プラン表示の XML スキーマ|新しい**IndexKind**属性が追加、 **ObjectType** Showplan XML スキーマに複合型。 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] プランを [!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)] スキーマに対して厳密に検証するアプリケーションは失敗します。|  
  
### <a name="transact-sql"></a>Transact-SQL  
  
|機能|説明|  
|-------------|-----------------|  
|ALTER_AUTHORIZATION_DATABASE DDL イベント|[!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)]、DDL イベント ALTER_AUTHORIZATION_DATABASE が発生、'object' で返される値、 **ObjectType**のエンティティが、セキュリティ保護可能なデータの定義に入力したときに、このイベントの EVENTDATA xml 要素言語 (DDL) 操作は、オブジェクトです。 [!INCLUDE[ssKatmai](../includes/sskatmai-md.md)] では、実際の種類 (たとえば 'table' や 'function') が返されます。|  
...
|OUTPUT 句|非決定的な動作を防ぐために、ビューまたはインライン テーブル値関数から返される列が次のいずれかの方法で定義されている場合、OUTPUT 句ではその列を参照できません。<br /><br /> サブクエリ。<br /><br /> ユーザー定義関数、ユーザーまたはシステム データ アクセスを実行するか、このようなアクセスを実行すると見なされます。<br /><br /> ユーザー データまたはシステム データにアクセスするユーザー定義関数を定義に含む計算列<br /><br /> <br /><br /> [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] が OUTPUT 句でそのような列を検出すると、エラー 4186 が発生します。 詳細については、次を参照してください。 [MSSQLSERVER_4186](../relational-databases/errors-events/mssqlserver-4186-database-engine-error.md)です。|  
...
|sp_helpuser|Sp_helpuser ストアド プロシージャの結果セットで返される次の列名が変更されました。<br /><br /> グループ名になります。<br />                            RoleName<br /><br /> "Group_name"は、ここでは。<br />                            Role_name<br /><br /> Group_id になります。<br />                            Role_id<br /><br /> Users_in_group になります。<br />                            Users_in_role|  
...
|機能|説明|  
|-------------|-----------------|  
|Datetime のサポート|[!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)]、データ型`xs:time`、 `xs:date`、および`xs:dateTime`タイム ゾーンのサポートはありません。 タイム ゾーン データは UTC タイム ゾーンにマップされます。 [!INCLUDE[ssKatmai](../includes/sskatmai-md.md)] では、標準に準拠した動作を提供するため、次のように変更されました。<br /><br /> タイム ゾーンを使用しない値の検証<br /><br /> タイム ゾーン指定の有無の保持<br /><br /> 内部ストレージ表現の変更<br /><br /> 格納された値の解決を増加<br /><br /> 負の年の使用禁止<br /><br /> <br /><br /> 注: は、新しい型の値に対応するには、アプリケーションと XQuery 式を変更します。|  
|XQuery 式と Xpath 式|[!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)]コロンで始まる XQuery または XPath 式のステップ (': ') は許可されています。 たとえば、次のステートメントのパス式には、コロンで始まる名前テスト (`CTR02)`) が含まれています。<br /><br /> `SELECT FileContext.query('for n$ in //CTR return <C>{data )(n$/:CTR02)} </C>) AS Files FROM dbo.MyTable;`<br /><br /> この使用法は XML 標準に準拠していないため、[!INCLUDE[ssKatmai](../includes/sskatmai-md.md)] では許可されません。 エラー 9341 が返されます。 先頭のコロンを削除するか、名前テストにプレフィックスを指定してください (n$/CTR02 や n$/p1:CTR02 など)。|  
  
### <a name="connecting"></a>接続  
  
|機能|説明|  
|-------------|-----------------|  
|SSL を使用した [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] Native Client からの接続|[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] Native Client を使用して接続するとき、以前は、緩和された検証により、サブジェクトに完全修飾ドメイン名 (FQDN) が指定されている証明書で "SERVER=shortname; FORCE ENCRYPTION=true" を使用するアプリケーションでも接続できましたが、 SQL Server 2008 R2 では、セキュリティ強化のために証明書で FQDN のサブジェクトが強制されるようになりました。 緩和された検証に依存しているアプリケーションでは次のいずれかの対処が必要になります。<br /><br /> 接続文字列で FQDN を使用する。<br /><br /> -このオプションでは、アプリケーションの外部接続文字列の SERVER キーワードが構成されている場合は、アプリケーションを再コンパイルすることは不要です。<br /><br /> -このオプションは、その接続文字列がハードコードされているアプリケーションでは機能しません。<br /><br /> -このオプションは、単純な名前のミラー化されたサーバー応答からのデータベース ミラーリングを使用するアプリケーションでは機能しません。|  
||shortname の別名を追加して FQDN にマップする。<br /><br /> -このオプションは、その接続文字列がハードコードされているアプリケーションに対しても機能します。<br /><br /> -このオプションは、プロバイダーは、受け取ったフェールオーバー パートナー名の別名を参照しないために、データベース ミラーリングを使用するアプリケーションでは機能しません。|  
||shortname に対して発行された証明書を使用する。<br /><br /> -このオプションは、すべてのアプリケーションに対して機能します。|  
  
##  <a name="Yukon"></a> SQL Server 2005 における重大な変更  
 導入された変更を中断するリストの[!INCLUDE[ssVersion2005](../includes/ssversion2005-md.md)]を参照してください[SQL Server 2005 におけるデータベース エンジン機能の重大な変更](http://msdn.microsoft.com/library/ms143179\(SQL.90\).aspx)です。  
