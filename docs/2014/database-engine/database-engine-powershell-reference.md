...
|説明|コマンドレット|  
|-----------------|------------|  
|使用して実行できるスクリプトなど、TRANSACT-SQL および XQuery スクリプトを実行、`sqlcmd`ユーティリティです。|[Invoke-Sqlcmd コマンドレット](../../2014/database-engine/invoke-sqlcmd-cmdlet.md)|  
...
### <a name="information-about-other-cmdlets"></a>その他のコマンドレットに関する情報  
 `Encode-Sqlname`と`Decode-Sqlname`コマンドレットのヘルプを PowerShell パスでサポートされていない文字を含む SQL Server 識別子を指定します。 詳細については、「 [PowerShell での SQL Server 識別子](../powershell/sql-server-identifiers-in-powershell.md)」を参照してください。  
