...
|**[パートナー (接続するユーザー)]**|データベースのフェールオーバー パートナーの名前です。 **コンソールユーザーの Windows 認証**または**ログイン ' ***\<login name>*** ' の SQL Server 認証**が、かっこ内に表示されます。 これは、既に追加されたインスタンスの場合は、現在使用されている認証情報を、まだモニターに追加されていないインスタンスの場合は、これから使用される認証情報を表します。|  
