...
2.  各サーバー インスタンスにセキュリティを設定します。  
    ...
    > [!NOTE]  
    >  サーバー インスタンスによりデータベースのミラーリングに使用される認証の形式は、データベース ミラーリング エンドポイントのプロパティで指定します。 データベース ミラーリングの 2 種類のトランスポート セキュリティがあります。Windows 認証または証明書ベースの認証。 詳細については、[データベース ミラーリングと AlwaysOn 可用性グループのトランスポート セキュリティ&#40;SQL Server&#41;](transport-security-database-mirroring-always-on-availability.md)を参照してください。  
