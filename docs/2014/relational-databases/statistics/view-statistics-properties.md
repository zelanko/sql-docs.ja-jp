...
1.  
  **オブジェクト エクスプローラー**で、新しい統計を作成するデータベースをプラス記号をクリックして展開します。  
...
6.  [ **統計のプロパティ -** _statistics_name_ ] ダイアログ ボックスの **[ページの選択]** ウィンドウで **[詳細]** を選びます。  
     ...
     **文字列インデックス**  
     Yes の場合は、統計オブジェクトに文字列の統計概要が含まれています。これにより、LIKE 演算子を使用するクエリ述語 (`WHERE ProductName LIKE '%Bike'` など) に対するカーディナリティの推定が向上します。 文字列の概要統計情報は、ヒストグラムとは別に格納され、型が**char**、 **varchar**、 **nchar**、 **nvarchar**、 **varchar (max)**、 **nvarchar (max)**、 **text**、または**ntext**の場合、統計オブジェクトの最初のキー列に作成されます。  
