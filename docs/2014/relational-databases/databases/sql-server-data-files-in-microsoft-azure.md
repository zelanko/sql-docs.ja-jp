...
-   **高可用性とディザスター リカバリーの利点:** Windows Azure 機能で SQL Server データ ファイルを使用して容易に高可用性とディザスター リカバリーのソリューション提供できます。 たとえば、Windows Azure の仮想マシンまたは SQL Server のインスタンスがクラッシュした場合、Windows Azure BLOB へのリンクを再設定するだけで、新しいマシンにデータベースを再作成できます。  
...
-   Windows Azure の仮想マシンで実行されている SQL Server: SQL Server は、Windows Azure 仮想マシンでインストールが場合、SQL Server 2014 をインストールするか、既存のインスタンスを更新します。 同様に、SQL Server 2014 CTP2 のプラットフォーム イメージを使用して Microsoft Azure に新しい仮想マシンを作成することもできます。 SQL Server 2014 のダウンロード方法については、「 [SQL Server 2014](http://www.microsoft.com/sqlserver/sql-server-2014.aspx)」をご覧ください。  
  
###  <a name="bkmk_Limitations"></a> 制限事項  
  
-   この機能の現在のリリースで格納する`FileStream`Windows Azure ストレージにデータがサポートされていません。 Windows Azure ストレージに統合されたローカル データベースに `Filestream` データを格納することはできますが、Windows Azure ストレージを使用してコンピューター間で Filestream データを移動することはできません。 `FileStream`データ、お勧めの異なるマシン間で Filestream に関連付けられているファイル (.mdf、.ldf) を移動する、従来の手法を使用し続けることです。  
...
-   *SQL Server 資格情報が正しく作成されていません。*   
    解決策: **[ID]** フィールドに 'Shared Access Signature' を使用して、シークレットを正しく作成したことを確認します。 レッスン 3 で説明されている手順を確認して[チュートリアル: Windows Azure ストレージ サービスでは、SQL Server データ ファイル](../tutorial-use-azure-blob-storage-service-with-sql-server-2016.md)します。  
...
1.  *データベースの作成中にエラーが発生しました*   
    解決方法: レッスン 4 に記載されている手順を確認する[チュートリアル: Windows Azure ストレージ サービスでは、SQL Server データ ファイル](../tutorial-use-azure-blob-storage-service-with-sql-server-2016.md)します。  
