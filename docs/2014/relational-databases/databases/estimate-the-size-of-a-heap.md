...
-   ラージ オブジェクト (LOB) の値  
     ...
     LOB データ型`varchar(max)` `varbinary(max)` `nvarchar(max)`、、 `text`、、 **ntextxml**、および`image`値の格納に使用される領域を正確に特定するためのアルゴリズムは複雑です。 LOB データ型の値で使用される領域の計算は、予想される LOB 値の平均サイズを合計し、ヒープの合計サイズに加算するだけで十分です。  
