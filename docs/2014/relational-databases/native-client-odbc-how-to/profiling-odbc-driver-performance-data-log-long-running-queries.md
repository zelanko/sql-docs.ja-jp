...
1.  呼び出す[SQLSetConnectAttr](../native-client-odbc-api/sqlsetconnectattr.md) SQL_COPT_SS_PERF_QUERY_LOG および実行時間の長いクエリのログ ファイルの完全パスとファイル名にします。 以下に例を示します。  
  
    ```  
    C:\\Odbcqry.log  
    ```  
  
2.  呼び出す[SQLSetConnectAttr](../native-client-odbc-api/sqlsetconnectattr.md) sql_copt_ss_perf_query_interval 属性とタイムアウトの間隔をミリ秒に設定します。  
  
3.  呼び出す[SQLSetConnectAttr](../native-client-odbc-api/sqlsetconnectattr.md) SQL_COPT_SS_PERF_QUERY および SQL_PERF_START 実行時間の長いクエリのログ記録を開始するとします。  
