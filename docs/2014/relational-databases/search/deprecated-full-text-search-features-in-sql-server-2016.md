...
## <a name="features-not-supported-in-a-future-version-of-sql-server"></a>SQL Server の今後のバージョンでサポートされない機能  
 以下のフルテキスト検索機能は [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]の次のバージョンではサポートされますが、その後のバージョンでは削除されます。 
...
|CREATE FULLTEXT CATALOG オプション<br /><br /> IN PATH '*rootpath*'<br /><br /> ON FILEGROUP *filegroup*|[なし] :|CREATE FULLTEXT CATLOG IN PATH<br /><br /> なし*|237<br /><br /> 存在.<sup>*</sup>|  
...
|sp_fulltext_service アクションの値: resource_usage には機能がありません。|なし|sp_fulltext_service @action=resource_usage|200|  
 ...
 \***SQL Server:** 使用されていない機能オブジェクトは、ファイルグループ*ファイル*グループでのフルテキスト catlog の作成の発生を監視しません。  
