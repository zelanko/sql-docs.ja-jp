---
title: サーバーにクライアントから変換 |Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
## <a name="conversions"></a>コンバージョン  
 このトピックでは、クライアントで行われる変換について説明します。 パラメーターに対して、サーバーで定義されているのとは異なる、秒の小数部の有効桁数をクライアントで指定した場合、サーバー側で変換すると処理に成功するのに、クライアント側で変換すると処理に失敗することがあります。 一方、クライアントで秒の小数部の切り捨てがエラーとして処理する具体的には、[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]の時間が最も近い全体の 2 番目の値を丸めます。  
