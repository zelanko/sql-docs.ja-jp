 ...
 **Issasynchstatus::getstatus**とまったく同じメソッドの動作は、 **idbasynchstatus::getstatus**点を除いてメソッド ソース オブジェクトのデータを初期化する場合は中止されます、E_UNEXPECTED が返されるではなくDB_E_CANCELED より (が[issasynchstatus::waitforasynchcompletion](issasynchstatus-waitforasynchcompletion-ole-db.md) DB_E_CANCELED が返されます)。 これは、初期化の中止後、追加の初期化操作が試行される場合に備えて、データ ソース オブジェクトの状態が通常のゾンビ状態のままにならないためです。  
