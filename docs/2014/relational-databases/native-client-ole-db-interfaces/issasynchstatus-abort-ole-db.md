...
## <a name="return-code-values"></a>リターン コードの値  
 S_OK  
 非同期操作を取り消す要求が処理されました。 ただし、その操作自体が取り消されることが保証されるわけではありません。 操作が取り消されたかどうかを決定するには、コンシューマーが呼び出す必要があります[issasynchstatus::getstatus](issasynchstatus-getstatus-ole-db.md)と db_e_canceled です。 ただし、その可能性がありますいない呼び出しで返される、非常に [次へ] です。  
