...
-   数値型の可変長ストレージ形式を使用 (たとえば`integer`、 `decimal`、および`float`) と数値に基づく型 (たとえば`datetime`と`money`)。  
...
|`float`|はい|0 が設定された最下位バイトは格納されません。 `float` 圧縮は、仮数の適用されます。|  
|`real`|はい|0 が設定された最下位バイトは格納されません。 `real` 圧縮は、仮数の適用されます。|  
...
|`datetimeoffset`|はい|よう`datetime2`2 バイトの形式 (HH:MM) のタイム ゾーンがあることを除いて、します。<br /><br /> `datetime2` と同様、圧縮により 2 バイト節約できます。<br /><br /> タイム ゾーンの値では、ほとんどの場合、MM 値が 0 になります。 したがって、圧縮によって 1 バイト節約できる可能性があります。<br /><br /> 行の圧縮でストレージに変更はありません。|  
