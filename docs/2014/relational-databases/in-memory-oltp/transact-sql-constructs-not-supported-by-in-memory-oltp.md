...
|データの種類|*データ型名*|指示されたデータ型はサポートされていません。 サポートされているデータ型に置き換えます。 詳細については、「[サポートされるデータ型](supported-data-types-for-in-memory-oltp.md)」を参照してください。|  
...
|機能|番号付きストアド プロシージャ|ネイティブ コンパイル ストアド プロシージャには番号を付けられません。 `;`ステートメントから*数値*を削除 `CREATE PROCEDURE` します。|  
...
|機能|ROLLUP|`ROLLUP` は、ネイティブ コンパイル ストアド プロシージャ内の `GROUP BY` 句では使用できません。 `ROLLUP`プロシージャの定義からを削除します。|  
|機能|CUBE|`CUBE` は、ネイティブ コンパイル ストアド プロシージャ内の `GROUP BY` 句では使用できません。 `CUBE`プロシージャの定義からを削除します。|  
|機能|GROUPING SETS|`GROUPING SETS` は、ネイティブ コンパイル ストアド プロシージャ内の `GROUP BY` 句では使用できません。 `GROUPING SETS`プロシージャの定義からを削除します。|  
...
|機能|EXECUTE AS CALLER|`EXECUTE AS` 句は必須です。 ただし `EXECUTE AS CALLER` はサポートされていません。 `EXECUTE AS OWNER`、 `EXECUTE AS` *User*、またはを使用し `EXECUTE AS SELF` ます。|  
