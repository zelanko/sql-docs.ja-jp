...
|機能|番号付きストアド プロシージャ|ネイティブ コンパイル ストアド プロシージャには番号を付けられません。 ステートメントから`;`数値を削除します。 *number* `CREATE PROCEDURE`|  
...
|機能|SELECT INTO|`INTO` 句は `SELECT` ステートメントではサポートされていません。 `INSERT INTO` *Table*テーブル`SELECT`としてクエリを書き直してください。|  
|機能|OUTPUT|`OUTPUT` 句はサポートされていません。 クエリから削除します。|  
|機能|不完全な挿入列リスト|`INSERT` ステートメントでは、テーブルのすべての列に値を指定する必要があります。|  
|機能|*関数*|ネイティブ コンパイル ストアド プロシージャ内では、組み込み関数はサポートされません。 ストアド プロシージャから関数を削除します。 サポートされている組み込み関数の詳細については、「[ネイティブコンパイルストアドプロシージャ](../in-memory-oltp/natively-compiled-stored-procedures.md)」を参照してください。|  
...
|機能|ROLLUP|`ROLLUP` は、ネイティブ コンパイル ストアド プロシージャ内の `GROUP BY` 句では使用できません。 プロシージャ`ROLLUP`の定義からを削除します。|  
|機能|CUBE|`CUBE` は、ネイティブ コンパイル ストアド プロシージャ内の `GROUP BY` 句では使用できません。 プロシージャ`CUBE`の定義からを削除します。|  
|機能|GROUPING SETS|`GROUPING SETS` は、ネイティブ コンパイル ストアド プロシージャ内の `GROUP BY` 句では使用できません。 プロシージャ`GROUPING SETS`の定義からを削除します。|  
...
|機能|EXECUTE AS CALLER|`EXECUTE AS` 句は必須です。 ただし `EXECUTE AS CALLER` はサポートされていません。 、 `EXECUTE AS OWNER` `EXECUTE AS` *User*、または`EXECUTE AS SELF`を使用します。|  
