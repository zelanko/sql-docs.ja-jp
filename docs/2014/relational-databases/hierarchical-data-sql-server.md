...
##  <a name="key-properties-of-hierarchyid"></a><a name="keyprops"></a> hierarchyid の主要な特性  
 `hierarchyid`データ型の値は、ツリー階層内の位置を表します。 `hierarchyid` の値には、以下の特性があります。  
...
-   深さ優先順で比較  
     ...
     A と`hierarchyid` **b**の2つの**値を指定**した場合、 **<b**は、ツリーの深さ優先走査で b の前に来ることを意味します。 `hierarchyid` データ型のインデックスは深さ優先順であり、深さ優先検査で近接するノードどうしは、相互に近接して格納されます。 たとえば、あるレコードの子は、そのレコードに隣接して格納されます。  
