...
ms.locfileid: "36071424"
---
# <a name="hierarchical-data-sql-server"></a>階層データ (SQL Server)
  組み込み`hierarchyid`データ型では、階層データの格納とクエリを容易にします。 `hierarchyid` 階層データの最も一般的な種類のツリーを表すに対して最適化されます。  
...
-   深さ優先順で比較  
     ...
     2 つ`hierarchyid`値 **、** と**b**、 **、< b**意味、後に b ツリーの深さ優先検査でします。 `hierarchyid` データ型のインデックスは深さ優先順であり、深さ優先検査で近接するノードどうしは、相互に近接して格納されます。 たとえば、あるレコードの子は、そのレコードに隣接して格納されます。  
...
##  <a name="limits"></a> hierarchyid の制限事項  
 `hierarchyid`データ型には、次の制限。  
  
-   型の列`hierarchyid`ツリーを自動的には表しません。 行と行の間に必要なリレーションシップが反映されるよう、`hierarchyid` 値を生成して割り当てるのは、アプリケーションの役割です。 アプリケーションによっては、別のテーブルに定義されている階層内の位置を示す `hierarchyid` 型の列を持つ場合もあります。  
  
-   生成と割り当てにおいて同時実行を管理するアプリケーションの責任です`hierarchyid`値。 アプリケーションで一意キー制約を使用したり、独自のロジックで一意性を適用したりしない限り、列内の `hierarchyid` 値の一意性は保証されません。  
  
-   によって表される階層リレーションシップ`hierarchyid`値は、外部キー リレーションシップとは適用されません。 階層リレーションシップでは、A に子 B があるとき、A だけを削除し、存在しないレコードに対するリレーションシップを B が引き続き保持することも可能であり、これが適切な場合もあります。 この動作を許容しない場合は、親を削除する前に、アプリケーションで子孫に対するクエリを実行する必要があります。  
...
-   サブツリーのクエリは大幅に高速`hierarchyid`です。  
  
-   直接の子孫のクエリは、`hierarchyid` を使用するとわずかに遅くなります。  
  
-   非リーフ ノードを移動するが遅くなります`hierarchyid`です。  
  
-   非リーフ ノードを挿入する場合、およびリーフ ノードを挿入または移動する場合も、`hierarchyid` を使用する場合と同様に複雑になります。  
  
 次の条件に当てはまるときは、親/子を使用した方がよい場合があります。  
  
-   キーのサイズが非常に重要なとき。 同じノード数の`hierarchyid`値が整数系以上 (`smallint`、 `int`、 `bigint`) 値です。 まれに親/子を使用する唯一の理由のためにこれは`hierarchyid`が大幅に向上する場所の絞り込み I/O と CPU の複雑さ、親/子構造体を使用しているときに必要な共通テーブル式よりもします。  
...
-   非リーフ サブツリーが頻繁に移動し、かつパフォーマンスが非常に重要なとき。 親/子表現では、階層内の行の場所を変更すると、1 行のみが影響を受けます。 内の行の場所を変更する、`hierarchyid`使用量に影響を与えます*n*行、場所*n*はサブツリー内のノードの数が移動されています。  
...
###  <a name="migrating"></a> 親/子から hierarchyid への移行  
 現在、ほとんどのツリーは親/子を使用して表されます。 親/子構造から使用してテーブルに移行する最も簡単な方法`hierarchyid`階層の各レベルのノードの数を追跡する、一時列または一時テーブルを使用することです。 親/子テーブルの移行例については、「 [チュートリアル : hierarchyid データ型の使用](../relational-databases/tables/tutorial-using-the-hierarchyid-data-type.md)」のレッスン 1 を参照してください。  
  
  
###  <a name="BKMK_ManagingTrees"></a> hierarchyid を使用したツリーの管理  
 ただし、`hierarchyid`列が必ずしもを表すツリーをアプリケーションが行われていることを確認できます簡単にします。  
...
###  <a name="findclr"></a> CLR を使用した先祖の検索  
 階層内の 2 つのノードに関連する一般的な操作は、最下位の共通の先祖を見つけることです。 これは、いずれかで記述できます[!INCLUDE[tsql](../includes/tsql-md.md)]または CLR、ため、`hierarchyid`型は両方で使用できます。 パフォーマンスが向上するため、CLR の使用をお勧めします。  
