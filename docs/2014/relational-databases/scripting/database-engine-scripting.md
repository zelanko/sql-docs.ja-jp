...
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] コマンドを実装する一連の [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] コマンドレット。 コマンドレットの 1 つは **Invoke-Sqlcmd**です。 これは実行に使用[!INCLUDE[ssDE](../../includes/ssde-md.md)]クエリを実行するスクリプト、`sqlcmd`ユーティリティです。  
...
-   使用して、**-i * * * input_file*パラメーターを使用して、ファイルを実行する、`sqlcmd`ユーティリティです。  
...
|使用する方法について説明します、`sqlcmd`ユーティリティを実行する[!INCLUDE[tsql](../../includes/tsql-md.md)]対話形式でスクリプトを作成する機能など、コマンド プロンプトからのスクリプト。|[sqlcmd 操作方法のトピック](../../database-engine/sqlcmd-how-to-topics.md)|  
