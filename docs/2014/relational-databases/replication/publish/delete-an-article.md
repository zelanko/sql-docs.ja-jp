...
5.  <xref:Microsoft.SqlServer.Replication.ReplicationObject.IsExistingObject%2A> プロパティを調べて、アーティクルが存在していることを確認します。 このプロパティの値が場合`false`、手順 3. でアーティクルのプロパティの定義が正しくなかったか、アーティクルが存在しません。  
