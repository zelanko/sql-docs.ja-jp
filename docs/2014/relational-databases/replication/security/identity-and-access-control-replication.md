...
# <a name="identity-and-access-control-replication"></a>ID およびアクセス制御 (レプリケーション)
  認証とは、エンティティ (通常、このコンテキストではコンピューター) が別のエンティティ (*プリンシパル*とも呼ばれます) (通常は別のコンピューターまたはユーザー) が要求するユーザーまたは対象となることを確認するプロセスです。 承認とは、認証されたプリンシパルにリソース (ファイル システムのファイルやデータベースのテーブルなど) へのアクセスを認めるプロセスです。  
