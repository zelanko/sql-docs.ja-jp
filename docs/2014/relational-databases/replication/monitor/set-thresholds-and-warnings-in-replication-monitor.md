...
-   [トランザクションパブリケーションのしきい値と警告を設定する](#Transactional)  
  
-   [マージパブリケーションのしきい値と警告の設定](#Merge)  
  
-   [スナップショットパブリケーションのしきい値と警告を設定する](#Snapshot)  
  
##  <a name="Transactional"></a>トランザクションパブリケーションのしきい値と警告を設定するには  
...
4.  
  **[しきい値]** 列で警告のしきい値を設定します。 たとえば、手順 3. で **[待機時間がしきい値を超えた場合に警告します]** チェック ボックスをオンにした場合、 **[しきい値]** 列で **60 秒** の待機時間を選択します。  
...
##  <a name="Merge"></a>マージパブリケーションのしきい値と警告の設定  
...
3.  適切なチェック ボックスをオンにして警告を有効にします。  
    ...
    -   **ダイヤルアップ接続のマージ長がしきい値を超えた場合に警告する**  
  
    -   **LAN 接続のマージ長がしきい値を超えた場合に警告する**  
  
    -   **LAN 接続で1秒あたりにマージされる行数がしきい値未満の場合に警告します**  
  
    -   **ダイヤルアップ接続で1秒間にマージされた行数がしきい値未満の場合に警告します**  
  
4.  
  **[しきい値]** 列で警告のしきい値を設定します。 たとえば、手順 3. で **[ダイヤルアップ接続のマージ長がしきい値を超えた場合に警告します]** を選択した場合、 **[しきい値]** 列では **[10 分]** を選択します。  
...
##  <a name="Snapshot"></a>スナップショットパブリケーションのしきい値と警告を設定する  
...
3.  
  **[サブスクリプションの有効期限がしきい値内で切れる場合に警告します。]** チェック ボックスをオンにし、警告を有効にします。  
  
4.  
  **[しきい値]** 列で警告のしきい値を設定します。 たとえば、 **[しきい値]** 列で **[70%]** の値を選択できます。  
