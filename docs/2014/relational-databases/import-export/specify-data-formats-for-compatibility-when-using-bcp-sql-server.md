...
-   ファイル ストレージ型  
     ...
     *ファイル ストレージ型* は、データ ファイルへのデータの格納方法を記述します。 データとしてエクスポートできますデータ ファイルに、データベース テーブルの型 (ネイティブ形式)、文字表現 (文字形式)、または任意のデータ型が暗黙的な変換がサポートされている場合です。たとえば、コピー、`smallint`として、`int`です。 ユーザー定義のデータ型は、基本データ型としてエクスポートされます。 詳細については、「 [bcp を使用したファイル ストレージ型の指定 &#40;SQL Server&#41;](specify-file-storage-type-by-using-bcp-sql-server.md)」を参照してください。  
...
##  <a name="FieldSpecificPrompts"></a> フィールド固有のプロンプトの概要  
 場合は、対話型`bcp`コマンドには、**で**または**アウト**オプションしますが、フォーマット ファイル スイッチも含まれていません (**-f**) またはデータ形式スイッチ (**-n**、 **-c**、 **-w**、または **-n**)、ソースまたはターゲット テーブル内の各列を上記の各要求されます属性では、有効にします。 各プロンプトで、`bcp`コマンドに基づいて、既定値には、[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]テーブル列のデータ型。 すべての問い合わせに対して既定値を受け入れることは、コマンド ラインでネイティブ形式 (**-n**) を指定するのと同じ結果になります。 各プロンプトには、[*default*] のように既定値が角かっこ付きで表示されます。 表示される既定値を受け入れるには、Enter キーを押します。 既定値以外を指定するには、プロンプトで新しい値を入力します。  
  
### <a name="example"></a>例  
 次の例では、`bcp`一括コマンドからのデータのエクスポート、`HumanResources.myTeam`を対話形式での表に、`myTeam.txt`ファイル。 このテーブルを作成しないと、例を実行できません。 テーブルの詳細とテーブルを作成する方法については、「[HumanResources.myTeam サンプル テーブル &#40;SQL Server&#41;](humanresources-myteam-sample-table-sql-server.md)」を参照してください。  
...
##  <a name="FieldByFieldNonXmlFF"></a> XML 以外のフォーマット ファイルでのフィールドごとのデータの格納  
 結局のところ、テーブルの列が指定されて、`bcp`コマンドでは、必要に応じて、フィールドの情報だけに提供された (前の例を参照) を格納する XML 以外のフォーマット ファイルを生成するように求められます。 フォーマット ファイルを生成することを選択すると、いつでも、そのテーブルからデータをエクスポートしたり、同じような構造のデータを [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]にインポートできます。  
