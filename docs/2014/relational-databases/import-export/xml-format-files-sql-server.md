...
##  <a name="structure-of-xml-format-files"></a><a name="StructureOfXmlFFs"></a> XML フォーマット ファイルの構造  
 ...
 XML フォーマットファイルには、主に次の2つのコンポーネントが \<RECORD> \<ROW> あります。  
  
-   \<RECORD>データファイルに格納されているデータについて説明します。  
  
     各 \<RECORD> 要素には、1つ以上の要素のセットが含まれ \<FIELD> ます。 それらの要素はデータ ファイル内のフィールドに対応します。 基本構文は次のとおりです。  
     ...
     各 \<FIELD> 要素には、特定のデータフィールドの内容が記述されています。 個々のフィールドは、テーブル内の 1 つの列にのみマップできます。 すべてのフィールドを列にマップする必要はありません。  
...
-   \<ROW>ファイルのデータをテーブルにインポートするときに、データファイルからデータ行を構築する方法について説明し [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ます。  
  
     要素には \<ROW> 、要素のセットが含まれてい \<COLUMN> ます。 それらの要素はテーブル列に対応します。 基本構文は次のとおりです。  
     ...
     各 \<COLUMN> 要素は、データファイル内の1つのフィールドにのみマップできます。 \<COLUMN>要素内の要素の順序に \<ROW> より、一括操作によって返される順序が定義されます。 XML フォーマットファイルでは、 \<COLUMN> 一括インポート操作の対象になるテーブルの列とのリレーションシップを持たないローカル名が各要素に割り当てられます。  
  
##  <a name="schema-syntax-for-xml-format-files"></a><a name="SchemaSyntax"></a> XML フォーマット ファイルのスキーマ構文  
 このセクションでは、XML フォーマット ファイルに対する XML スキーマの要素および属性について概要を説明します。 フォーマット ファイルの構文は、操作の方向とは関係がありません。つまり、一括エクスポートでも一括インポートでも構文は同じです。 また、一括インポートで要素 \<ROW> と要素を使用 \<COLUMN> する方法、および要素の xsi: type 値をデータセットに格納する方法についても説明します。  
...
###  <a name="basic-syntax-of-the-xml-schema"></a><a name="BasicSyntax"></a> XML スキーマの基本構文  
 この構文ステートメントでは、要素 ( \<BCPFORMAT> 、 \<RECORD> 、 \<FIELD> 、 \<ROW> 、および \<COLUMN> ) とその基本属性のみが表示されます。  
...
> [!NOTE]  
>  または要素の xsi: type の値に関連付けられている追加の属性については、 \<FIELD> \<COLUMN> このトピックの後半で説明します。  
...
####  <a name="schema-elements"></a><a name="SchemaElements"></a> Schema 要素  
 ...
 \<RECORD .../>  
 1つ以上の要素を含む複合要素を定義 \<FIELD> します。 フォーマット ファイルでフィールドが宣言される順序は、データ ファイルでフィールドが表示される順序と同じです。  
  
 \<FIELD .../>  
 データを含むデータ ファイル内のフィールドを定義します。  
  
 この要素の属性については、このトピックで後述する「[属性 \<FIELD> ](#AttrOfFieldElement)」で説明します。  
  
 \<ROW .../>  
 1つ以上の要素を含む複合要素を定義 \<COLUMN> します。 要素の順序 \<COLUMN> は、レコード定義内の要素の順序とは関係あり \<FIELD> ません。 代わりに、フォーマットファイル内の要素の順序によって、 \<COLUMN> 結果の行セットの列の順序が決定されます。 データフィールドは、対応する \<COLUMN> 要素が要素内で宣言されている順序で読み込まれ \<COLUMN> ます。  
  
 詳細については、このトピックの「[一括インポートでの \<ROW> 要素の使用方法](#HowUsesROW)」を参照してください。  
  
 \<COLUMN>  
 列を要素として定義 \<COLUMN> します ()。 各 \<COLUMN> 要素は、要素 \<FIELD> の SOURCE 属性で ID が指定されている要素に対応 \<COLUMN> します。  
  
 この要素の属性については、このトピックで後述する「[属性 \<COLUMN> ](#AttrOfColumnElement)」で説明します。 また、このトピックの「[一括インポートでの \<COLUMN> 要素の使用方法](#HowUsesColumn)」も参照してください。  
  
 \</BCPFORMAT>  
 フォーマット ファイルを終了するために必要なフォーマット ファイル要素です。  
  
####  <a name="attributes-of-the-field-element"></a><a name="AttrOfFieldElement"></a>要素の属性 \<FIELD>  
 このセクションでは、要素の属性について説明し \<FIELD> ます。次のスキーマ構文にまとめられています。  
...
|MAX_LENGTH **= " *`m`* "**|指定したフィールドに格納できる最大バイト数を定義します。 対象のテーブルがない場合、列の最大長を決めることはできません。 MAX_LENGTH 属性では、出力先の文字列型の列の最大長を制限し、列の値に割り当てる領域を制限しています。 この属性は、SELECT FROM 句で OPENROWSET 関数の BULK オプションを使用している場合に特に有益です。<br /><br /> *m* の値は、正の整数にする必要があります。 既定では、 **char** 列の最大長は 8,000 文字で、 **nchar** 列の最大長は 4,000 文字です。|オプション|  
...
#####  <a name="xsitype-values-of-the-field-element"></a><a name="XsiTypeValuesOfFIELD"></a>要素の Xsi: type 値 \<FIELD>  
 xsi:type 値は、要素のインスタンスのデータ型を特定する XML コンストラクトです (これは属性のように使用します)。 詳細については、このトピックの「xsi:type 値のデータセットへの格納」を参照してください。  
  
 要素の xsi: type 値は、 \<FIELD> 次のデータ型をサポートしています。  
...
####  <a name="attributes-of-the-column-element"></a><a name="AttrOfColumnElement"></a>要素の属性 \<COLUMN>  
 このセクションでは、要素の属性について説明し \<COLUMN> ます。次のスキーマ構文にまとめられています。  
...
|xsi **:** type **= " *`ColumnType`* "**|要素のインスタンスのデータ型を特定する XML コンストラクトです (これは属性のように使用します)。 *ColumnType* の値により、要素のインスタンスで必要なオプションの属性 (下記参照) が決まります。<br /><br /> 注: *ColumnType*とそれらに関連付けられている属性の有効な値を次の表に示します。|オプション|  
...
#####  <a name="xsitype-values-of-the-column-element"></a><a name="XsiTypeValuesOfCOLUMN"></a>要素の Xsi: type 値 \<COLUMN>  
 xsi:type 値は、要素のインスタンスのデータ型を特定する XML コンストラクトです (これは属性のように使用します)。 詳細については、このトピックの「xsi:type 値のデータセットへの格納」を参照してください。  
...
###  <a name="how-bulk-import-uses-the-row-element"></a><a name="HowUsesROW"></a>一括インポートで要素を使用する方法 \<ROW>  
 \<ROW>要素は、一部のコンテキストでは無視されます。 要素が \<ROW> 一括インポート操作に影響するかどうかは、操作の実行方法によって異なります。  
  
-   **bcp** コマンド  
  
     データが対象テーブルに読み込まれると、 **bcp**はコンポーネントを無視し \<ROW> ます。 代わりに、 **bcp** コマンドでは、インポート先テーブルの列の型に基づいてデータが読み込まれます。  
  
-   [!INCLUDE[tsql](../../../includes/tsql-md.md)] ステートメント (BULK INSERT および OPENROWSET の一括行セット プロバイダー)  
  
     データをテーブルに一括インポートする場合、ステートメントはコンポーネントを使用して [!INCLUDE[tsql](../../../includes/tsql-md.md)] \<ROW> 入力行セットを生成します。 また、ステートメントでは、の [!INCLUDE[tsql](../../../includes/tsql-md.md)] 下で指定された列の型と、対象のテーブルの対応する列に基づいて、適切な型変換が実行さ \<ROW> れます。 フォーマット ファイルで指定されている列の型とインポート先のテーブルの列の型が一致しない場合、追加の型変換が実行されます。 この追加の型変換によって、 **bcp**コマンドと比べたときの BULK INSERT または OPENROWSET の一括行セット プロバイダーの動作に矛盾が生じる (精度が低下する) ことがあります。  
  
     要素内の情報を使用すると、 \<ROW> 追加情報を必要とせずに行を構築できます。 このため、SELECT ステートメント (SELECT \* FROM OPENROWSET(BULK *datafile* FORMATFILE=*xmlformatfile*) を使用して行セットを生成することができます。  
  
    > [!NOTE]  
    >  OPENROWSET BULK 句では、フォーマット ファイルが必要です (フィールドのデータ型から列のデータ型に変換する処理には、XML フォーマット ファイルが必要であることに注意してください)。  
  
###  <a name="how-bulk-import-uses-the-column-element"></a><a name="HowUsesColumn"></a>一括インポートで要素を使用する方法 \<COLUMN>  
 テーブルにデータを一括インポートする場合、 \<COLUMN> フォーマットファイルの要素は、次のように指定することによって、データファイルフィールドをテーブル列にマップします。  
...
-   列の型 (この型はフィールドのデータ型を適切な列のデータ型に変換する際に使用します)。  
 ...
 同様に、テーブルからデータを一括エクスポートする場合、 \<COLUMN> フォーマットファイル内の各列は、入力テーブル行の列を出力データファイル内の対応するフィールドにマップします。  
 ...
 **データファイル (レコード):** Age \<tab> Firstname \<tab> Lastname\<return>  
 ...
 **データファイル**(レコード): Age \<tab> Lastname \<tab> Firstname\<return>  
 ...
 **データファイル (レコード):** Age \<tab> employeeID \<tab> Firstname \<tab> Lastname\<return>  
...
###  <a name="d-mapping-field-xsitype-to-column-xsitype"></a><a name="MapXSItype"></a> D. Xsi \<FIELD> : type から \<COLUMN> xsi: type へのマッピング  
