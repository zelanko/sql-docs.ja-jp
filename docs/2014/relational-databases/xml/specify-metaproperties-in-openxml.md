...
|**\@mp:prev**|ノードよりも前の兄弟を格納します。 これにより、ドキュメント内の要素の順序に関する情報が得られます。<br /><br /> **\@mp:prev**直前の兄弟を持つ、同じ親要素の XML ID が含まれています。 要素が兄弟リストの先頭にある場合 **\@mp:prev**は NULL です。|  
...
|親メタプロパティ属性|説明|  
|-----------------------------------|-----------------|  
|**\@mp:parentid**|対応する **./\@mp:id**|  
|**\@mp:parentlocalname**|対応する **./\@mp:localname**|  
|**\@mp:parentnamespacerui**|対応する **./\@mp:namespaceuri**|  
|**\@mp:parentprefix**|対応する **./\@mp:prefix**|  
...
-   **Id**列にマップされます、  **\@mp:id**メタプロパティ属性と要素のシステム生成の一意な XML ID が、列に含まれていることを示します。  
  
-   **親**列にマップされます **\@mp:parentid**列に、要素の親の XML ID が含まれることを示します。  
  
-   **ParentLocalName**列にマップされます **\@mp:parentlocalname**と列に、親のローカル名が含まれていることを示します。  
 ...
 **コメント**列がマップすることによって、オーバーフロー列として識別される、  **\@mp:xmltext**メタプロパティです。 *flags* パラメーターは **9** (XML_ATTRIBUTE and XML_NOCOPY) に設定されています。 この設定は、 **属性中心** のマッピングであり、未使用データのみがオーバーフロー列にコピーされることを示しています。  
  
 SELECT ステートメントは OPENXML で提供される行セットを返します。  
  
 この例で、  **\@mp:parentlocalname** 、列のメタプロパティが設定されます**ParentLocalName**、OPENXML によって生成された行セット。 その結果、この列には親要素のローカル名が含まれます。  
  
 行セットには他に 2 つの列 ( **parent** と **comment**) が指定されています。 **親**列にマップされます **\@mp:parentid**列に、要素の親要素の XML ID が含まれることを示します。 Comment 列がマップすることによって、オーバーフロー列として識別される、  **\@mp:xmltext**メタプロパティです。  
