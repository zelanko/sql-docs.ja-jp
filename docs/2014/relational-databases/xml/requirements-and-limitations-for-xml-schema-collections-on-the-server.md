...
|ファセット長|**長さ**、 **minLength**、および**maxLength**としてファセットが格納されている、`long`型です。 この型は 32 ビット型です。 したがって、これらの値の許容値の範囲は 2<sup>^</sup>31 です。|  
...
|タイム ゾーン情報|[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]以降のバージョンでタイム ゾーン情報が完全にサポートおよび`xs:date`、 `xs:time`、および`xs:dateTime`値について、XML スキーマ検証。 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] 後方互換性モードでは、タイム ゾーン情報は常に協定世界時 (グリニッジ標準時) を基準として正規化されます。 `dateTime` 型の要素の場合は、サーバー側でオフセット値 ("-05:00") を使用して対応する GMT 時を返すことによって、指定された時間を GMT に変換します。|  
