...
### <a name="example-computed-column-based-on-the-xml-data-type-method"></a>例:Xml データ型のメソッド ベースの列の計算  
...
### <a name="example-queries-on-a-computed-column-based-on-xml-data-type-methods"></a>例:クエリ、計算列に基づいて xml データ型メソッド  
...
### <a name="example-create-a-property-table"></a>例:プロパティ テーブルを作成します。  
...
### <a name="example-create-a-user-defined-function-to-generate-a-rowset-from-an-xml-instance"></a>例:XML インスタンスから行セットを生成するユーザー定義関数を作成します。  
...
### <a name="example-create-triggers-to-populate-a-property-table"></a>例:プロパティ テーブルを設定するトリガーを作成します。  
...
### <a name="example-find-xml-instances-whose-authors-have-the-same-first-name"></a>例:著者が同じ名を持つ XML インスタンスを検索します。  
...
### <a name="example-solution-using-the-clr-streaming-table-valued-function"></a>例:CLR テーブル値関数のストリーミングを使用したソリューション  
