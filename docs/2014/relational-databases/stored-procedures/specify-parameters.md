...
## <a name="passing-values-into-parameters"></a>パラメーターに値を渡す処理  
 プロシージャ呼び出しで指定されるパラメーター値は、定数か変数である必要があります。関数名をパラメーター値として使用することはできません。 ユーザー定義変数を指定できますかなどのシステム変数\@ \@spid。  
...
## <a name="specifying-parameter-names"></a>パラメーター名の指定  
 ときにプロシージャを作成し、パラメーター名を宣言する、パラメーター名が、1 つで始める必要があります\@文字し、プロシージャのスコープ内で一意である必要があります。  
  
 パラメーターに明示的に名前を付け、プロシージャ呼び出しで各パラメーターに適切な値を代入することで、パラメーターを任意の順序で指定できます。 たとえば場合、プロシージャ**my_proc**という 3 つのパラメーターを必要**\@最初**、  **\@2 番目**、および **\@3 番目**プロシージャに渡された値をなど、パラメーター名に割り当てることができます。 `EXECUTE my_proc @second = 2, @first = 1, @third = 3;`  
  
> [!NOTE]  
>  1 つのパラメーター値を **/@parameter =***value* の形式で指定した場合は、後続のパラメーターもすべてこの形式で指定する必要があります。 パラメーターの値がフォームに渡されないかどうか **\@パラメーター = * * * 値*、CREATE PROCEDURE ステートメント内のパラメーターが示されている、(左右から) と同じ順序で値を指定する必要があります。  
  
> [!WARNING]  
>  フォームで渡す **\@パラメーター = * * * 値*スペルが間違っているパラメーターを使用すると、[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]エラーが生成され、プロシージャの実行を防ぐためにします。  
...
```  
 ...
 既定値が指定されているパラメーターは省略できますが、パラメーターの一覧を切り捨てることしかできません。 たとえば、プロシージャに 5 つのパラメーターがある場合は、4 番目と 5 番目のパラメーターを両方とも省略できます。 ただし、4 番目のパラメーターことはできませんがスキップされる 5 番目のパラメーターが含まれている限り、フォームでパラメーターを指定しない限り、**\@パラメーター = * * * 値*します。  
...
```  
 ...
 `usp_GetList` を実行し、原価が $700 未満である [!INCLUDE[ssSampleDBCoShort](../../includes/sssampledbcoshort-md.md)] 製品 (自転車) の一覧を返します。 出力パラメーター **\@コスト**と **\@compareprices**内のメッセージを返すにはフロー制御言語と共に使用、**メッセージ**ウィンドウです。  
  
> [!NOTE]  
>  プロシージャの作成中にも変数の使用中にも、OUTPUT 変数を定義する必要があります。 パラメーター名と変数名が一致する必要はありません。 データ型とパラメーターの位置がただし、一致する必要があります (しない限り、  **\@listprice =** *変数*使用されます)。  
