...
|**EventSubClass**|**nvarchar**|イベント サブクラスの種類です。各イベント クラスについての詳細な情報を提供します。 この列には次の値が含まれます。<br /><br /> **Local**: 選択されるルートには LOCAL アドレスが含まれます。<br /><br /> **リモート**: 選択されるルートは LOCAL 以外のアドレス。<br /><br /> **遅延**: 転送が無効か、またはある一致するルートが存在しないため、メッセージが遅延します。|21|はい|  
