...
|**EventSubClass**|**nvarchar**|イベント サブクラスの種類です。各イベント クラスについての詳細な情報を提供します。 この列には次の値が含まれます。<br /><br /> **ローカル**:選択されるルートには LOCAL アドレスが含まれます。<br /><br /> **リモート**:選択されるルートには、LOCAL 以外のアドレスが含まれます。<br /><br /> **遅延**:転送が無効か、一致するルートが存在しないので、メッセージが遅延されます。|21|はい|  
