...
# <a name="audit-database-scope-gdr-event-class"></a>Audit Database Scope GDR イベント クラス
  **Audit database Scope GDR**イベントクラスは、データベースに対する権限の許可など[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]のデータベースのみのアクションで、ステートメント権限に対して GRANT、REVOKE、または DENY が実行されるたびに発生します。  
...
|**TextData**|**ntext**|Grant/Revoke/Deny ステートメントの SQL テキストです。|1 で保護されたプロセスとして起動されました|はい|  
