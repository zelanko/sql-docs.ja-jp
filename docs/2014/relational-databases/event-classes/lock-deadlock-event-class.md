...
|DatabaseID|`int`|ロックが取得されていたデータベースの ID です。 [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)]では、ServerName データ列がトレースにキャプチャされ、そのサーバーが使用可能な場合、データベースの名前が表示されます。 データベースに対応する値は、DB_ID 関数を使用して特定します。|3|はい|  
...
|TextData|`ntext`|取得されていたロックの種類に依存するテキスト値です。|1 で保護されたプロセスとして起動されました|はい|  
