...
# <a name="blocked-process-report-event-class"></a>Blocked Process Report イベント クラス
  ブロックされた**Process Report**イベントクラスは、指定された時間を超えてタスクがブロックされたことを示します。 このイベント クラスには、システム タスクやデッドロックを検出できないリソースを待機しているタスクは含まれません。  
  ...
  **blocked process threshold** オプションの設定に関する詳細については、「 [blocked process threshold サーバー構成オプション](../../database-engine/configure-windows/blocked-process-threshold-server-configuration-option.md)」を参照してください します。  
...
|**EndTime**|**DATETIME**|イベントの終了時刻。 
  **SQL:BatchStarting** や **SP:Starting**などの開始イベント クラスについては、この列に値が格納されません。|15|はい|  
...
|**LoginSid**|**絵**|ログインしたユーザーのセキュリティ識別子 (SID)。 このイベントは必ずシステム スレッドから報告されます。 IsSystem は 1、SID は sa です。|41|はい|  
...
|**ObjectID**|**int**|ロックを取得したオブジェクトのシステム割り当て ID (使用可能かつ適用可能な場合)。|22|はい|  
