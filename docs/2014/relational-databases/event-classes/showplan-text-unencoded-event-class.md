...
# <a name="showplan-text-unencoded-event-class"></a>Showplan Text (Unencoded) イベント クラス
  Showplan Text (エンコードされていない) [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]イベントクラスは、によって SQL ステートメントが実行されるときに発生します。 このイベント クラスは、Showplan Text イベント クラスと同じです。ただし、イベント情報がバイナリ データとしてではなく、文字列としてフォーマットされます。  
