...
|EventSubClass|`nvarchar`|イベント サブクラスの種類です。各イベント クラスについての詳細な情報を提供します。 この列には次の値が含まれます。<br /><br /> 作成されたテーブル: パラメーター テーブルがデータベースに作成されたことを示します。<br /><br /> テーブルの削除の試行: データベースがリソースを解放、未使用のパラメーター テーブルを自動的に削除しようとしたことを示します。<br /><br /> テーブルのドロップ操作が失敗しました。 データベースが未使用のパラメーター テーブルを削除しようとした、失敗したことを示します。 リソースを解放するために、パラメーター テーブルを削除するスケジュールが [!INCLUDE[ssDE](../../includes/ssde-md.md)] によって自動的に組み直されます。<br /><br /> テーブルの削除: データベースが、パラメーター テーブルを正常に削除されたことを示します。<br /><br /> テーブルのピン留め: パラメーター テーブルが内部処理によって現在の使用量のマークされていることを示します。<br /><br /> 固定テーブル: パラメーター テーブルが固定されたことを示します。 内部処理によるこのテーブルの使用は完了しています。<br /><br /> 増加ユーザー数: パラメーター テーブルを参照するクエリ通知サブスクリプションの数が増加したことを示します。<br /><br /> デクリメントされたユーザーの数: パラメーター テーブルを参照するクエリ通知サブスクリプションの数が減少したことを示します。<br /><br /> LRU カウンターのリセット: パラメーター テーブルの使用状況カウントがリセットされたことを示します。<br /><br /> クリーンアップ タスクが開始されました。 このパラメーター テーブル内のすべてのサブスクリプションのクリーンアップが開始された時点を示します。 このクリーンアップは、データベースを起動したとき、またはこのパラメーター テーブルのサブスクリプションが基づいているテーブルが削除されたときに発生します。<br /><br /> クリーンアップ タスクが完了しました: このパラメーター テーブル内のすべてのサブスクリプションのクリーンアップが終了したことを示します。|21|はい|  
