...
ms.locfileid: "62663230"
---
# <a name="cursorrecompile-event-class"></a>CursorRecompile イベント クラス
  **Cursor recompile**イベントクラスは、アプリケーションプログラミングインターフェイス (API) のカーソルで発生するカーソルの再コンパイルイベントを表します。 カーソルの再コンパイルイベントは[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] 、がスキーマ変更のために transact-sql カーソルを再コンパイルしたときに発生します。  
  
 
  **CursorRecompile** イベント クラスは、カーソルのパフォーマンスを記録しているトレースに含めます。 
  **CursorRecompile** イベント クラスをトレースに含めた場合、発生するオーバーヘッドの量は、トレース中にデータベースに対してカーソルを使用する頻度によって異なります。 カーソルの使用頻度が高い場合は、トレースによってパフォーマンスが大幅に低下する可能性があります。  
