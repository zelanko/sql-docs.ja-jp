...
|CPU|`int`|イベントによって使用された CPU 時間。 
  [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]からは、マイクロ秒単位。 それより前のバージョンではミリ秒単位。|18|はい|  
...
|Duration|`bigint`|イベントによって使用された時間。 
  [!INCLUDE[ssKilimanjaro](../../includes/sskilimanjaro-md.md)]からは、マイクロ秒単位。 それより前のバージョンではミリ秒単位。|13|はい|  
...
|TextData|`ntext`|リモート プロシージャ コールのテキスト。|1 で保護されたプロセスとして起動されました|はい|  
