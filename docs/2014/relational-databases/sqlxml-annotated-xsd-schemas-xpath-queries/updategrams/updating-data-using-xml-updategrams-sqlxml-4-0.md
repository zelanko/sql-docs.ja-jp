 ...
 ** \<** `updg:id` ** \<Before>** および** \<after>** ブロックに複数の要素が指定されている場合は、属性を使用して、>ブロックの** \<前>** および after に行をマークします。 処理ロジックでは、この情報を使用して、 ** \<before>** ブロックのペアのレコードと** \<after>** ブロックのレコードを判別します。  
...
-   アップデートグラムのキー フィールドに特定の値が 1 つ以上提供されている。  
 ...
 どちらの場合も、アップデートグラムでは、に指定されているキー `sql:key-fields`列を使用して、>ブロックの** \<前の>** と** \<後**の要素をペアにします。  
  
 マッピング スキーマで `sql:key-fields` によりキー列が指定されていない場合、またはアップデートグラムでキー列の値を更新する場合は、`updg:id` を指定する必要があります。  
  
 Before>および** \<after>** ブロックで識別されるレコードは、同じ順序である必要はありません。 ** \<** 属性`updg:id`は、 ** \<に**指定されている要素と>ブロックの** \<後**>との間の関連付けを強制します。  
  
 Before>block に1つの要素を指定し、 ** \<after>** ブロック内で対応する要素を 1 `updg:id`つだけ指定した場合、を使用する必要はありません。 ** \<** ただし、あいまいになるのを避けるため、この場合も `updg:id` を指定することをお勧めします。  
...
-   10:00AM から始まる、"Late Morning" という名前の新しい勤務時間を挿入する。  
 ...
 アップデートグラムでは、 `updg:id`属性によって、>ブロックの** \<前の>** と** \<後**の要素の間の関連付けが作成されます。  
...
```  
 ...
 `updg:id`属性が\<、 \< **after>ブロックの humanresources.employee> 要素の2番目のインスタンスで、before>ブロックの humanresources.employee> 要素の最初のインスタンスとペアになっていることに注意して\<** ください。 ** \<**  
