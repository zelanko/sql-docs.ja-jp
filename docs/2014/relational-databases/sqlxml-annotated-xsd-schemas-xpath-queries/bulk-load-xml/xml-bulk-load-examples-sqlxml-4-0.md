...
5.  VBScript コードを実行します。  
     ...
     スキーマは、対応するを指定する必要があります`sql:datatype`の**CustomerID**ときに属性の値は、 **CustomerID**として中かっこ ({および}) などが含まれる GUID として指定されます。  
    ...
    ```  
     ...
     ときに`sql:datatype`として列の型を識別する指定`uniqueidentifier`、一括読み込み操作は、中かっこを削除します ({および}) から、 **CustomerID**列に挿入する前に値。  
...
## <a name="j-bulk-loading-in-xml-data-type-columns"></a>J. xml データ型の列に一括読み込みを行う  
 マッピング スキーマが指定されている場合、 [xml データ型](/sql/t-sql/xml/xml-transact-sql)列を使用して、`sql:datatype="xml"`注釈、XML 一括読み込みは、ソース ドキュメントからマップされたフィールドの XML 子要素をこの列にコピーできます。  
  
 次の XSD スキーマを考えてみます。この XSD スキーマでは、サンプル データベース AdventureWorks の Production.ProductModel テーブルのビューがマップされます。 CatalogDescription フィールドは、このテーブルで`xml`データ型にマッピングされますが、  **\<Desc >** 要素を使用して、`sql:field`と`sql:datatype="xml"`注釈。  
