...
#### <a name="query-statistics-history-report"></a>クエリ統計の履歴レポート  
 総 CPU 時間ごとの上位のクエリのグラフには、選択した時間範囲の各クエリの相対的な負荷が総 CPU 使用率に基づいて示されます。 相対的なクエリの負荷の別のビューを表示するには、グラフの下のハイパーリンクのいずれかをクリックします。**期間**、 **I/O の合計**、**物理読み取り**、または**論理書き込み**します。  
...
#### <a name="query-details-subreport"></a>クエリの詳細サブレポート  
 ...
 クエリ プランと実行ごとの平均実行時間のグラフが表示されます。 相対的なクエリ プランのコストの別のビューを表示するには、グラフの下に表示されるハイパーリンクのいずれかをクリックします。**期間**、**物理読み取り**、または**論理書き込み**します。 グラフ線はアクティブで、任意の箇所をクリックするとクエリ プランの詳細サブレポートを開くことができます。  
