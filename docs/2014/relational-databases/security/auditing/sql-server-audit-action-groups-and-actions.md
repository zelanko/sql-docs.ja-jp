...
|DATABASE_PRINCIPAL_IMPERSONATION_GROUP|このイベントは、データベーススコープ内で、EXECUTE AS \<user> や SETUSER などの権限の借用がある場合に発生します。 [Audit Database Principal Impersonation Event Class](../../event-classes/audit-database-principal-impersonation-event-class.md)と同じです。|  
