...
2.  **オブジェクトの追加**ダイアログ ボックスで、次のオプションのいずれかを選択します。**特定のオブジェクト.**、**型のすべてのオブジェクト。**、または **サーバー * * * server_name*します。 [!INCLUDE[clickOK](../../../includes/clickok-md.md)]  
