...
|ISSCommandWithParameters::SetParameterInfo|テーブル値パラメーターのコマンド パラメーターを指定する、コンシューマーで指定します、パラメーターの型"table"または"DBTYPE_TABLE"として、 *pwszName* DBPARAMBINDINFO 構造体のメンバー。 *UlParamSize*に設定されている ~ 0。 詳細については、「テーブル値パラメーターの指定」を参照してください[Executing Commands Containing Table-Valued パラメーター](executing-commands-containing-table-valued-parameters.md)します。|  
