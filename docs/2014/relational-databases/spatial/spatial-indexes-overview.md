...
>  空間インデックスのための空間の分解は、アプリケーション データが使用する測定単位に依存しません。  
 ...
 ![4 × 4 のレベル 1 グリッドに配置された多角形と直線](../../database-engine/media/spndx-level-1-objects.gif "4 × 4 のレベル 1 グリッドに配置された多角形と直線")  
...
#### <a name="covering-rule"></a>カバリング ルール  
 ...
 ![カバリングの最適化](../../database-engine/media/spndx-opt-covering.gif "カバリングの最適化")  
...
#### <a name="deepest-cell-rule"></a>最下位のセルのルール  
 ...
 次の図では、比較的小さな菱形の多角形がテセレーションされています。 このインデックスで使用されているオブジェクトごとのセル数の制限は既定の 16 で、この小さなオブジェクトではその制限に達しないため、 テセレーションはレベル 4 まで続けられます。 多角形は、レベル 3 のセルを次のレベル 1 に存在します。4、4.4、および 4.4.10 と 4.4.14 します。 テセレーションは、最下位のセルのルールを使用して、12 個のレベル 4 のセルのみをカウントします。4.4.10.13-15 と 4.4.14.1 ~ 3、4.4.14.5 ~ 7、および 4.4.14.9-11 します。  
...
-   *ジオメトリ グリッド テセレーション*、用のスキームは、`geometry`データ型。  
  
-   *地理グリッド テセレーション スキーム*の列に適用されます、`geography`データ型。  
...
3.  平面化された四角錐を結合して、非ユークリッド平面を形成します。  
 ...
 ![平面への楕円体の投影](../../database-engine/media/spndx-geodetic-projection.gif "平面への楕円体の投影")  
...
###  <a name="geometry"></a> 空間インデックスでサポートされるジオメトリ メソッド  
 空間インデックスでは、特定の条件下で、次のセット指向のジオメトリ メソッドをサポートします。STContains()、STDistance()、STEquals()、STIntersects()、STOverlaps()、STTouches()、および STWithin() します。 空間インデックスでサポートされるために、これらのメソッドは、クエリの WHERE 句または JOIN ON 句内で、次の一般的な述語の形式で使用する必要があります。  
...
###  <a name="geography"></a> 空間インデックスでサポートされる地理メソッド  
 特定の条件下では、空間インデックスは、次のセット指向の地理メソッドをサポートします。Stintersects()、Stequals()、STDistance(). 空間インデックスでサポートされるために、これらのメソッドは、クエリの WHERE 句内で、次の一般的な述語の形式で使用する必要があります。  
