...
-   [Reporting Services での SharePoint ライブラリの配信](sharepoint-library-delivery-in-reporting-services.md)SharePoint ライブラリへのサブスクリプションの配信について説明します。  
  
-   [データドリブンサブスクリプション](data-driven-subscriptions.md)データドリブンサブスクリプションを使用して実行時にレポート出力をカスタマイズする方法について説明します。  
