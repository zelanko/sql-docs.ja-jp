...
-   セクションの各要素の説明を`URLReservations`表示するには、オンライン ブックの[RSReportServer 構成ファイル](../report-server/rsreportserver-config-configuration-file.md)を[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]参照してください。  
  
-   `UrlString`要素のみの構文の詳細については、「 [SSRS 構成マネージャー &#40;URL 予約構文&#41;](url-reservation-syntax-ssrs-configuration-manager.md)」を参照してください。  
...
#### <a name="urls-for-report-access"></a>レポートにアクセスするための URL  
 ...
 ほとんどの場合、`UrlRoot` を変更する必要はありません。 ただし、レポート サーバーに完全修飾 URL を使用してアクセスし、ホスト ヘッダーを使用する URL を完全修飾サイト名に構成しなかった場合は、RSReportServer.config を手動で編集して、`UrlRoot``https://www.adventure-works.com/mywebapp/reportserver`レポートの表示に使用される完全修飾レポート サーバー URL に設定する必要があります (たとえば)。  
