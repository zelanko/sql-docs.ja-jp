...
#### <a name="urls-for-report-access"></a>レポートにアクセスするための URL  
 ...
 ほとんどの場合、`UrlRoot` を変更する必要はありません。 ただし、完全修飾 URL を使用してレポートサーバーにアクセスし、ホストヘッダーを使用する URL を完全修飾サイト名に構成しなかった場合は、RSReportServer を手動で編集して、 `UrlRoot`をレポートの表示に使用される完全に修飾されたレポートサーバー URL (など`https://www.adventure-works.com/mywebapp/reportserver`) に設定する必要があります。  
