...
# <a name="tutorial-creating-a-matrix-report-report-builder"></a>チュートリアル: マトリックス レポートの作成 (レポート ビルダー)
 ...
 このチュートリアルで作成するレポートの拡張のバージョンは、サンプルとして使用可能な[!INCLUDE[ssCurrent](../includes/sscurrent-md.md)]レポート ビルダーでレポートします。 このサンプル レポートおよびその他のダウンロードの詳細については、次を参照してください。[レポート ビルダーのサンプル レポート](http://go.microsoft.com/fwlink/?LinkId=184851)です。  
...
6.  [マトリックスのセルを結合します。](#MergeCells)  
  
7.  [レポートのヘッダーを追加およびレポート タイトル](#HeaderTitle)  
 ...
 **作業の開始**レポート ビルダー ダイアログ ボックスが共有データ ソースを選択し、埋め込みデータセットを作成およびをマトリックスにデータを表示します。  
...
1.  **[スタート]** ボタンをクリックし、 **[プログラム]**、 **[Microsoft SQL Server 2012 レポート ビルダー]** の順にポイントして、 **[レポート ビルダー]** をクリックします。  
    ...
    > [!NOTE]  
    >  **[作業の開始]** ダイアログ ボックスが表示されます。 インストールされていない場合、レポート ビルダーのボタンをクリックして**新規**です。  
...
6.  **データ ソースへの接続の選択** ページで、既存のデータ ソースを選択またはレポート サーバーを参照して、データ ソースを選択します。 使用できるデータ ソースがなく、レポート サーバーにもアクセスできない場合は、代わりに埋め込みデータ ソースを使用できます。 埋め込みデータ ソースの作成の詳細については、次を参照してください。[チュートリアル: 基本的なテーブル レポートを作成する&#40;レポート ビルダー&#41;](../reporting-services/tutorial-creating-a-basic-table-report-report-builder.md)です。  
...
4.  Product をドラッグして**列グループ、** して Subcategory の下に配置します。  
  
     内のフィールドが表示される順序**列グループ**グループ階層を定義します。  
...
12. [スタイル] ページのスタイル ペインで、次のように選択します。**スレート**です。  
...
3.  **[ホーム]** タブの **[数値]** グループで、 **[通貨]** をクリックします。 セルの表示が、通貨の書式に変更されます。  
     ...
     地域設定が英語 (米国) の場合、既定のサンプル テキストは **[$12,345.00]** です。 通貨値の例が表示されない場合はクリックして**プレース ホルダーのスタイル**で、**番号**グループ化、およびをクリックして**サンプル値**です。  
  
4.  `[SalesDate]`が格納されたセルをクリックします。  
  
5.  **数**選択ドロップダウン リストから、グループ**日付**です。  
...
> [!NOTE]  
>  他の曜日のデータが含まれている場合は、レポートにそれらの曜日の列も表示されます。 各列が列ヘッダーを`Sales`、および販売区域ごとの売上の合計。  
...
4.  セルを右クリックし、をクリックして**セルの結合**です。  
  
5.  コーナー セルを右クリックし、をクリックして**テキスト ボックスのプロパティ**です。  
  
6.  **[塗りつぶし]** タブをクリックします。  
  
7.  をクリックします (***fx***) ボタンをクリックして**塗りつぶしの色**です。  
...
2.  含むレポート本文の上部にあるテキスト ボックスをクリックして**タイトルを追加するをクリックして**、Del キーを押します。  
  
3.  **挿入** タブ、リボンのをクリックして**ヘッダー**  をクリックし、**ヘッダーの追加**です。  
...
6.  入力したテキスト、右クリックし、クリックして選択**テキスト プロパティ**です。  
  
    > [!NOTE]  
    >  複数の文字の書式を同時に設定するには、文字が連続している必要があります。  
  
7.  **テキスト プロパティ**ダイアログ ボックスで、をクリックして**フォント**です。  
  
8.  **フォント**一覧で、 **Times New Roman**; で**サイズ**選択**24 pt**の**色**の選択**茶色**、し、**スタイル**選択**斜体**です。  
...
3.  プロパティ ペインで、WritingMode プロパティを見つけてそのドロップダウン リストで選択**Rotate270**です。  
  
     [プロパティ] ペインが表示されていない場合は、リボンの **[表示]** タブの **[プロパティ]** チェック ボックスをオンにします。  
  
4.  CanGrow プロパティに設定されていることを確認`True`です。  
