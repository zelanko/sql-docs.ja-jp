...
##  <a name="bkmk_trace_configuration_settings"></a>トレース構成設定  
 トレースログの動作は、構成ファイル**Reportingservices**によって管理されます。構成ファイルは、次のフォルダーパスにあります。  
  
 `\Program Files\Microsoft SQL Server\MSRS12.<instance name>\Reporting Services\ReportServer\bin`.  
  
 次の例では、`RStrace` の設定の XML 構造を示しています。 
  `DefaultTraceSwitch` の値によって、ログに追加される情報の種類が決まります。 
  `Components` 属性を除き、`RStrace` の値は構成ファイル間で同じになります。  
...
|**/Db**|ログ ファイル名の最初の部分を指定します。 
  `Prefix` で指定した値が付加されて、完全な名前になります。|  
