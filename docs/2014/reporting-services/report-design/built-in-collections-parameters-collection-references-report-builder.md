...
|例|説明|  
|-------------|-----------------|  
|`=Parameters!` *\<ParameterName>* `.IsMultiValue`|`False` が返されます。<br /><br /> パラメーターが複数値であるかどうかを確認します。 の`True`場合、パラメーターは複数値で、オブジェクトのコレクションです。 の`False`場合、パラメーターは単一の値で、は単一のオブジェクトです。|  
