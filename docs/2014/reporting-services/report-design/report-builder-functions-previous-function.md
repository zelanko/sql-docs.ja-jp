 ...
 *スコープ (scope)*  
 (`String`) 省略可。 式によって指定された前の値の`Nothing`取得元となるスコープを指定する、グループまたはデータ領域の名前**、または null (で[!INCLUDE[vbprvb](../../includes/vbprvb-md.md)]は) です。  
...
> [!NOTE]  
>  関数`Previous`は、詳細グループ内のフィールド参照のみをサポートします。 たとえば、詳細グループのテキスト ボックスの場合、 `=Previous(Fields!Quantity.Value)` では、直前の行から `Quantity` フィールドのデータが返されます。 この式が先頭行にある場合は、NULL (`Nothing` では [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)]) が返されます。  
  
 *Expression*に既定のスコープを使用する集計関数が含ま`Previous`れている場合、は、集計関数の呼び出しで指定されたスコープの前のインスタンス内のデータを集計します。  
  
 *Expression*に既定以外のスコープを指定する集計関数が含まれている** 場合、 `Previous`関数の scope パラメーターは、集計関数の呼び出しで指定されたスコープのスコープを含んでいる必要があります。  
  
 式パラメーター `Level`で`InScope`は`Aggregate` 、 `Previous` 、、および関数は** 使用できません。 集計関数に *recursive* パラメーターを指定することはサポートされていません。  
