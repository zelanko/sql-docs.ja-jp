...
## <a name="remarks"></a>コメント  
 ...
 `Last`関数は、現在の (既定) のスコープ以外にグループ化フィルター式では使用できません。  
  
 使用することも`Last`から最後の値を返し、ページ ヘッダーで、`ReportItems`ページの最初と最後のエントリを表示する辞書形式のヘッダーを生成するためにページのコレクション。  
