...
ms.locfileid: "36179251"
---
# <a name="tutorial-format-text-report-builder"></a>チュートリアル: テキストの書式設定 (レポート ビルダー)
  このチュートリアルでは、さまざまな方法でテキストの書式設定を訓練ことができます。 空のレポートと、データ ソースおよびデータセットを設定した後、必要な手順を選んで進めることができます。  
 ...
 このチュートリアルで作成するレポートについては、[!INCLUDE[ssCurrent](../includes/sscurrent-md.md)] レポート ビルダーのサンプル レポートに発展版が含まれています。 このサンプル レポートおよびその他のダウンロードの詳細については、次を参照してください。[レポート ビルダーのサンプル レポート](http://go.microsoft.com/fwlink/?LinkId=184851)です。  
  
##  <a name="BackToTop"></a> 学習する内容  
  
### <a name="set-up-the-report"></a>レポートを設定する  
 1. [空のレポート データを作成、ソースおよびデータセット](#CreateReport)  
  
 2. [(正しくない、し、正しく)、レポート デザイン画面にフィールドを追加します。](#AddField)  
...
##  <a name="CreateReport"></a> 空のレポート データを作成、ソースおよびデータセット  
...
1.  [レポート データ] ペインからデザイン画面に **FullName** フィールドをドラッグします。  
     ...
     レポート ビルダーの式として表されることにテキスト ボックスを作成する\<Expr >。  
  
2.  **[実行]** をクリックします。  
  
     1 つのレコードがあることに注意してください**Fernando Ross**クエリの最初のレコードが表示されます。 フィールドの表示が繰り返されず、フィールド内の他のレコードが表示されません。  
...
##  <a name="AddTable"></a> レポート デザイン画面にテーブルを追加します。  
 ハイパーリンクと回転したテキストを配置する場所を用意するために、このテーブルを作成します。  
  
#### <a name="to-add-a-table-to-the-report"></a>テーブルをレポートに追加するには  
  
1.  **挿入** メニューのをクリックして**テーブル**、クリックして**テーブル ウィザード**です。  
  
2.  **データセットの選択**ページをクリックして、新しいテーブル/マトリックス ウィザードの**このレポートまたは共有データセット内の既存のデータセットを選択**、] をクリック **[TextDataset (このレポートの場合)**、クリックして**次**です。  
  
3.  **フィールドの配置** ページで、ドラッグ、 **Territory**、 **LinkText**、および**製品**フィールドを**行グループ**、ドラッグ、 **Sales**フィールドを**値**、順にクリック**次**です。  
  
4.  **のレイアウトを選択** ページで、クリア、**グループの展開/折りたたみ**チェック ボックスをクリックしてテーブル全体を表示できるように**次**です。  
  
5.  **スタイルを選択する** ページで、をクリックして**スレート**、順にクリック**完了**です。  
  
6.  テーブルをドラッグして、タイトルのブロックの下へ移動します。  
  
7.  **[実行]** をクリックします。  
  
     テーブルは問題ないように見えますが、合計行が 2 か所に含まれています。 **LinkText**フィールド合計行は必要ありません。  
  
8.  **[デザイン]** をクリックしてデザイン ビューに戻ります。  
  
9. 含むテキスト ボックスを右クリックして`[LinkText]`、 をクリック**セルの分割**です。  
  
10. 下の空のセルを選択して、`[LinkText]`セル、SHIFT キーを押しながら、その右側に 2 つのセルを選択:**合計**セル、**製品**列と`[Sum(Sales)]`内のセル**Sales**列です。  
  
11. これら 3 つのセルを選択、それらのセルの 1 つを右クリックし、をクリックして**行の削除**です。  
...
3.  **テキスト ボックスのプロパティ**ボックスで、クリックして**アクション**です。  
  
4.  をクリックして**URL に移動する**です。  
  
5.  **URL の選択**ボックスで、をクリックして **[URL]**、順にクリック**OK**です。  
  
6.  テキストが、他と同じに見えることに注目してください。 リンク テキストのように表示する必要があります。  
  
7.  [ `[LinkText]`] を選択します。  
  
8.  **フォント**のセクションで、**ホーム** タブで、をクリックして、**下線**ボタンをクリックし、横にドロップダウン矢印をクリックして、**色** ボタンをクリックして**青**です。  
...
5.  プロパティ ペインに WritingMode プロパティが見つかりません。  
...
6.  リスト ボックスで、をクリックして**Rotate270**です。  
  
7.  **ホーム** タブで、**段落**セクションで、をクリックして、**中間**と**Center**セルの中央にテキストを検索するボタン垂直および水平方向です。  
  
8.  [実行]\(**!**) をクリックします。  
  
 `[Territory]` セル内のテキストがセルの下から上に向かって縦に表示されます。  
  
##  <a name="FormatHTML"></a> HTML 形式でテキストを表示します。  
...
6.  **全般** ページの **マークアップの種類**、 をクリックして**HTML: HTML タグのスタイルとして解釈**です。  
...
4.  (省略可能)**ホーム**] タブで、**数**グループで、[、**プレース ホルダーのスタイル**ボタンをクリックし、をクリックして**サンプル値**どのような数値を表示するには書式設定します。  
...
## <a name="next-steps"></a>次の手順  
 レポート ビルダーでのテキストを書式設定方法はたくさんあります[チュートリアル: 自由形式レポートを作成する&#40;レポート ビルダー&#41; ](../reporting-services/tutorial-creating-a-free-form-report-report-builder.md)他の例が含まれています。  
