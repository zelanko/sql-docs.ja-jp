...
# <a name="script-with-the-rsexe-utility-and-the-web-service"></a>rs.exe ユーティリティと Web サービスを使用したスクリプト
 ...
 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] スクリプトを使用すると、レポート サーバー Web サービスのあらゆる操作を実行できます。 スクリプトを使用すれば、サーバー上の複数のレポートにセキュリティをコピーしたり、アイテムを追加または削除したり、サーバー間でレポート サーバー アイテムをコピーすることができます。 スクリプト環境の詳細については、次を参照してください。 [Reporting Services スクリプト ファイルを実行](run-a-reporting-services-script-file.md)します。 スクリプト ファイルは特定の書式を使用しており、 [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)] .NET で記述されます。 詳細については、次を参照してください。 [Reporting Services スクリプト ファイルを書式設定](format-a-reporting-services-script-file.md)します。  
