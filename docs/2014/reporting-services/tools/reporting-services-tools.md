...
|ツール|説明|アクセス方法|  
|----------|-----------------|-------------------|  
|[!INCLUDE[ssCrescent](../../includes/sscrescent-md.md)]|[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 表形式モデルに基づいてレポートを作成し、対話できるようにデザインされた、対話型データの探索およびビジュアル化が可能です。<br /><br /> 注: SharePoint [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]モードではが必要です。|Silverlight がインストールされているブラウザー|  
