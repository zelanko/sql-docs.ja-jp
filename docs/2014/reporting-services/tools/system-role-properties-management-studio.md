...
# <a name="system-role-properties-management-studio"></a>システム ロールのプロパティ (Management Studio)
 ...
 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]には、システム**管理者**と**システムユーザー**という2つの定義済みシステムロール定義があります。 タスクの一覧を変更することにより、ロールの定義を変更できます。また、異なるタスクの組み合わせをサポートする新しいシステム ロールを作成できます。 ロールの定義を編集すると、そのロールの定義を含むすべてのロールの割り当てに反映されます。  
