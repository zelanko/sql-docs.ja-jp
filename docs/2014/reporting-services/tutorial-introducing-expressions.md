...
ms.locfileid: "36082806"
---
# <a name="tutorial-introducing-expressions"></a>チュートリアル: 式の概要
  式を使用すると、強力で柔軟なレポートを作成できます。 このチュートリアルでは、一般的な関数および演算子を使用した式を作成および実装する方法を説明します。 使用して、**式**名前値の連結を見て、独立したデータセット内の値を式を作成する ダイアログ ボックスは、フィールドの値、およびなに基づいて別の画像を表示します。  
...
1.  [テーブルまたはマトリックス ウィザードから表形式のレポートとデータセットを作成します。](#Setup)  
...
5.  [CountryRegion 名を参照します。](#Lookup)  
  
6.  [最後に日数をカウントを購入します。](#Count)  
  
7.  [インジケーターを使用して売上比較を表示するには](#Indicator)  
  
8.  [レポートを縞状レポートします。](#GreenBar)  
...
1.  をクリックして**開始**、 をポイント**プログラム**、 をクリックして[!INCLUDE[ssCurrentUI](../includes/sscurrentui-md.md)]**レポート ビルダー**、順にクリック**レポート ビルダー**です。  
    ...
    > [!NOTE]  
    >  レポート ビルダーの ClickOnce バージョンを使用する場合は、レポート マネージャーを開き、をクリックして**レポート ビルダー**、SharePoint サイト上のどの Reporting Services コンテンツなどのレポートが有効で、をクリックするか、または**レポート ビルダーのレポート**上、**新しいドキュメント**メニューで、**ドキュメント**共有ドキュメント ライブラリのタブです。  
...
13. **値**ボックスの一覧を右クリックして`CountryRegionID` をクリックし、**合計**オプション。  
...
17. **スタイルを選択する** ページで、をクリックして**スレート**、順にクリック**完了**です。  
...
4.  Penguins.JPG をクリックし、**[開く]** をクリックします。  
     ...
     **イメージのプロパティ**ダイアログ ボックスで、をクリックして**可視性**をクリックし、**を非表示に**オプション。  
...
1.  右クリックし、**名前**列をポイント**列の挿入**、クリックして**右**です。  
...
-   **挿入** タブ、リボンのをクリックして**四角形**のデータ セル内をクリックし、**性別**列です。  
  
     四角形がセルに追加されます。  
  
#### <a name="to-add-an-image-to-the-rectangle"></a>四角形に画像を追加するには  
  
1.  四角形内を右クリックし、**挿入**、クリックして**イメージ**です。  
  
2.  **イメージのプロパティ** ダイアログ ボックスで、横にある下矢印をクリックして**このイメージを使用して**、Penguins.JPG など、追加した画像のいずれかを選択します。  
  
3.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
#### <a name="to-use-images-to-show-gender"></a>画像を使用して性別を示すには  
  
1.  内のデータ セルの画像を右クリックし、**性別**列とクリック**イメージのプロパティ**です。  
  
2.  **イメージのプロパティ** ダイアログ ボックスで、式 をクリックして**fx**横に、**このイメージを使用して**テキスト ボックス。  
...
13. をクリックして**OK**を閉じます、**イメージのプロパティ** ダイアログ ボックス。  
  
14. **[実行]** をクリックして、レポートをプレビューします。  
  
##  <a name="Lookup"></a> 5.CountryRegion 名を参照する  
 CountryRegion データセットを作成しを使用して、**ルックアップ**国/地域の識別子の代わりに国/地域の名前を表示する関数。  
...
1.  クリックして、 **Country Region ID**列のタイトルとテキストの削除: id です。  
...
4.  **式** ダイアログ ボックスで、展開**共通の関数** をクリック**その他**です。  
  
5.  **項目**リストで、ダブルクリック**ルックアップ**です。  
  
6.  **[カテゴリ]** ボックスの一覧の **[フィールド (Expressions)]** をクリックします。  
  
7.  **値**リストで、ダブルクリック`CountryRegionID`です。  
...
9. 右かっこを削除し、「**,Fields!ID.value, Fields!CountryRegion.value, "CountryRegion")**」と入力します。  
     ...
     構文、**ルックアップ**関数でも、CountryRegion データセット内にある CountryRegion 値を返す、CountryRegion データセット内 CountryRegionID と ID の間に参照を指定します。  
...
##  <a name="Count"></a> 6.前回の購入日からの日数をカウントする  
 列を追加し、使用、**今すぐ**関数または`ExecutionTime`個人の最後の日から今日までの数を計算する組み込みのグローバル変数を購入します。  
...
10. **値**リストで、ダブルクリック**LastPurchase**です。  
  
11. カーソルが別の位置にある場合は、`Fields!LastPurchase.Value` の直後に置きます。  
  
12. 「**,**」と入力します。  
  
13. **カテゴリ**一覧で、クリックして**の日付と時刻**もう一度です。  
  
14. **項目**リストで、ダブルクリック**今すぐ**です。  
...
##  <a name="Indicator"></a> 7.インジケーターを使用して売上比較を示す  
 ...
 インジケーターとその状態を構成するには、多数の手順が必要です。 する場合、"、インジケーターを構成するには」の手順でスキップしてこのチュートリアルから完成した式をコピー/貼り付け、**式** ダイアログ ボックス。  
...
1.  **挿入** タブ、リボンのクリックして**インジケーター**、用のデータ セルをクリックして、 **+ OR-AVG sales**列です。  
...
6.  **値**リストで、ダブルクリック**YTDPurchase**です。  
  
7.  カーソルが別の位置にある場合は、`Fields!YTDPurchase.Value` の直後に置きます。  
  
8.  型  **-**  
  
9. 展開**共通の関数**もう一度をクリックして**集計**です。  
  
10. **項目**リストで、ダブルクリック**Avg**です。  
  
11. **[カテゴリ]** ボックスの一覧の **[フィールド (Expressions)]** をクリックします。  
  
12. **値**リストで、ダブルクリック**YTDPurchase**です。  
...
21. **値**リストで、ダブルクリック**YTDPurchase**です。  
  
22. カーソルが別の位置にある場合は、`Fields!YTDPurchase.Value` の直後に置きます。  
  
23. 型  **-**  
  
24. 展開**共通の関数**もう一度をクリックして**集計**です。  
  
25. **項目**リストで、ダブルクリック**Avg**です。  
  
26. **[カテゴリ]** ボックスの一覧の **[フィールド (Expressions)]** をクリックします。  
  
27. **値**リストで、ダブルクリック**YTDPurchase**です。  
...
##  <a name="GreenBar"></a> 8 です。レポートを縞状レポートにする  
...
1.  クリックして、**ビュー**リボンのタブし、ことを確認**プロパティ**が選択されています。  
...
4.  プロパティ ペインで、 **[BackgroundColor]** をクリックします。  
     ...
     [プロパティ] ウィンドウには、カテゴリ別プロパティが一覧表示場合、表示されます、 **BackgroundColor**下にある、**塗りつぶし**カテゴリ。  
...
8.  展開**共通の関数** をクリック**集計**です。  
  
9. **項目**リストで、ダブルクリック**RunningValue**です。  
  
10. **[カテゴリ]** ボックスの一覧の **[フィールド (Expressions)]** をクリックします。  
  
11. **[値]** ボックスの一覧の **[FirstName]** をダブルクリックします。  
  
12. カーソルが既に直後にない場合`Fields!FirstName.Value`ありますに配置し、入力 **、**  
  
13. 展開**共通の関数** をクリック**集計**です。  
  
14. **項目**リストで、ダブルクリック**カウント**です。  
...
17. 展開**演算子** をクリック**算術**です。  
  
18. **項目**リストで、ダブルクリック**Mod**です。  
...
1.  されていない場合、**ホーム** タブで、をクリックして**ホーム**デザイン ビューに戻ります。  
  
2.  **[実行]** をクリックします。  
  
3.  **色を選択**ドロップダウン リストで、レポートで、白以外のバーの色を選択します。  
  
4.  **[レポートの表示]** をクリックします。  
  
     選択した背景色が 1 行おきに適用された状態でレポートが表示されます。  
  
##  <a name="DateFormat"></a> (省略可能)形式の日付の列  
 形式、 **Last Purchase**列で、日付が含まれています。  
  
#### <a name="to-format-date-column"></a>日付列の書式を設定するには  
  
1.  **[デザイン]** をクリックしてデザイン ビューに戻ります。  
  
2.  データ セルを右クリックし、 **Last Purchase**列とクリック**テキスト ボックスのプロパティ**です。  
  
3.  **テキスト ボックスのプロパティ**ダイアログ ボックスで、をクリックして**数**、 をクリックして**日付**、種類をクリック **\*1/31/2000**です。  
...
3.  含むテキスト ボックスを右クリックして**Sales Comparison Summary**  をクリック**テキスト ボックスのプロパティ**です。  
...
4.  **名前**、既定の名前を**Sales Comparison Summary**です。  
...
2.  をクリックして**デスクトップ`, `マイ ドキュメント**、または**コンピューター**、レポートを保存するフォルダーを参照します。  
  
3.  **名前**、既定の名前を**Sales Comparison Summary**です。  
