...
|[ツール バーの位置]|レポート ビューアー内のツール バーの位置を決定します。 有効な値は`Top`と`Bottom`です。|  
|[プロンプト領域]|有効な値は`Displayed`、 `Collapsed`、および`Hidden`です。 `Displayed` パラメーター化された値が含まれていると、レポートを実行する前にユーザー入力を必要とするレポートのパラメーター領域を表示します。 使用して`Hidden`のすべてのレポート パラメーターが指定されていて、パラメーター領域をユーザーに表示しないようにします。|  
