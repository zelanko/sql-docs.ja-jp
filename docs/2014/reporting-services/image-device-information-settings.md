...
|`DpiX`|出力画像の水平方向の解像度。 既定値は **96**です。 適用されます`BMP`、 `GIF`、 `PNG`、および`TIFF`出力形式。|  
|`DpiY`|出力画像の垂直方向の解像度。 既定値は **96**です。 適用されます`BMP`、 `GIF`、 `PNG`、および`TIFF`出力形式。|  
...
|**OutputFormat**|1 つ、 [!INCLUDE[ndptecgdiexpanded](../includes/ndptecgdiexpanded-md.md)] ([!INCLUDE[ndptecgdi](../includes/ndptecgdi-md.md)]) サポートされている出力形式: `BMP`、 `EMF`、 `GIF`、 `JPEG`、 `PNG`、または`TIFF`です。|  
