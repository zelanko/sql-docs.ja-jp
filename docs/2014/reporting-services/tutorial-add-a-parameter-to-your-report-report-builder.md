...
ms.locfileid: "36174116"
---
# <a name="tutorial-add-a-parameter-to-your-report-report-builder"></a>チュートリアル: レポートへのパラメーターの追加 (レポート ビルダー)
  ユーザー フィルター レポート データ データ ソースからまたはレポートできるようにする、レポートにパラメーターを追加します。 レポート パラメーターは、データセット クエリに追加したクエリ パラメーターごとに自動で作成されます。 パラメーターのデータ型により、パラメーターがレポート ビューアー ツール バーに表示される方法が決まります。  
...
4.  [レポート パラメーターの既定のデータ型およびその他のプロパティを変更します。](#ChangeDefaultProperties)  
  
    1.  [使用可能な値を提供するデータセットを追加および表示名](#AddDataset)  
  
    2.  [値のドロップダウン リストの作成に使用できる値を指定します。](#AvailableValues)  
  
    3.  [レポートが自動的に実行されるように、既定値を指定します。](#DefaultValues)  
  
    4.  [名前/値ペアを持つデータセットから値を検索します。](#NameValue)  
  
5.  [レポートに、選択したパラメーターの値を表示します。](#Expression)  
  
6.  [フィルター内のレポート パラメーターを使用します。](#Filter)  
  
7.  [複数の値をそのまま使用するレポート パラメーターを変更します。](#Multivalued)  
  
8.  [条件付き表示のブール型のパラメーターを追加します。](#Boolean)  
...
5.  **レイアウトを選択して**] ページの [**オプション**、いることを確認**小計と総計を表示する**が選択されています。  
...
3.  クエリの最後の行に、次の [!INCLUDE[tsql](../includes/tsql-md.md)] `WHERE` 句を追加します。  
    ...
    ```  
     ...
     `WHERE`句は、クエリ パラメーターで指定されている、店舗 id を取得したデータを限定 *@StoreID*です。  
...
8.  レポート データ ペインで **[パラメーター]** フォルダーを展開します。  
 ...
 レポート パラメータの名前は今すぐがあることに注意してください *@StoreID*です。 既定では、パラメーターがデータ型を持つ**テキスト**です。 店舗 ID は整数なので、次の手順でデータ型を [整数] に変更します。  
...
1.  [レポート データ ペインで、**パラメーター** ] ノードを右クリックして*@StoreID*、順にクリック**パラメーターのプロパティ**です。  
  
2.  プロンプトで、次のように入力します**Store identifier?。** レポートを実行すると、レポート ビューアーのツール バーにこのテキストが表示されます。  
...
6.  レポートをプレビューします。 レポート ビューアーで表示の入力を求める *@StoreID*です。  
  
7.  レポート ビューアー ツール バーで、店舗 ID の横に「 **200**」と入力し、 **[レポートの表示]** をクリックします。  
  
##  <a name="AddDataset"></a> 4a です。 使用可能な値と表示名を提供するデータセットを追加する  
...
##  <a name="AvailableValues"></a> 4b です。 値のドロップダウン リストの作成に使用可能な値を指定します  
...
1.  レポート データ ペインで、パラメーターを右クリックし*@StoreID*、クリックして**パラメーター プロパティ**です。  
...
9. レポートをプレビューします。  
     ...
     レポート ビューアー ツールバーでパラメーター テキスト ボックスが、ドロップダウン リストを表示する**\<値を選択 >** です。  
  
10. ドロップダウン リストから Contoso Catalog Store を選択し、をクリックして**レポートの表示**です。  
  
 レポートには、店舗 ID **200**について、Accessories、Camcorders、および Digital SLR Cameras の販売数量が表示されます。  
  
##  <a name="DefaultValues"></a> 4 c です。 既定値を指定してレポートが自動的に実行されるようにする  
...
3.  をクリックして**既定値の**、クリックして**クエリから値を取得**です。  
 ...
 *@StoreID*、レポート ビューアーには、値"Contoso North America Online Store"が表示されます。 これは、データセットの結果セットから最初の値**ストア**です。 レポートには、店舗 ID **199**について、Digital Cameras の販売数量が表示されます。  
...
3.  をクリックして**既定値の**、 をクリック**値を指定**、順にクリック**追加**です。 新しい値の行が追加されます。  
 ...
 *@StoreID*、レポート ビューアーには、値"Contoso Catalog Store"が表示されます。 これは、店舗 id の表示名**200**です。 レポートには、店舗 ID **200**について、Accessories、Camcorders、および Digital SLR Cameras の販売数量が表示されます。  
...
7.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
     ...
     店舗の列見出しには、複雑な式の表示テキストが含まれています:  **< \<Expr >>** です。  
...
2.  ページ フッターを右クリックし、順にポイント**挿入**、クリックして**テキスト ボックス**です。 タイムスタンプを示すテキスト ボックスの横にテキスト ボックスをドラッグします。 テキスト ボックスの側面にあるハンドルをクリックしてドラッグすることにより、幅を拡張します。  
...
5.  最初の式を右クリックし、をクリックして**式**です。 **[式]** ダイアログ ボックスが表示されます。 「`Value`」を「`Label`」に置き換えます。  
...
6.  変更`equals`(=) を`IN`で、 [!INCLUDE[tsql](../includes/tsql-md.md)] `WHERE`クエリの最後の行で句。  
...
##  <a name="Boolean"></a> 8 です。条件付き表示のためにブール型パラメーターを追加する  
...
4.  **データ型**、ドロップダウン リストからをクリックして**ブール**です。  
...
8.  レポート ビューアー ツールバーで、横に**Show selections**をクリックして`True`です。  
