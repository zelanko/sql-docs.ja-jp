...
##  <a name="Setup"></a>1. テーブルまたはマトリックスウィザードでマトリックスレポートとデータセットを作成する  
...
##  <a name="Query"></a>3. クエリパラメーターを追加してレポートパラメーターを作成する  
...
##  <a name="ChangeDefaultProperties"></a>4. レポートパラメーターの既定のデータ型およびその他のプロパティを変更する  
...
##  <a name="AvailableValues"></a>4b. 使用可能な値を指定して、値のドロップダウンリストを作成します  
...
##  <a name="DefaultValues"></a>4c. 既定値を指定してレポートが自動的に実行されるようにする  
...
##  <a name="Expression"></a>5. 選択したパラメーター値をレポートに表示する  
...
5.  最初の式を右クリックし、[**式**] をクリックします。 
  **[式]** ダイアログ ボックスが表示されます。 「`Value`」を「`Label`」に置き換えます。  
...
##  <a name="Filter"></a>6. フィルターでレポートパラメーターを使用する  
...
##  <a name="Multivalued"></a>7. レポートパラメーターを複数の値を受け入れるように変更する  
...
##  <a name="Boolean"></a>8. 条件付き表示のためのブール型パラメーターを追加する  
...
4.  次の式を入力します。 `=Not Parameters!ShowSelections.Value`  
  ...
  `Not` 演算子を適用すると、パラメーターが選択されたときに、[非表示] プロパティが false になり、テキスト ボックスが表示されます。  
...
3.  レポートを保存する権限があるレポート サーバーの名前を入力するか選択します。  
     ...
     **レポートサーバーに接続するためのメッセージが表示され**ます。 接続が完了すると、レポート サーバー管理者がレポートの既定の場所として指定したレポート フォルダーのコンテンツが表示されます。  
