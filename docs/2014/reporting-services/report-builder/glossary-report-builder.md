...
|承認 (authorization)|1.アクセス許可と、ユーザーに与えられるアクセス権を確認する操作。 2. 特定の情報、サービス、または機能へのアクセスを、個人、コンピューター プロセス、またはデバイスに許可する処理。|  
...
|バッチ (batch)|1.SQL ステートメントのセットは、一緒に送信し、グループとして実行します。  2. Reporting Services では、1 つのトランザクション内の一連の SOAP メソッド呼び出し。|  
...
|コレクション (collection)|1.その他のオブジェクトのセットを格納するオブジェクト。 2. Master Data Services で、明示的階層およびその他のコレクションのメンバーで編成された階層グループ。|  
...
|データ ソース (data source)|1.ADO と OLE DB での OLE DB プロバイダーによって公開されているデータ ソースの場所。 2. キューブやディメンションなどのオブジェクトのデータのソース。 または、ソース データへのアクセスに必要な情報の指定です。 ClassType clsDataSource のオブジェクトを指す場合もあります。 <br />3.Reporting Services、指定されたデータ ソースの種類、接続文字列と資格情報ではレポート サーバーに個別に保存し、レポート プロジェクト間で共有またはできる、レポート定義 (.rdl) ファイルに埋め込まれています。|  
...
|データセット (dataset)|1.OLE db for OLAP、多次元式 (MDX) の SELECT ステートメントの実行結果である多次元データのセット。   <br />2.Reporting services の場合は、データ ソースの定義、クエリの定義と省略可能なパラメーター値、計算フィールド、およびレポート定義 (.rdl) ファイルの一部としてフィルターや照合順序の情報を含む名前付きの仕様。 .rdl ファイルには、複数のデータセットを含めることができます。|  
|DBCS (DBCS)|2 バイト文字セット。|  
|既定値 (default)|1.データ値、オプションの設定、照合順序、または値、設定、照合順序は、ユーザーが指定されていない場合、システムによって自動的に割り当てられた名前または名前。 2. 特定のイベントが生じたときに、ユーザーが動作の実行を指定しないと自動的に行われる動作のことも指します。|  
...
|区切り記号 (delimiter)|1.Transact sql で開始し、オブジェクト名の末尾を示す文字を使用して二重引用符 ("") または角かっこ ()。 2. Integration Services および Reporting Services では、レコード、フィールド、または文字列を区切る際に使用する文字。|  
...
|ドメイン (domain)|1.Windows のセキュリティ管理のため、共通のセキュリティ データベースを共有して表示のグループ化されたコンピューターのコレクション。 2. リレーショナル データベースでは、列で使用できる有効な値のセット。|  
|ドリル ダウン/ドリル アップ|最も集約されたレベル (最上位レベル) から最も詳細なレベル (最下位レベル) までの範囲内でデータのレベル間を移動すること。|  
|ドリルスルー (drill through)|1.Analysis Services で、キューブ セルのデータの集約元となる詳細データを取得します。 2. Reporting Services で、メイン 詳細レポートのハイパーリンクをクリックして関連レポートを表示すること。|  
...
|エンティティ (entity)|1.Reporting Services では、エンティティは、ソース フィールド、ロール、フォルダー、および式、慣れ親しんだビジネス用語などのモデル アイテムの論理的なコレクションが。 2. Master Data Services において、エンティティはモデル内のオブジェクトです。 各エンティティには、属性によって定義される関連するメンバーが含まれています。|  
...
|エラー ログ (error log)|1.SQL server のシステム情報を記録するテキスト ファイル。 2. Integration Services では、エラー、警告、またはイベントの記録。この記録は、SQL Server Profiler、Windows イベント ログ、SQL Server などの複数のログ プロバイダーによって保存できます。|  
...
|式 (expression)|1.SQL で 1 つのデータ値に評価される記号と演算子の組み合わせ。 2. Integration Services では、単一のデータ値に評価されるリテラル、定数、関数、および演算子の組み合わせ。|  
...
|フィルター (filter)|1.レコード セットを制御する一連の条件は、その結果セットに返されます。  2. フルテキスト検索では、ファイル拡張子を指定した場合に、varbinary(max) 型または image 型の列に保存されたファイルからテキストを抽出すること。|  
...
|ローカル サーバー (local server)|1.SQL Server 接続では、アプリケーションと同じコンピューターで実行されている SQL Server のインスタンス。 <br />2.TRANSACT-SQL ステートメントでは、解決するときに、ステートメントを実行している SQL Server のインスタンスのデータベース オブジェクトを参照します。 <br />3.SQL Server 分散クエリでは、分散クエリを実行している SQL Server のインスタンス。 ローカル サーバーは、クエリで参照されるリンク サーバーにアクセスします。|  
...
|オブジェクト ID (object identifier)|1.オブジェクトに指定された一意の名前。 2. Meta Data Services では、GUID (グローバル一意識別子) と内部識別子から構成される一意識別子。|  
|ODBC データ ソース (ODBC data source)|1.一連の ODBC ドライバーを使用してアクセスできるデータの場所。 2. ODBC アプリケーションからデータ ソースに接続するために必要なすべての接続情報が含まれた、保存されている定義。|  
...
|パディング (padding)|1.通常、最後のテキスト ブロックが短い場合に、追加文字列。 2. 特定のサイズを保つためにセルに割り当てられる空白。|  
...
|再帰型階層 (recursive hierarchy)|1.組織階層内のマネージャーと従業員のリレーションシップのレポート構造などの階層構造内の複数の階層レベルを含む 1 つのレポート データセットからのデータの階層。 2. Master Data Services で、再帰リレーションシップを含む派生階層。 再帰リレーションシップは、エンティティ自体に基づくドメイン ベースの属性がエンティティにある場合に存在します。|  
...
|リレーションシップ (relationship)|1.別のテーブルの外部キーの 1 つのテーブルの主キーを参照するテーブル間のリンク。 テーブル間で参照整合性が設定されている場合は実線で、INSERT トランザクションまたは UPDATE トランザクションに参照整合性が設定されていない場合は破線で、データベース ダイアグラムにリレーションシップを示す線が表示されます。 この線の両端には、主キー対外部キーのリレーションシップを示す主キー記号、または一対多のリレーションシップの外部キー側を示す無限大記号が表示されます。 <br />2.メタデータ サービスでリレーションシップとは、1 つのオブジェクトは、配信元とその他のオブジェクトが変換先のオブジェクトのペア間のアソシエーション。 関連付けは後続のオブジェクトのペアでも繰り返されるので、あるリレーションシップで対象になるオブジェクトは次のリレーションシップでは基になるオブジェクトになります。 このようにして、情報モデルのすべてのオブジェクトは、情報モデル全体にわたって 1 つのオブジェクトから次のオブジェクトへと伸びるリレーションシップの連鎖によって関連付けられます。|  
...
|ロール (role)|1.ユーザーまたはグループは、特定の権限のセットが与えられます。  <br />2.Analysis Services では、ロールは Windows セキュリティ アカウントを使用して、データベース、キューブ、ディメンション、およびデータ マイニング モデルにユーザーがアクセスするときに、アクセスとアクセス許可のスコープを制限します。  <br />3.データベース ミラーリング セッションでは、プリンシパル サーバーとミラー サーバーは補完的なプリンシパルおよびミラーの役割を実行します。 必要に応じて、3 番目のサーバー インスタンスがミラーリング監視の役割を果たします。|  
...
|行識別子 (row identifier)|1.列またはテーブル内の他のすべての行から任意の 1 つの行を区別するために使用される列のセット。 2. ヒープでは、行を指すポインターを指します。|  
...
|テーブル|1.2 次元のオブジェクトの行と列で構成され、エンティティに関するデータを格納するリレーショナル データベースにモデル化されます。 2. データを列形式で表示する、レポート レイアウト上のデータ領域。|  
...
|変数 (variable)|1.Integration Services では、列の値とパッケージ オブジェクトのプロパティを設定するスクリプト、式、およびプロパティ式で使用できる値を格納します。 <br />2.値が代入される定義されたエンティティ。 ローカル変数は、DECLARE@localvariable ステートメントで定義され、SELECT ステートメントまたは SET@localvariable ステートメントで宣言されているステートメント バッチ内で初期値が代入されます。|  
