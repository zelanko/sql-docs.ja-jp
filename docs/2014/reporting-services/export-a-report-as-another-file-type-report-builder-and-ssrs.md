...
6.  選択したエクスポート形式でレポートを表示するには、 **[開く]** をクリックします。  
     ...
     選択した形式に関連付けられているアプリケーションを使用して、レポートが表示または保存されます。 
  **[保存]** をクリックした場合、レポートを保存する場所を指定するよう求められます。  
  
     **メモ**このファイルの種類に関連付けられているプログラムがないために、選択した形式でレポートを開くことができない場合は、エクスポートしたレポートを保存するか、レポートを開くためのプログラムをオンラインで検索するように求められます。  
