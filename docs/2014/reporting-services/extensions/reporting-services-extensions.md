...
# <a name="reporting-services-extensions"></a>Reporting Services の拡張機能
  ...
  [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] のモジュール式アーキテクチャは、拡張性を考慮して設計されています。 マネージド コード API を使用して、[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] の多くのコンポーネントで使用される拡張機能を容易に開発、インストール、および管理できます。 を使用して[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)]プライベートアセンブリまたは共有アセンブリを作成[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]し、進化するビジネスニーズに合わせて新しい機能を追加することができます。  
