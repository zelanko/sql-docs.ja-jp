 ...
 **作業の開始** ダイアログ ボックスを使用して、グラフ ウィザードで、埋め込みデータセットを作成するが共有データ ソースを選択し、縦棒グラフを作成します。  
...
5.  **スタイルを選択する** ページで、スタイル ボックスで、スタイルを選択します。  
...
2.  水平方向の軸を右クリックし、をクリックして**横軸のプロパティ**です。  
...
2.  横軸のタイトルを右クリックし、をクリックして**軸のタイトル**タイトルを削除します。 横軸には日付が表示されるので、タイトルは必要ありません。  
  
3.  横軸を右クリックし、をクリックして**横軸のプロパティ**です。  
  
4.  **軸のオプション**ページで、**軸の範囲と間隔**、型**3**の**間隔**です。 3 日ごとの日付がグラフに表示されるようになります。  
  
5.  をクリックして**ラベル**です。  
  
6.  **軸ラベル自動調整オプションを変更する****自動調整を無効にする**です。  
...
2.  グラフの凡例を右クリックし、をクリックして**凡例のプロパティ**です。  
...
5.  (省略可) このチュートリアルではカテゴリが 1 つしかないので、凡例は必要ありません。 凡例を削除する凡例を右クリックし、をクリックして**凡例の削除**です。  
...
2.  ある単語を選択**グラフのタイトル**上部のグラフ、および次のテキストを入力: **Store Sales Order Totals**です。  
...
3.  リボンで、上、**ホーム** タブの 、**数**グループで、、**通貨**ボタンをクリックします。 軸ラベルの表示が、通貨形式に変更されます。  
  
4.  リボンで、上、**ホーム**] タブの [、**数**グループで、[、**小数点表示桁下げ**2 回、最も近いドルに丸められる数を表示する] ボタンをクリックします。  
  
5.  縦軸を右クリックし、をクリックして**縦軸のプロパティ**です。  
  
6.  をクリックして**数**です。 注意してください**通貨**で既に選択されて、**カテゴリ**ボックス、および**小数点以下桁数**は既に**0** (ゼロ)。  
  
7.  **の値を表示する**ボックスで、クリックして**何千も**します。  
  
8.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
9. グラフの側面に沿った縦軸のタイトルを右クリックし、をクリックして**軸のタイトルのプロパティ**です。  
  
10. テキストを置き換える、**タイトル テキスト**を次のテキスト フィールド: **Sales Total (in Thousands)** です。 タイトルの表示形式に関連した各種オプションを指定することもできます。  
...
3.  右クリックし、 **[sum (sales)]** フィールドを**値**領域およびクリック**計算系列の**します。  
...
##  <a name="Title"></a> 8 です。レポート タイトルを追加する  
...
3.  型**Sales Chart**を入力し、enter キーを押しながら、 **January to December 2009**ので、次のようにします。  
  
     **Sales Chart**  
  
     **January to December 2009**  
  
4.  選択**Sales Chart**、 をクリックし、**太字**ボタンをクリックして、**フォント**セクションで、**ホーム**リボンのタブです。  
  
5.  選択**January to December 2009**、し、[、**フォント**セクションで、**ホーム**] タブで、フォント サイズを設定**10**です。  
  
6.  (省略可能)作成する必要があります、**タイトル**テキスト ボックスの下端の中央をクリックすると、両方向矢印に引いて、2 つの行のテキストに合わせて高さ。  
