---
title: データ ドリブン サブスクリプション ページ (レポート マネージャー) を作成 |Microsoft ドキュメント
ms.custom: ''
ms.date: 05/24/2017
 ...
 **[パス]**  
 ネットワーク接続経由でアクセス可能な共有フォルダーを指定します。 そのフォルダーにアクセスできることを確認するには、をクリックして**実行**[スタート] メニューでこの形式でフォルダー パスを入力: \\ \\< computername\>\\< sharedfoldername\>.  
 ...
 **ユーザー名**  
 この形式で共有フォルダーにファイルを追加する権限を持つドメイン ユーザー アカウントを入力:\<ドメイン >\\< ユーザー名\>です。  
 ...
 **毎日**  
 選択した日の指定した時刻に実行されるスケジュールを定義します。 次の方法で日を指定することができます: すべて*\<日 >*、毎平日の業務とすべて*\<数 >* 日。 いずれかのオプションを選択すると、それ以外のオプションは表示されていても無効になります。  
