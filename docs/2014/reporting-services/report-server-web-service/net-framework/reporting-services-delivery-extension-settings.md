...
|**[Priority]**|電子メール メッセージを送信する優先順位。 有効な値は`LOW`、 `NORMAL`、および`HIGH`します。 既定値は `NORMAL` です。|  
...
|**WRITEMODE**|ディスクへのアクセスに使用する書き込みモード。 有効な値は`None`、 `Overwrite`、および`AutoIncrement`します。|  
