...
2.  クレンジング プロセスが完了したら、確認**統計**で、 **Profiler**タブ。ソースの統計情報は、処理されたレコードは、正しい、DQS によって修正レコード、DQS によって提案された変更が含まれるレコードの数、および無効なレコードの数の数値として検出されたレコードの数の数を提供します。 右側のリスト ボックスには、クレンジング プロセスに含まれる各ドメインの、修正された値、提案された値、値の完全性 (データがどの程度存在するか) と正確性 (データがどの程度意図されたとおりに使用できるか) が表示されます。  
