---
title: 時系列予測 (中級者向けデータ マイニング チュートリアル) 高度な |Microsoft ドキュメント
ms.custom: ''
ms.date: 06/13/2017
...
2.  [(モデルの構築) 用の集計データを準備します。](#bkmk_newReplaceData)  
  
3.  [(クロス予測) の系列データを準備します。](#bkmk_CrossData2)  
...
7.  [新しい予測を確認します。](../../2014/tutorials/comparing-predictions-for-forecasting-models-intermediate-data-mining-tutorial.md)  
...
###  <a name="bkmk_newReplaceData"></a> 新しい売上データを使用してデータ ソース ビューを作成します。  
  
1.  **ソリューション エクスプ ローラー**を右クリックして**データ ソース ビュー**、し、**新しいデータ ソース ビュー**です。  
...
##  <a name="bkmk_CrossData2"></a> データ、クロス予測モデルの作成  
 元で使用されたデータ モデルが既にを予測より小さい数のカテゴリに複数の自転車モデルを折りたたむし、地域に個々 の国からの結果をマージする vTimeSeries ビューによってグループ化されています。 世界的な予測に使用できるモデルを作成するには、データ ソース ビュー デザイナーで直接、追加の簡単な集計をいくつか作成します。 新しいデータ ソース ビューには、すべての地域におけるすべての製品の売上の合計と平均だけが含まれます。  
...
3.  **データ ソースの選択** ページで選択[!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)]、クリックして **[次へ]** です。  
  
4.  **[テーブルとビューの選択]** ページでは、テーブルを追加せずに、単に **[次へ]** をクリックします。  
  
5.  ページで、**ウィザードの完了**、名前を入力します`AllRegions`、クリックして**完了**です。  
  
6.  次に、空のデータ ソース ビュー デザイン画面を右クリックし **新しい名前付きクエリ**です。  
  
7.  **名前付きクエリの作成** ダイアログ ボックスの**名前**、型`AllRegions`、および**説明**、型**合計とすべてのモデルの売上の平均と地域**です。  
...
9. 右クリックし、`AllRegions`テーブルをクリックし **データの探索**です。  
  
###  <a name="bkmk_CrossData"></a> クロス予測のシリーズ データを作成するには  
...
4.  空のデザイン画面を右クリックして**T1000 Pacific Region.dsv**、し、**新しい名前付きクエリ**です。  
...
6.  データ ソース ビュー デザイン画面で T1000 Pacific を右クリックし、**データの探索**データが正しくフィルター選択されていることを確認します。  
