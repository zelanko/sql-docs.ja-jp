...
|ページ|入力値|Source|説明|  
|----------|-----------------|------------|-----------------|  
|**構成する PowerPivot for SharePoint または修復**|既定のアカウント|現在のユーザー|既定のアカウントは、ファーム内の共有サービスの準備に使用するドメイン Windows ユーザー アカウントです。 PowerPivot サービス アプリケーション、Secure Store Service、Excel Services、Web アプリケーション プールの ID、サイト コレクション管理者、および PowerPivot 自動データ更新アカウントの準備に使用します。<br /><br /> 既定では、ツールは、現在のユーザーのドメイン アカウントを入力します。 評価の目的でサーバーを構成する場合を除き、この値を別のドメイン ユーザー アカウントに置き換える必要があります。<br /><br /> また、サービス ID は、後からサーバーの全体管理を使用して変更することもできます。<br /><br /> 必要に応じて、PowerPivot 構成ツールで次の専用のアカウントを指定できます。<br /><br /> Web アプリケーションを使用して、**既定の Web アプリケーションの作成**ページ (このツールには、ファームの web アプリケーションを作成) する場合。<br /><br /> PowerPivot 自動データ更新アカウントを使用して、**データ更新用自動アカウントの作成**このツールのページです。|  
...
|**PowerPivot サービス アプリケーションを作成します。**|PowerPivot サービス アプリケーション名|既定|既定の名前は、"既定の PowerPivot サービス アプリケーション" です。 ツールで別の値に置き換えることができます。|  
||PowerPivot サービス アプリケーション データベース サーバー|既定|PowerPivot サービス アプリケーション データベースをホストするデータベース サーバー。 既定のサーバー名は、ファームに使用するのと同じデータベース サーバーです。 ツールで別の値に置き換えることができます。|  
||PowerPivot サービス アプリケーション データベース名|既定|既定のデータベース名は、サービス アプリケーション名の後ろに GUID を付けて一意の名前にしたものです。 ツールで別の値に置き換えることができます。|  
...
|**[既定の Web アプリケーションの作成]**|Web アプリケーション名|既定 (必要な場合)|Web アプリケーションが存在しない場合は 1 つ作成されます。 Web アプリケーションはクラシック モード認証用とリッスンするように構成されます**ポート 80**です。 ファイルの最大アップロード サイズは、SharePoint で許容される最大値の 2047 MB に設定されます。 大きい PowerPivot ファイルに対応するために、ファイル アップロード サイズが拡大されます。|  
...
||ファイルの最大サイズ (MB)|既定 (必要な場合)|既定の設定は 2047 です。 SharePoint ドキュメント ライブラリにも最大サイズがあり、PowerPivot の設定がドキュメント ライブラリの設定を超えないようにする必要があります。 詳細については、次を参照してください。[構成ファイルの最大アップロード サイズ&#40;PowerPivot for SharePoint&#41;](power-pivot-sharepoint/configure-maximum-file-upload-size-power-pivot-for-sharepoint.md)です。|  
...
|**サイト コレクション内の PowerPivot 機能をアクティブ化します。**|サイトの URL||PowerPivot 機能をアクティブにするサイト コレクションの URL。|  
...
1.  SharePoint 2010 のハードウェアとソフトウェア要件のドキュメントに移動して[決定 Hardware and Software Requirements (SharePoint 2010)](http://go.microsoft.com/fwlink/?LinkId=169734)  
