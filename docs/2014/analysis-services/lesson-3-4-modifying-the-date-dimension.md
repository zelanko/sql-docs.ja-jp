...
2.  下部の近く、**テーブル** ウィンドウを右クリックして`Date`、クリックして**新しい名前付き計算**です。  
  
3.  **名前付き計算の作成** ダイアログ ボックスで、「`SimpleDate`で、**列名**ボックスで、入力またはコピーして貼り付け、次`DATENAME`内のステートメント、**式**ボックス。  
...
4.  をクリックして**OK**、順に展開`Date`で、**テーブル**ウィンドウです。  
  
     `SimpleDate`名前付き計算が Date テーブル内の列の一覧に表示されることを示すアイコンが名前付き計算します。  
  
5.  **[ファイル]** メニューの **[すべてを保存]** をクリックします。  
  
6.  **テーブル** ウィンドウを右クリックして`Date`、クリックして**データの探索**です。  
  
7.  **[Date テーブルの探索]** ビューで、右にスクロールして最後の列を確認します。  
  
     注意して、`SimpleDate`元のデータ ソースを変更することがなく、基になるデータ ソースから複数の列からデータを適切に連結列、データ ソース ビューに表示されます。  
...
1.  **で Date ディメンションの** ディメンション デザイナー [!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)]を開きます。 これを行うをダブルクリックして、`Date`ディメンション、**ディメンション**のノード**ソリューション エクスプ ローラー**です。  
...
5.  選択`SimpleDate`の下部にある、**基になる列**ボックスの一覧し、をクリックして**OK**です。  
...
1.  **ディメンション構造**のディメンション デザイナーのタブ、`Date`ディメンションには、ドラッグ、**暦年**属性から、**属性**ペインに、**階層**ウィンドウです。  
  
2.  ドラッグ、 **Calendar Semester**属性から、**属性**ペイン、 **\<新しいレベル >** セル、**階層**ペイン、下、**暦年**レベル。  
  
3.  ドラッグ、 **Calendar Quarter**属性から、**属性**ペイン、 **\<新しいレベル >** セル、**階層**ペイン、下、 **Calendar Semester**レベル。  
  
4.  ドラッグ、 **English Month Name**属性から、**属性**ペイン、 **\<新しいレベル >** セル、**階層**ペイン、下、 **Calendar Quarter**レベル。  
  
5.  ドラッグ、 **Date Key**属性から、**属性**ペイン、 **\<新しいレベル >** セル、**階層**ペイン、の下に、 **English Month Name**レベル。  
  
6.  **階層** ウィンドウのタイトル バーを右クリックし、**階層**階層、 ボタンをクリック**の名前を変更**、し、入力`Calendar Date`です。  
  
7.  右クリック コンテキスト メニューを使用して、`Calendar Date`階層、名前を変更、 **English Month Name**レベルを`Calendar Month`の名前を変更し、 **Date Key**レベルを`Date`です。  
...
1.  **ディメンション デザイナー**の`Date`ディメンションをクリックして、**属性リレーションシップ**タブです。  
...
2.  **テーブル** ウィンドウを右クリックして`Date`、クリックして**新しい名前付き計算**です。  
  
3.  **名前付き計算の作成** ダイアログ ボックスで、「`MonthName`で、**列名**ボックスで、入力またはコピーして貼り付け、で次のステートメント、**式**ボックス:  
...
5.  **テーブル** ウィンドウを右クリックして`Date`、クリックして**新しい名前付き計算**です。  
  
6.  **名前付き計算の作成** ダイアログ ボックスで、「`CalendarQuarterDesc`で、**列名**ボックスで、入力またはコピーして貼り付け、次の SQL スクリプトを、**式**ボックス:  
...
8.  **テーブル** ウィンドウを右クリックして`Date`、クリックして**新しい名前付き計算**です。  
  
9. **名前付き計算の作成** ダイアログ ボックスで、「`CalendarSemesterDesc`で、**列名**ボックスで、入力またはコピーして貼り付け、次の SQL スクリプトを、**式**ボックス:  
...
8.  **名前列**] ダイアログ ボックスで、**基になる列**一覧で、[ `MonthName`、順にクリック **[ok]** です。  
...
6.  **名前列**] ダイアログ ボックスで、**基になる列**一覧で、[ `CalendarQuarterDesc`、順にクリック **[ok]** です。  
...
6.  **名前列**] ダイアログ ボックスで、**基になる列**一覧で、[ `CalendarSemesterDesc`、順にクリック **[ok]** です。  
...
2.  受信した後、**配置が正常に完了した**メッセージで、をクリックして、**ブラウザー**のタブ**ディメンション デザイナー**の`Date`ディメンション、およびデザイナーのツールバーの [再接続] ボタンをクリックします。  
...
1.  開く、**ディメンション構造**ディメンションのディメンション デザイナーのタブ、`Date`ディメンションをクリックし  **Calendar Semester**で、**属性**ウィンドウです。  
...
9. **で、** [ビルド] [!INCLUDE[ssBIDevStudio](../includes/ssbidevstudio-md.md)]メニューの **[Analysis Services Tutorial の配置]** をクリックします。 展開が正常に完了したらをクリックして、**ブラウザー**  タブのディメンション デザイナーで、`Date`ディメンションです。  
