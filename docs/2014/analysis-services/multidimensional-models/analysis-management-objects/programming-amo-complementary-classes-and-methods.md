...
3.  必要に応じてトレースを開始/停止します。  
     ...
     トレースを停止した後もトレースはサーバーに存在します。また、AutoRestart=`true` として定義されている場合は再び開始されます。  
...
1.  サーバー属性 CaptureXml を `true` に設定して XMLA ログのキャプチャを開始します。  
...
3.  CaptureXml を `false` に設定して XMLA ログのキャプチャを停止します。  
...
## <a name="see-also"></a>参照  
 ...
 [AMO の他のクラスとメソッド](amo-other-classes-and-methods.md)   
