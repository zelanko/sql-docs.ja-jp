...
-   **クエリスコープ**MDX クエリの一部として定義された計算されるセルを作成するために、スコープをそのクエリに限定する場合は、WITH キーワードを使用します。 その計算されるセルは、MDX の SELECT ステートメントの中で使用できます。 この方法では、`WITH` キーワードを使用して作成した計算されるセルを、SELECT ステートメントを修正せずに変更できます。  
...
-   **セッションスコープ**スコープが MDX セッションの有効期間である、クエリのコンテキストよりも範囲が広い計算されるメンバーを作成するには、CREATE CELL CALCULATION または ALTER CUBE ステートメントを使用します。  
