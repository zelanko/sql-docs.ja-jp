...
### <a name="using-the-iif-function"></a>IIf 関数の使用  
 この例の操作が一般的でない場合、または操作が1つの MDX クエリ[IIf](/sql/mdx/iif-mdx)のみに適用される場合は`RollupChildren` 、関数と共に IIf 関数を使用して同じ結果を得ることができます。 次の MDX クエリでは、前の例の MDX クエリと同じ結果を返します。ただし、カスタム メンバー プロパティは使用していません。  
