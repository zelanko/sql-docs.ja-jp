...
## <a name="readwritemode-database-property"></a>ReadWriteMode データベース プロパティ  
 `ReadWriteMode` データベース プロパティは、データベースが読み取り/書き込みモードと読み取り専用モードのどちらであるかを指定します。 このプロパティの使用可能な値は 2 つだけです。 データベースが読み取り専用モードの場合は、変更または更新をデータベースに適用できません。 一方、データベースが読み取り/書き込みモードの場合は、変更や更新を行うことができます。 `ReadWriteMode`データベース プロパティが読み取り専用プロパティとして定義されている; を使ってのみ設定できます、`Attach`コマンド。  
...
|MDX ステートメント<br /><br /> <br /><br /> 注: これらのステートメントのいずれかを実行すると、エラーが発生します。|`COMMIT TRAN`<br /><br /> `CREATE SESSION CUBE`<br /><br /> `ALTER CUBE`<br /><br /> `ALTER DIMENSION`<br /><br /> `CREATE DIMENSION MEMBER`<br /><br /> `DROP DIMENSION MEMBER`<br /><br /> `ALTER DIMENSION`<br /><br /> <br /><br /> メモ: Excel ユーザー機能を使用できません、グループ化ピボット テーブルでその機能が使用して内部的に実装されているため`CREATE SESSION CUBE`コマンド。|  
...
## <a name="readwritemode-usage"></a>ReadWriteMode の使用方法  
 `ReadWriteMode` データベース プロパティは、`Attach` データベース コマンドの一部として使用されます。 `Attach`コマンドを使用して、データベース プロパティを設定するいずれかに`ReadWrite`または`ReadOnly`です。 `ReadWriteMode` データベース プロパティの値は、読み取り専用として定義されているため、直接更新することはできません。 データベースは、`ReadWriteMode` プロパティが `ReadWrite` に設定された状態で作成されます。 データベースを読み取り専用モードで作成することはできません。  
  
 切り替えるには、`ReadWriteMode`データベース間でプロパティ`ReadWrite`と`ReadOnly`のシーケンスを実行する必要があります`Detach/Attach`コマンド。  
  
 例外を除いてすべてのデータベース操作、 `Attach`、保持、`ReadWriteMode`データベース プロパティの現在の状態。 たとえば、`Alter`、`Backup`、`Restore` および `Synchronize` などの操作では、`ReadWriteMode` 値が保持されます。  
