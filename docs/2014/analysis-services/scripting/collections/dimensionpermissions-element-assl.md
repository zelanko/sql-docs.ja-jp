...
## <a name="remarks"></a>コメント  
 `CubePermission` 要素の場合、このコレクション内の `DimensionPermission` 要素が、明示的に参照されている各ディメンションの `DimensionPermissions` コレクションで指定された権限をオーバーライドします。 ディメンションがこのコレクションで参照されない場合、`CubePermission` 要素は、ディメンションの `DimensionPermissions` コレクションで指定されている権限を継承します。  
