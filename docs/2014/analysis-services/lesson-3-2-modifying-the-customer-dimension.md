...
2.  **[属性]** ペインで **[English Country Region Name]** を右クリックし、 **[名前の変更]** をクリックします。 属性の名前を変更`Country-Region`です。  
...
1.  ドラッグ、`Country-Region`属性から、**属性**ペイン、**階層**ウィンドウです。  
  
2.  ドラッグ、`State-Province`属性から、**属性**ペイン、 **\<新しいレベル >** セル、**階層**下にある、ウィンドウ`Country-Region`レベル。  
  
3.  ドラッグ、**市区町村**属性から、**属性**ペイン、 **\<新しいレベル >** セル、**階層** ウィンドウで、下にある、`State-Province`レベル。  
  
4.  **階層**のペイン、**ディメンション構造** タブのタイトル バーを右クリックし、**階層**階層で、 **の名前を変更**、し、入力`Customer Geography`です。  
  
     階層の名前が`Customer Geography`です。  
...
3.  **名前付き計算の作成** ダイアログ ボックスで、「`FullName`で、**列名**ボックスで、入力またはコピーして貼り付け、次`CASE`内のステートメント、**式**ボックス。  
    ...
    ```  
     ...
     `CASE`ステートメントを連結、 **FirstName**、 **MiddleName**、および**LastName**として、顧客に使用する 1 つの列に列の寸法表示名、**顧客**属性。  
  
4.  **[OK]** をクリックします。次に、 **[テーブル]** ペインで **Customer** を展開します。  
  
     `FullName`名前付き計算が Customer テーブル内の列の一覧に表示されることを示すアイコンが名前付き計算します。  
...
7.  **[Customer テーブルの探索]** ビューで、最後の列を確認します。  
     ...
     注意して、`FullName`元のデータ ソースは変更されず、基になるデータ ソースから複数の列からデータを適切に連結列、データ ソース ビューに表示されます。  
...
4.  **名前**プロパティ フィールドに「`Full Name`です。  
  
5.  下部の **[NameColumn]** プロパティ フィールド内をクリックし、参照ボタン (**[…]**) をクリックして、 **[名前列]** ダイアログ ボックスを開きます。  
  
6.  選択`FullName`の下部にある、**基になる列**ボックスの一覧し、をクリックして**OK**です。  
  
7.  [ディメンション構造] タブで、ドラッグ、`Full Name`属性から、**属性**ペイン、 **\<新しいレベル >** セル、**階層**ペイン、下、**市区町村**レベル。  
...
3.  プロパティ ウィンドウ、 **AttributeHierarchyDisplayFolder**上部で (完全な名前を表示する をポイントする必要があります)、プロパティ フィールドと入力し、`Location`です。  
  
4.  **階層** ウィンドウで、をクリックして`Customer Geography`、し、右側の プロパティ ウィンドウで次のように選択します。`Location`の値として、 **DisplayFolder**プロパティです。  
...
6.  [プロパティ] ウィンドウ、 **AttributeHierarchyDisplayFolder**プロパティは、上部にあるフィールドし、入力`Demographic`です。  
...
6.  設定する、 **NameColumn**のプロパティ、`State-Province`属性をクリックして、 **NameColumn**  フィールド、プロパティ ウィンドウに、参照ボタンをクリックして (**.**) ボタンをクリックします。  
...
3.  **[属性リレーションシップの作成]** ダイアログ ボックスで、 **[基になる属性]** に **[City]** を指定します。 設定、**関連属性**に`State-Province`です。  
...
6.  ダイアグラムを右クリックし、`State-Province`属性を選択し、**新しい属性リレーションシップ**です。  
  
7.  **属性リレーションシップの作成** ダイアログ ボックスで、**基になる属性**は`State-Province`します。 設定、**関連属性**に`Country-Region`です。  
...
3.  いることを確認`Customer Geography`でが選択されている、**階層**一覧を展開し、ブラウザー ウィンドウで、次のように展開します**すべて**、展開**オーストラリア**、展開**新しい南部。ウェールズ**の順に展開および**Coffs あるハーバー**です。  
...
## <a name="see-also"></a>参照  
 ...
 [属性の名前を変更します。](multidimensional-models/attribute-properties-rename-an-attribute.md)   
