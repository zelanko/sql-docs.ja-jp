...
##  <a name="bkmk_Top"></a>デシジョンツリーモデルの構造について  
...
##  <a name="bkmk_ModelContent"></a>デシジョンツリーモデルのモデルコンテンツ  
 ...
 **ツリーノード**各ツリーの [**すべて**のノードを使用すると、対象の属性に使用された値の数がわかります。  
 ...
 **(すべて) ノード**モデル全体の概要が含まれています。  
  
 **内部ノード**リーフノードの集計された統計情報を格納します。  
  
 **リーフノード**現在のリーフノードを指すパス内のすべての条件を指定した場合に、予測される結果のサポートと確率を格納します。  
  
 **回帰ノード**入力と予測可能な属性の間のリレーションシップを表す回帰式が含まれます。  
...
###  <a name="NodeCaption"></a>ノードのキャプションとノードの説明  
...
###  <a name="NodeRule"></a>ノードルールとルールの規則  
...
##  <a name="bkmk_RegressionNodes"></a>デシジョンツリーモデル内の回帰ノード  
