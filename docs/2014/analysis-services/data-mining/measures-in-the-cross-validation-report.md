...
|分類|分類モデルに適用されるメジャー:<br /><br /> **真陽性**/<br />                      **True 負**/ の**偽陽性**/ の**肯定**: 予測された状態が対象の状態と一致し、予測確率が指定されたしきい値を超えるパーティション内の行または値の数。 対象の属性の不足値があるケースは除外されます。つまり、すべての値のカウントが加算されない可能性があります。|  
