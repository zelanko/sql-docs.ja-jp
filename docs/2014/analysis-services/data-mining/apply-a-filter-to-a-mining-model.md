...
8.  **[値]** ボックスをクリックし、ボックスに値を入力します。  
     ...
     たとえば、列と`Income`してを選択し、大なり演算子 (>) を選択して`30000`、「」と入力します。  
  
9. グリッドの次の行をクリックします。  
  
     作成したフィルター条件が [式] ボックスに自動的に追加されます。 たとえば、 `[Income] > '30000'` と記述します。  
...
11. 演算子を選択し、手順 7. と手順 8. に示したように値を入力します。  
     ...
     たとえば、列と`Income`してをもう一度選択し、小なり演算子 (<) を選択し`40000`て、「」と入力します。  
...
1.  [ ** \<モデルフィルターの名前>** ] ダイアログボックスで、[**マイニング構造列**] の下のグリッドの空の行をクリックします。  
...
4.  [**値**] ボックスをクリックし、[ **..** .] ボタンをクリックして式を作成します。  
     ...
     [ ** \<名前>フィルター** ] ダイアログボックスが表示されます。 このダイアログ ボックスでは、現在のテーブルにのみ条件を設定できます。ここでは、入れ子になったテーブルです。  
...
7.  **[値]** をクリックし、値を入力します。  
     ...
     たとえば、[**マイニング構造列**] には`Model`、を選択します。 [**演算子**] で`<>`、を選択し、 `Water Bottle`値を入力します。 この条件で次のフィルター式が作成されます。  
...
1.  必要に応じて条件を追加します。条件`AND`を`OR`組み合わせるには、[**条件**] グリッドの左側にある **[** または] ボックスを選択します。 [!INCLUDE[clickOK](../../includes/clickok-md.md)]  
