...
## <a name="calculation-of-the-missing-state"></a>不足状態の計算  
 ...
 マイニング モデルを作成するときに、`Missing`状態が自動的にすべての不連続列のモデルを追加します。 たとえば、入力列 [Gender] に、Male と Female の 2 つの値が含まれている場合、3 番目の値が自動的に追加を表す、`Missing`値、および列には常の数が含まれています、すべての値の分布を示すヒストグラム持つケース`Missing`値。 Gender 列に値の不足がない場合は、Missing 状態が見つかったケースの数が 0 としてヒストグラムに示されます。  
...
|Missing|0|  
 ...
 入力が連続列の場合には、この属性に対して `Existing` と `Missing` の 2 つの可能な状態が適用されます。 つまり、その列に何らかの数値データ型の値が含まれているか、値が何も含まれていないかのいずれかになります。 値があるケースの場合は、平均、標準偏差、およびその他の意味のある統計がモデルで計算されます。 値がない場合は、モデルを使用するの数、`Missing`値と予測が適宜調整されます。 予測を調整する方法はアルゴリズムによって異なります。詳細については次のセクションを参照してください。  
...
|Missing|0|0.03%|  
 ...
 奇妙に見えるかもしれませんが、確率、`Missing`値ケースの数が 0 なのに 0.03% として計算されてです。 実のところ、この動作は意図的なものであり、モデルで不明な値を適切に処理できるようにするための調整を表しています。  
...
### <a name="special-handling-of-missing-values-in-decision-tree-models"></a>デシジョン ツリー モデルにおける不足値の特別な処理  
 Microsoft デシジョン ツリー アルゴリズムによる不足値の確率の計算方法は、他のアルゴリズムと異なります。 デシジョン ツリー アルゴリズムを調整単純ケースの合計数に 1 を追加ではなく、`Missing`若干異なる式を使用して状態。  
...
|処理手順|リンク|  
|-----------|-----------|  
|不足値の処理を制御するためのフラグを個々のモデル列に追加する。|[モデリング フラグの変更を表示または&#40;データ マイニング&#41;](modeling-flags-data-mining.md)|  
