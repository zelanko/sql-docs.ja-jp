...
-   [式の時系列表示モデル&#40;データ マイニング&#41;](view-the-formula-for-a-time-series-model-data-mining.md)  
 ...
 **NODE_CAPTION:** 式の短い形式が表示されます。 この短い形式からは、表現されている周期構造の数と、その係数の数がわかります。 たとえば、式の短い形式が {4,0,6}である場合、ノードは係数が 6 である 1 つの周期構造を表します。 短い形式はのような場合{2,0,8}x {1,0,0}(4) は、ノードには、2 つの周期構造が含まれています。  
