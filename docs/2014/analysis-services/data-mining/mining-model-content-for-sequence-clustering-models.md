 ...
 **モデルルート**カーディナリティの値は、クラスターの数に1を加えた数と等しくなります。 詳細については、「 [カーディナリティ](#bkmk_cardinality)」を参照してください。  
  
 **クラスターノード**各クラスターには1つの子ノードがあるため、カーディナリティは常に1です。これには、クラスター内のシーケンスの一覧が含まれています。  
  
 **シーケンスノード**カーディナリティは、そのクラスターに含まれる遷移の数を示します。 たとえば、モデル ルートのシーケンス ノードのカーディナリティは、モデル全体で検出された遷移の数を示します。  
 ...
 **クラスターノード**モデル内のクラスターの調整された確率。 調整済みの確率は、合計が 1 になりません。これは、シーケンス クラスターで使用されるクラスタリング手法では、複数のクラスターでの部分的なメンバーシップが許可されるためです。  
 ...
 **クラスターノード**NODE_PROBABILITY と同じ値。  
 ...
 **クラスターノード**クラスターの未加工のサポート。つまり、このクラスターに対してケースを寄与するトレーニングケースの数を表します。  
  
 **シーケンスノード**常に0です。  
  
 **切り替えノード**クラスター内の特定の遷移を表すケースの割合。 0 または正の値になります。 クラスター ノードの未加工のサポートにクラスターの確率を掛けて計算されます。  
...
###  <a name="bkmk_NODEDIST"></a>NODE_DISTRIBUTION テーブル  
 ...
 遷移のテーブルには、有効な `Missing` 値を表すための行が常に 1 行追加されます。 値の`Missing`意味と計算に与える影響の詳細については、「[欠損値 &#40;Analysis Services データマイニング&#41;](missing-values-analysis-services-data-mining.md)」を参照してください。  
...
|-----------------|----------------|-----------|  
|SO58845|1 で保護されたプロセスとして起動されました|Mountain-500|  
...
3.  
  **[クラスター]** ボックスの一覧で、 **[母集団 (すべて)]** が選択されていることを確認します。  
...
3.  
  **[ノードのキャプション]** ペインで、 **[クラスター 16 のシーケンス レベル]** という名前のノードをクリックします。  
