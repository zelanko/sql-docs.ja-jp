---
title: 現地の通貨参照 (ビジネス インテリジェンス ウィザード) の定義 |Microsoft ドキュメント
ms.custom: ''
ms.date: 06/13/2017
 ...
 **ファクト テーブル内の識別子**  
 選択すると、 **[メジャーの選択]** ページで選択されたメジャーを格納するファクト テーブルから参照される、通貨ディメンションの現地通貨の通貨識別子を表す属性を指定できます (通貨ディメンションのいずれかである`Type`プロパティに設定されている*通貨*)。  
 ...
 **通貨ディメンションとファクト データによって参照される属性**  
 通貨ディメンションのメンバーが現地通貨の通貨識別子を表す、その通貨ディメンション内の通貨属性を選択します。 (通貨属性は 1 つ持つ`Type`プロパティに設定されている*通貨*)。  
