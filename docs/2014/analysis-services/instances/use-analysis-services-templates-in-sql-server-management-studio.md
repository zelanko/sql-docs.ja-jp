 ...
 **テンプレート エクスプローラー**を開くには、 **[表示]** メニューから選択します。 次に、キューブ アイコンをクリックして、 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]で使用できるテンプレートの一覧を表示します。  
 ...
 ![Analysis Services テンプレート エクスプ ローラーで、フィルター処理されていて](../media/ssas-templateexplorer.gif "テンプレート エクスプ ローラーで、Analysis Services に対してフィルター処理")  
...
> [!WARNING]  
>  直接クエリ モードで配置されているテーブル モデルに対して MDX クエリを使用することはできません。 ただし、EVALUATE コマンドと DAX テーブル クエリを使用して、同等のクエリを送信することができます。 詳細については、次を参照してください。 [DAX クエリのパラメーター](https://msdn.microsoft.com/library/gg492200(v=sql.120).aspx)です。  
...
4.  クエリはそのまま実行できますが、特定のメンバーを返す関数を追加するなど、変更を加えることもできます。 たとえば、入力`.members`後 **[Product Category] です [。Product Category Name]** です。 詳細については、「 [メンバー式の使用](/sql/mdx/using-member-expressions)」をご覧ください。  
...
2.  ドラッグ、`Backup`空のクエリ ウィンドウにテンプレート。  
...
3.  テンプレートで、置換、 [RequestType 要素&#40;XMLA&#41; ](../xmla/xml-elements-properties/type-element-xmla.md)要素を次のテキスト。 `<RequestType>MDSCHEMA_INPUT_DATASOURCES</RequestType>`  
...
||連続列値|DMX SELECT DISTINCT FROM を使用する方法を示します*\<モデル >* ステートメントを DMX`RangeMin`と`RangeMax`連続列から指定された範囲の値のセットを取得する関数、指定したマイニング モデルです。|  
...
|MDX\式|移動平均 - 固定|MDX を使用する方法を示します`ParallelPeriod`と`CurrentMember`固定数の時間間隔の時間ディメンションの階層に含まれるメジャーの移動平均を提供する、計算メジャーを作成する自然順序のセットを持つ関数です。|  
||移動平均 - 可変|MDX を使用する方法を示します`CASE`内のステートメント、`Avg`可変個の時間間隔の時間ディメンションの階層に含まれるメジャーの移動平均を提供する、計算メジャーを作成する関数。|  
||期間取得|計算メンバー内の MDX `PeriodsToDate` 関数の使用方法を示します。|  
||親に対する比率|MDX を使用する方法を示します`Parent`をそれぞれの子の指定された階層で親メンバーのメジャーの比率を表す、計算されるメジャーを作成する関数。|  
...
||KPI クエリ|MDX を使用する方法を示します`KPIValue`と`KPIGoal`関数を MDX クエリで主要業績評価指標 (KPI) 情報を取得します。|  
...
|XMLA\管理|バックアップ|XMLA を使用する方法を示します`Backup`をバックアップするコマンド、[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]ファイルへのデータベースです。|  
||キャンセル|XMLA を使用する方法を示します`Cancel`コマンド以外のユーザーの管理者またはサーバー管理者)、現在のセッションのすべての実行操作を取り消し、(管理者用)、データベースまたはインスタンスをサーバー管理者)。|  
...
||パーティションの処理|XMLA を使用する方法を示します`Batch`コマンドと組み合わせて、`Parallel`要素および`Process`並列バッチ操作を使用してパーティションを完全に処理する場合はコマンド。|  
||[復元]|XMLA を使用する方法を示します`Restore`を復元するコマンド、[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]既存のバックアップ ファイルからデータベース。|  
||同期|XMLA を使用する方法を示します`Synchronize`別を同期するためにコマンド[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]、現在のデータベース[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]SynchronizeSecurity タグの SkipMembership オプションを使用してデータベースします。|  
...
||[場所]|XMLA を使用する方法を示します`Discover`バックアップ ファイルの場所のパスを指定する、DISCOVER_LOCATIONS スキーマ行セットの内容を取得します。|  
