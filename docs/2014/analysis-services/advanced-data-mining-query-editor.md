...
> [!NOTE]  
>  **警告:** ビューを切り替えると、クエリを実行する前に、いくつかの変更が失われる可能性があることを示すのメッセージが表示されます。 DMX ステートメントは有効であれば、多くの場合、**クエリ ビルダー**によってこれらの変更を適切に変換します。 ただし、特に複雑な DMX ステートメントを作成した場合は、ビューを切り替える前に作業内容を必ず保存してください。  
