---
title: シーケンス クラスターのクラスターの特性 タブ (マイニング モデル ビューアー) |Microsoft ドキュメント
ms.custom: ''
ms.date: 03/06/2017
...
|値|説明|  
|-----------|-----------------|  
|**変数**|この列は、特性が値であるかどうかまたは移行を示します。<br /><br /> 特性が、値の場合、**変数**列には、属性名が含まれています。<br /><br /> 特性が状態遷移を表している場合、**変数**列には「転移」というテキストが含まれています。|  
|**値**|この列の値は、特性が単純な属性と値ペアであるかどうか、またはアイテムまたはイベントの一般的なシーケンスを表す状態遷移に依存します。<br /><br /> 特性が、値の場合、**値**列には、状態が含まれています。<br /><br /> 特性が状態遷移を表している場合、**値**列には、状態遷移の説明が含まれています。|  
