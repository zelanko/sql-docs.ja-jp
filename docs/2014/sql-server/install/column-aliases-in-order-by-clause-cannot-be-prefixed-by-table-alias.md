...
### <a name="exception"></a>Exception  
 ORDER BY 句で指定された、プレフィックスの付いた列の別名が、指定したテーブルの有効な列名であれば、クエリは問題なく実行されます。[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] では、ステートメントのセマンティクスが異なる場合があります。 たとえば、以下のステートメントで指定された列の別名 (`id`) が `sysobjects` テーブル内の有効な列名であるとします。 
  [!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)] ではステートメントが実行されると、結果セットが並べ替えられた後、`CAST` 演算が実行されます。 つまり、`name` 列が並べ替え操作で使用されます。 
  [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] では並べ替え操作の前に `CAST` 演算が実行されます。 つまり、テーブル内の `id` 列が並べ替え操作に使用され、結果セットが予期しない順序で返されます。  
