...
2.  [ ** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]コンポーネント**] ページで、スキャンするサーバーの名前を [**サーバー名**] ボックスに入力し、[**検出**] をクリックします。 サーバー名については次のガイドラインに従ってください。  
    ...
    >   
    >  アップグレードアドバイザーを実行し[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]ているコンピューターにがインストールされていない場合は、[**開始**] を`cliconfg`クリックし、を実行します。 [ ** [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]クライアントネットワークユーティリティ**] ダイアログボックスが開きます。 エイリアスを作成するには、[**別名**] タブを使用します。  
