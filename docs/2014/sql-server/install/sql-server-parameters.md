---
title: SQL Server パラメーター |Microsoft ドキュメント
ms.custom: ''
ms.date: 03/06/2017
 ...
 **分析[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]ファイルをバッチ処理**  
 ファイル システムの [!INCLUDE[tsql](../../includes/tsql-md.md)] バッチ ファイルを分析するには、このチェック ボックスをオンにします。  
  
 **パス[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]ファイルをバッチ処理**  
 1 つ以上のバッチ ファイルを分析できます。 場所を参照して複数のファイルを選択したり、複数のファイル名を入力できます。 各ファイルへの完全パス名およびファイル名を使用し、パイプ文字 (|) でエントリを区切ります。  
  
 有効にした場合**分析 SQL バッチ ファイル**、 **[次へ]** パス名とファイル名を入力するまで、ボタンは無効にします。  
  
 **SQL バッチ区切り記号**  
 [!INCLUDE[tsql](../../includes/tsql-md.md)] ステートメントの個別のバッチに使用するテキストです。 既定値は**移動**です。  
