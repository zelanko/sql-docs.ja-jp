...
-   エラーの原因となったプロセスのメモリまたはファイルからの情報。  
 ...
 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 意図的に収集しません、ファイル、名前、アドレス、電子メール アドレス、またはその他の個人情報の形式。 ただし、エラーの原因となったプロセスのメモリまたはファイルに格納されている個人情報が、エラー レポートとして送信される場合もあります。 この情報を使用者の ID を判断するために使用できる可能性がありますが、[!INCLUDE[msCoName](../../includes/msconame-md.md)] はこの情報をそのような目的に使用することはありません。  
