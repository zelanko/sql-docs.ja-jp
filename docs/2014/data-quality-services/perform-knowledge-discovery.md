...
5.  **[次へ]** をクリックして、ナレッジ検出ウィザードの **[検出]** ページに進みます。 代わりに次の操作を行うこともできます。  
    ...
    -   **[閉じる]** をクリックします。作業内容が保存され、DQS ホーム ページに戻ります。 ナレッジ ベースがロックされ、 **[ナレッジ ベースを開く]** 画面のナレッジ ベース テーブルのナレッジ ベースの状態が **[検出 - マッピング]** になります。 クリックした後**閉じる**、ドメイン管理アクティビティを実行するには、をクリックする必要があります**ナレッジ検出**から、**ナレッジ ベースを開く**画面に進んでください、**ナレッジ ベース管理。ドメイン用語の管理**画面で、[**完了**、] をクリックし、**はい**ナレッジ ベースを発行するまたは**いいえ**ナレッジ ベースに対して、作業を保存して終了します。  
...
2.  検出プロセスが実行されている場合、検出の各手順に対して表示される完了ステータスを確認します。**レコードの前処理**、**ドメイン ルールを実行している**、および**探索を実行する**します。 各手順の完了率と完了状態が表示されます。  
...
6.  **[次へ]** をクリックして、ナレッジ検出ウィザードの **[ドメイン値の管理]** ページに進みます。 このページでは、ナレッジ ベースのドメインに追加されたナレッジを変更できます。 代わりに次の操作を行うこともできます。  
    ...
    -   **[閉じる]** をクリックします。作業内容が保存され、DQS ホーム ページに戻ります。 ナレッジ ベースがロックされ、 **[ナレッジ ベースを開く]** 画面のナレッジ ベース テーブルのナレッジ ベースの状態が **[検出 - 検出]** になります。 クリックした後**閉じる**、ドメイン管理アクティビティを実行するには、をクリックする必要があります**ナレッジ検出**から、**ナレッジ ベースを開く**画面に進んでください、**ナレッジ ベース管理。ドメイン用語の管理**画面で、[**完了**、] をクリックし、**はい**ナレッジ ベースを発行するまたは**いいえ**ナレッジ ベースに対して、作業を保存して終了します。  
...
4.  値の状態を変更するには、次の手順に従います。  
    ...
    1.  **修正済みとして選択したドメイン値を設定**:適切にエラーまたは無効なから値の状態を変更するに値を選択し、**修正済みとしてのドメインの値を選択した設定**(チェック)、アイコン バーの下矢印をからまたは型のドロップダウン リストから。 その "エラー" または "無効" の値が "適切" の値とグループ化される場合は、この操作の後でその値を削除します。  
  
    2.  **選択したドメイン値をエラーとして設定**:エラーを修正または無効なから値の状態を変更するに値を選択し、**選択したドメイン値をエラーとして設定**(間) のアイコン、アイコン バーの下矢印をからまたは型のドロップダウン リストから。 **[次に修正]** 列に修正を入力することも、空のままにしておくこともできます。  
  
    3.  **選択したドメイン値が無効として設定**:エラーを修正または無効な値の状態を変更するには、値を選択クリックして、**選択したドメイン値を無効として設定**(三角形) アイコン、アイコン バーの下矢印をからまたは型のドロップダウン リストから。 **[次に修正]** 列に修正を入力することも、空のままにしておくこともできます。  
  
    4.  **正しい**:エラー"または"無効"として値を設定した後に新しい値を入力、**に修正**列。 その置換値のための新しい行が追加され、"適切" として指定されて、2 つの値がグループ化されます。 新しい値は先頭の値として太字で表示され、"エラー" または "無効" の値はインデントされます。  
  
5.  値をシノニムのグループとして指定するには、"適切" の値を複数選択し、次の手順に従います。  
  
    -   **選択したドメイン値をシノニムとして設定**:クリックすると、選択した値をシノニムとして設定します。 値の 1 つが先頭の値として指定され、その他の値がその値に置き換えられます。  
  
        > [!NOTE]  
        >  グループ内の複数の値とグループ外の値を選択してシノニムとして設定すると、正しくないエラー メッセージが表示されますが、 そのエラー メッセージのポップアップを閉じると、それらの値がシノニムとして正しく設定されます。  
  
    -   **選択したシノニム間の関係**:シノニムの指定を元に戻す をクリックします。  
  
    -   **選択したドメイン値をそのグループの先頭の値として設定**:グループの先頭の値を変更するには、先頭の値として指定されていないグループ内の値を選択し、をクリックして、**選択したドメイン値をそのグループの先頭の値として設定**ボタンをクリックします。  
  
6.  **スペル チェック**:ドメインのプロパティ ページでスペル チェックを有効にした場合を赤い波線、スペル チェック、修正が提案 ことを示す値を持つ任意の値を検索します。 その値を右クリックし、必要に応じて修正を選択します。 修正を選択すると、値の種類が "エラー" になり (最初から "エラー" の場合はそのまま)、その修正が **[次に修正]** 列に追加されます。 下矢印をクリックすると、その他の修正案が表示されます。 手動で修正を入力してスペル チェックの辞書に追加すると、修正として選択できるようになります。 詳細については、「 [Use the DQS Speller](../../2014/data-quality-services/use-the-dqs-speller.md) 」および「 [Set Domain Properties](../../2014/data-quality-services/set-domain-properties.md)」を参照してください。  
...
7.  **新しいドメインの値を追加**:クリックして、ドメインに新しい値を追加、**新しいドメインの値を追加**テーブルの末尾に行を追加するボタンをクリックします。 その行に値を入力すると、行がアルファベット順に並べ替えられます。  
  
8.  **Excel からドメイン値のインポート**:下矢印をクリックして、Excel スプレッドシートから新しい値を追加、**値のインポート**アイコン、および選択**Excel からドメイン値のインポート**します。 ファイル名を入力し、必要に応じて **[先頭の行を見出しとして使用]** を選択し、 **[OK]** をクリックします。 詳細については、「 [値を Excel ファイルからドメインへインポートする](../../2014/data-quality-services/import-values-from-an-excel-file-into-a-domain.md)」をご参照ください。  
  
9. **プロジェクトの値をインポート**:下矢印をクリックして、データ品質プロジェクトから新しい値を追加、**値のインポート**アイコン、および選択**プロジェクトの値をインポート**します。 ファイル名を入力し、必要に応じて **[先頭の行を見出しとして使用]** を選択し、 **[OK]** をクリックします。 値をインポートするプロジェクトを選択し、 **[OK]** をクリックします。 インポートされた値が表示されます。 **[完了]** をクリックします。 詳細については、「プロジェクトの値をドメインにインポートする」を参照してください。  
  
10. **選択したドメインの値を削除**:1 つまたは複数の既存の値を値を選択し、をクリックして、ドメインから削除、**選択したドメイン値を Delete**ボタンをクリックします。 DQS_NULL というエントリは削除できません。したがって、削除する値を複数選択した場合に、選択した値の中に DQS_NULL が含まれていると、操作が失敗します。  
...
11. ナレッジ検出アクティビティを完了するには、 **[完了]** をクリックします。 まだ確認されていないドメインがある場合は、ポップアップ画面が表示されます。 確認を続ける場合は **[はい]** を、先に進む場合は **[いいえ]** をクリックします。 [いいえ] をクリックすると、また別のポップアップ画面が表示されます。そこで次のいずれかを選択します。  
    ...
    1.  **発行**:現在のユーザーまたは他のユーザーを使用して、に対してナレッジ ベースが発行されます。 ナレッジ ベースはロックされず、(ナレッジ ベース テーブルの) ナレッジ ベースの状態が空白に設定されます。ドメイン管理アクティビティとナレッジ検出アクティビティの両方を使用できるようになります。 ホーム ページに戻ります。 プロセスを完了するには、ポップアップ画面で **[はい]** をクリックします。  
  
    2.  **いいえ**:作業が保存され、ナレッジ ベースがロックされたままに、作業で、ナレッジ ベースの状態設定されます。 ドメイン管理アクティビティとナレッジ検出アクティビティの両方を使用できるようになります。 ホーム ページに戻ります。  
  
    3.  **キャンセル**:ポップアップが閉じの維持ができます、**ドメイン値の管理**ページ。  
...
12. 代わりに次の操作を行うこともできます。  
    ...
    -   **[戻る]** をクリックして **[検出]** ページに戻ります。 クリックした後**閉じる**、ドメイン管理アクティビティを実行するには、をクリックする必要があります**ナレッジ検出**から、**ナレッジ ベースを開く**画面に進んでください、**ナレッジ ベース管理。ドメイン用語の管理**画面で、[**完了**、] をクリックし、**はい**ナレッジ ベースを発行するまたは**いいえ**ナレッジ ベースに対して、作業を保存して終了します。  
  
##  <a name="FollowUp"></a> フォローしてください：ナレッジ検出を実行した後  
...
-   **そうです：** これは、ドメインに属している、構文エラーがない値です。 たとえば、City ドメインの "Chicago" は正しい値です。  
  
-   **エラー:** これは、ドメインに属しているが、正しくない値を値です。 たとえば、City ドメインの "Chicago" が "Shicago" になっている場合はエラーになります。 DQS では、検出プロセスで構文エラーおよび関連付けられた修正が検出された場合に、値が "エラー" として指定されます。 構文エラーには、スペルミスなどがあります。  
  
-   **無効です：** これは、値は、ドメインに属していませんし、修正はありません。 たとえば、City ドメインの値 "12345" は無効です。 DQS では、ドメイン ルールが失敗した場合に値が "無効" として指定されます。  
...
-   **レコード**:検出されたデータ サンプル レコードの数  
  
-   **値を合計**:各フィールドおよびの合計数の合計値が見つかりました  
  
-   **新しい値**:値の合計に占める割合、前回の検出プロセス以降の各フィールドの合計値の数とすべてのマップされたフィールドは新しい機能  
  
-   **一意の値**:数、合計値の各フィールドおよびマップされたすべてのフィールドが一意であると値の合計に占める割合  
  
-   **新しい一意の値**:値の合計に占める割合、前回の検出プロセス以降の各フィールドの一意の値の数とすべてのマップされたフィールドは新しい機能  
  
-   **ドメイン値で有効**:数、合計値の各フィールドおよびマップされたすべてのフィールドが有効な場合と値の合計に占める割合  
...
-   **新しい**:新しい値と、フィールドの既存の値と比較して、新しい値の割合の数  
  
-   **一意**:フィールドと、合計に占める一意のレコードの数  
  
-   **ドメインで有効**:有効なドメイン値と合計の割合の数  
  
-   **完全を期すため**:照合のテストのためにマップされた各ソース フィールドの完全性  
