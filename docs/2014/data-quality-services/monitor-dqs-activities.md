...
3.  アクティビティ監視画面には、アクティビティ グリッド内の各アクティビティに関する情報が表示されます。 アクティビティ グリッドには、各 DQS アクティビティに関する次の情報が表示されます。  
    ...
    |**アクティブ**|アクティビティが現在アクティブかどうかを示します。 次のいずれかの値になります。<br /><br /> **アクティブな**:アクティビティは現在実行中です。<br /><br /> **終了**:アクティビティは終了しました。<br /><br /> **終了**:アクティビティが終了されたアクティビティ監視画面を使用して、DQS 管理者、または、アクティビティは、該当する機能領域で実行中に、ユーザーによって取り消されました[!INCLUDE[ssDQSClient](../includes/ssdqsclient-md.md)]します。|  
    ...
    |**現在の状態**|アクティビティの現在の状態を示します。 アクティビティの状態は、最後の計算プロセスによって決定されます。 次のいずれかの値になります。<br /><br /> **実行している**:計算プロセスが実行されています。<br /><br /> **成功した**:状態が に設定されているすべての計算プロセスが実行すると、前に**Succeeded**します。 計算プロセスが正常に終了すると、状態は再度 **[成功]** に設定されます。<br /><br /> **失敗**:計算プロセスが失敗しました。<br /><br /> **停止**:計算プロセスが停止しました。<br /><br /> <br /><br /> 注:(ナレッジ検出アクティビティ内で) 検出プロセスを複数回実行する場合など、1 つのアクティビティ内に複数の計算プロセスが存在する場合があります。 そのため、アクティビティの存続期間の間に状態が何度か変化する場合があります。|  
...
1.  フィルター条件を決定します。アクティビティ レコードをフィルター処理する際に、アクティビティ グリッド内のいずれかの列の値に基づくか (値ベース)、日付範囲に基づくか、その両方に基づくかを決定します。  
    ...
    1.  **値ベースのフィルター処理**:内のフィルター条件を選択、**フィルターによって**ボックスの一覧し、選択し、適切な値でフィルターを**値**一覧。 **[フィルター条件]** ボックスの一覧でオプションを選択すると、 **[値]** ボックスの一覧が設定可能な値で更新されます。 アクティビティ レコードの次のフィールドでフィルター処理できます。**アクティブな**、**型**、**サブタイプ**、**現在の状態**、 **DQKB**、および**ユーザー**します。  
  
    2.  **日付範囲ベースのフィルター処理**:適切な日付を選択すると、**から日付**と**に日付**日付コントロール。 既定では、 **[開始日]** には現在の日付の 2 日前の日付が表示され、 **[終了日]** には現在の日付が表示されます。 フィルター処理は、 *開始日* および *終了日* ではなく範囲に基づいて行われます。 これは、選択した日付範囲の間に実行されていた各アクティビティが表示されることを意味します。  
...
2.  下部ペインの次の 2 つのタブに、選択したアクティビティの詳細が表示されます。  
    ...
    -   **アクティビティ ステップ**:選択したアクティビティに関連付けられている計算プロセス (アクティビティ ステップ) のグリッドが表示されます。 このタブには、1 つのアクティビティに対して複数のアクティビティ ステップが表示される場合があります。このような状況は、ユーザーが 1 つのアクティビティで同じアクティビティ ステップを複数回実行したときに発生します。 たとえば、アクティビティ ステップを停止した後に再び開始したときに発生します。 このタブの下のグリッドには、アクティビティに関連付けられたアクティビティ ステップごとに、次の情報が表示されます。**型**、**現在の状態**、**開始時刻**、**経過時間**、および**終了時刻**します。  
  
    -   **Profiler**:現在と過去のアクティビティのプロファイル情報を表示します。 現在のアクティビティについては、部分的ではあるが一貫性のある情報が表示されます。 アクティビティのプロファイル情報は、対応するアクティビティの詳細を Excel ファイルにエクスポートしたときに Excel ファイルにエクスポートされます。 この情報は、エクスポートした Excel ファイルの **[プロファイラー - ソース]** シートと **[プロファイラー - フィールド]** シートに表示されます。  
...
3.  保存する Excel ファイルの名前と場所を指定するように求めるメッセージが表示されます。 エクスポートした Excel ファイルには、次のシートが含まれています。  
    ...
    |プロファイラー - ソース|**クレンジング**サブタイプ、アクティビティに関する次の情報が含まれています。 レコード、適正なレコード、修正されたレコード、および無効なレコード。<br /><br /> **ナレッジ検出**、**ドメイン管理**、**照合ポリシー**、および**照合**サブタイプの場合、次が含まれています利用状況に関する情報: レコード、合計値、新しい値、一意の値、および新しい一意の値。|  
    |プロファイラー - フィールド|**クレンジング**と**SSIS クレンジング**サブタイプの場合、アクティビティに関する次の情報が含まれています。 フィールド、ドメインは、値、完全を期すため、および精度に修正された値、お勧めします。<br /><br /> **ナレッジ検出**、**ドメイン管理**、**照合ポリシー**、および**照合**サブタイプの場合、次が含まれています利用状況に関する情報: フィールドを新しいドメイン、一意のドメイン、および完全を期すためでは無効です。|  
