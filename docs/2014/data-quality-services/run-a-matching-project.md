...
1.  **エクスポート**で一致するデータのエクスポート先の選択 ページで、**変換先の型**:**SQL Server**、 **CSV ファイル**、または**Excel ファイル**します。  
...
-   **フィールド**:マッピングに含まれているフィールドの名前。  
  
-   **ドメイン**:フィールドにマップされたドメインの名前です。  
  
-   **新しい**:検出された新しい一致の数と合計に占める割合  
  
-   **一意**:フィールドと、合計に占める一意のレコードの数  
  
-   **完全を期すため**:ルールの実行が完了した割合。  
