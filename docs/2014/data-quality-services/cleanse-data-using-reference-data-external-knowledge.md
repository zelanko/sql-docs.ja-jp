...
-   ドメインの値は、次のように分類されます。**推奨される**、**新しい**、**無効な**、**修正**、および**正しい**します。  
...
2.  **マップ**ページで、ソース データ内の適切な列で次の 4 つのドメインをマップします。**住所行**、**市区町村**、**状態**、および**Zip**します。 **[次へ]** をクリックします。  
...
3.  **[最適化]** ページで、 **[開始]** をクリックしてコンピューター支援型のクレンジング プロセスを実行します。 クレンジング プロセスが完了したら、 **[次へ]** をクリックします。  
    ...
    >   
    >  -   以下のメッセージが表示されます、**開始**ボタンをクリックします。"ドメイン\<Domain1 >、 \<Domain2 >、.\<DomainN> を、参照データ サービス プロバイダーを使用してクレンジングします" というメッセージが表示されます。 この例では、次のメッセージが表示されます。「ドメイン Address Verification は参照データ サービス プロバイダーを使用してクレンジングします。」  
