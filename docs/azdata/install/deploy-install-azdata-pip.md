...
> [!IMPORTANT]
> 新しいバージョンのビッグ データ クラスターをインストールする場合は、データをバックアップして以前のクラスターを削除し、`azdata` をアップグレードして新しいリリースをインストールします。 詳細については、[新しいリリースへのアップグレード](../../big-data-cluster/deployment-upgrade.md)に関するページを参照してください。
