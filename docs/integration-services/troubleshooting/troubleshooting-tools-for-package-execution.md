...
[!INCLUDE[ssis-appliesto](../../includes/ssis-appliesto-ssvrpluslinux-asdb-asdw-xxx.md)]
 ...
 [!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)] では、デザイン時に、パッケージの実行を一時停止するブレークポイント、進行状況ウィンドウ、およびデータ ビューアーが提供されます。 ブレークポイントを使用すると、データ フローを通過するときにデータを監視することができます。 しかし、これらの機能は、配置済みのパッケージを実行する場合は使用できません。 配置済みのパッケージのトラブルシューティングを行うための主な技法は次のとおりです。  
...
## <a name="catch-and-handle-package-errors-by-using-event-handlers"></a>イベント ハンドラーを使用したパッケージ エラーのキャッチおよび処理  
 イベント ハンドラーは、パッケージおよびパッケージ オブジェクトによって発生したイベントに対応するために使用します。  
  
-   **OnError イベントのイベント ハンドラーを作成する**。 イベント ハンドラーでは、メール送信タスクを使用して、管理者にエラーを通知することができます。 スクリプト タスクとカスタム ロジックを使用して、トラブルシューティングのためのシステム情報を取得したり、一時リソースや不完全な出力をクリーンアップしたりします。 詳細については、「[Integration Services &#40;SSIS&#41; のイベント ハンドラー](../../integration-services/integration-services-ssis-event-handlers.md)」を参照してください。  
...
-   **関連情報をエラー出力に追加する**。 エラー出力をより明確にするには、エラー出力によって提供される 2 つの数値識別子に加え、エラー メッセージと列名を追加します。 これら 2 つの列を追加する方法の例については、「 [スクリプト コンポーネントによるエラー出力の強化](../../integration-services/extending-packages-scripting-data-flow-script-component-examples/enhancing-an-error-output-with-the-script-component.md)」を参照してください。  
...
-   **または、DiagnosticEx イベントを記録して列名を取得します**。 このイベントは、データ フロー系列マップをログに書き込みます。 エラー出力でキャプチャされた列識別子を使用して、この系列マップの列名を調べることができます。  詳細については、「[データのエラー処理](../../integration-services/data-flow/error-handling-in-data.md)」を参照してください。  
     ...
     **DiagnosticEx** のメッセージ列の値は XML テキストです。 パッケージ実行のメッセージ テキストを表示するには、[catalog.operation_messages &#40;SSISDB データベース&#41;](../../integration-services/system-views/catalog-operation-messages-ssisdb-database.md) ビューのクエリを実行します。 **DiagnosticEx** イベントでは、ログのサイズを小さくするために XML 出力の空白が保持されないことに注意してください。 読みやすくするために、XML 書式と構文の強調表示をサポートする XML エディター (たとえば Visual Studio) にログをコピーします。  
...
## <a name="troubleshoot-package-execution-by-using-logging"></a>ログ記録を使ったパッケージ実行のトラブルシューティング  
 ログ記録を使用して、実行中のパッケージで発生する多くの内容を追跡できます。 ログ プロバイダーでは、指定されたイベントに関する情報を後で分析するためにキャプチャし、複数のサポートされている形式のうちのいずれかでその情報を保存します。  サポート ログ プロバイダーの形式には、データベース テーブル、フラット ファイル、XML ファイルなどがあります。  
...
-   **既定のログ出力を拡張する**。 ログ記録は、通常、パッケージを実行するたびに、ログの記録先に行を追加します。 各行のログ出力では名前や一意識別子でパッケージを識別していますが、一意の ExecutionID で各パッケージの実行も識別しています。そのため、1 つの一覧に大量のログ記録が出力され、分析が難しくなります。  
    ...
    3.  **行数データのキャプチャを検討する**。 行数情報用に別のテーブルを作成することを検討します。このテーブルでは、パッケージ実行の各インスタンスを ExecutionID で識別します。 行数変換を使用して、データ フロー内の重要な時点の行数を一連の変数に保存します。 後で分析やレポートを行うために、一連の値をテーブルの行に挿入する SQL 実行タスクを追加します。  
...
-   **既知の無効なパッケージ要素に DelayValidation プロパティを構成する**。 既知の構成の問題があるパッケージ要素の検証エラーを回避するには、**DelayValidation** を **True** に設定します。 たとえば、パッケージには、実行時に SQL 実行タスクでテーブルが作成されるまで存在しないターゲット テーブルを使用する、データ フロー タスクが含まれます。 **DelayValidation** プロパティはパッケージ ベル、またはパッケージに含まれている個別のタスクやコンテナーのレベルで有効にできます。  
...
## <a name="troubleshoot-errors-without-a-description"></a>説明のないエラーのトラブルシューティング  
 説明のない [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] エラーが発生した場合は、「[Integration Services のエラーおよびメッセージのリファレンス](../../integration-services/integration-services-error-and-message-reference.md) 」の説明を参照できます。 現在、この一覧にはトラブルシューティング情報は含まれていません。  
