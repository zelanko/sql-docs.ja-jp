---
title: SSIS パッケージ実行用のダンプ ファイルを生成する
description: '[エラー時にダンプする] オプションを使用して SQL Server Integration Services のトラブルシューティングを行う方法について説明します。 これらのオプションでは、.mdmp デバッグ ダンプ ファイルと、テキストの .tmp デバッグ ダンプファイルが生成されます。 デバッグ ダンプ ファイルの形式について説明します。'
ms.date: 08/24/2016
ms.prod: sql
