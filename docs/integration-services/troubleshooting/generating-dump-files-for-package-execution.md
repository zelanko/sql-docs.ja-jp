---
title: "パッケージの実行のダンプ ファイルを生成する |Microsoft ドキュメント"
ms.custom: 
ms.date: 08/24/2016
...
-   .tmp デバッグ ダンプ ファイル。 これはテキスト形式のファイルです。  
 ...
 既定では、[!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]フォルダーで、これらのファイルを格納*\<ドライブ >:*\Program Files\Microsoft SQL server \110\shared\errordumps です。  
