...
-   コマンド プロンプトで **setup.exe** を実行し、**/ACTION=upgrade** オプションを指定します。 詳細については、「 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]コマンド プロンプトからの SQL Server 2016 のインストール [」の「](../../database-engine/install-windows/install-sql-server-2016-from-the-command-prompt.md)のインストール スクリプト」を参照してください。  
