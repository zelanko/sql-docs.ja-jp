---
title: "手順 2: 追加して、Foreach ループ コンテナーの構成 |Microsoft ドキュメント"
ms.custom: 
ms.date: 03/14/2017
...
---
# <a name="lesson-2-2---adding-and-configuring-the-foreach-loop-container"></a>レッスン 2-2 を追加して、Foreach ループ コンテナーの構成
...
> [!NOTE]  
> [!INCLUDE[msCoName](../includes/msconame-md.md)][!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)] クリックすると制御フローとデータ フロー、制御フローに追加するループ処理は、データ フローに変更する必要ありません。 したがって、レッスン 1 で作成したデータ フローは変更する必要がありません。  
...
5.  **[フォルダーの参照]** ダイアログ ボックスで、Currency_*.txt ファイルが保存されている、コンピューター上のフォルダーに移動します。  
    ...
    3.  リンクをクリックして、 [SQL2012 です。Integration_Services.Create_Simple_ETL_Tutorial.Sample.zip](http://msftisprodsamples.codeplex.com/downloads/get/596031)ファイル。  
...
2.  **変数のマッピング**] ページの [、**変数**列は、空のセルをクリックし、選択**\<新しい変数… >**です。  
