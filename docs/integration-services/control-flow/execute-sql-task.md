---
title: "SQL 実行タスク |Microsoft ドキュメント"
ms.custom:
- ssisdev020617
ms.date: 03/13/2017
...
## <a name="connect-to-a-data-source"></a>データ ソースへの接続します。  
...
## <a name="create-sql-statements"></a>SQL ステートメントを作成します。  
...
## <a name="send-multiple-statements-in-a-batch"></a>バッチ内で複数のステートメントを送信します。  
...
## <a name="run-parameterized-sql-commands"></a>パラメーター化 SQL コマンドを実行します。  
...
## <a name="specify-a-result-set-type"></a>結果セットの種類を指定します。  
...
## <a name="configure-the-execute-sql-task"></a>SQL 実行タスクを構成します。  
...
## <a name="general-page---execute-sql-task-editor"></a>[全般] ページで SQL 実行タスク エディター
...
## <a name="parameter-mapping-page---execute-sql-task-editor"></a>パラメーター マッピング ページで SQL 実行タスク エディター
**[SQL 実行タスク エディター]** ダイアログ ボックスの **[パラメーター マッピング]** ページを使用すると、SQL ステートメント内のパラメーターに変数をマップできます。  
  
### <a name="options"></a>オプション  
 **[変数名]**  
 クリックしてパラメーター マッピングを追加した後に**追加**、一覧から、システムまたはユーザー定義変数を選択するかクリックして\<**新しい変数しています.**> を使用して新しい変数を追加する、**変数の追加** ダイアログ ボックス。  
 ...
 **[パラメーター名]**  
 ...
 タスクで使用される接続マネージャーの種類によって、数字またはパラメーター名を使用する必要があります。 いくつかの接続マネージャーの種類は、パラメーター名の最初の文字がである必要があります、@ 記号、固有の仕様の名前と同様に@Param1、またはパラメーター名として列名。  
 ...
 **[変数名]**  
 結果を変数またはをクリックして選択すると、変数にセット マップ\<**新しい変数しています.**> を使用して新しい変数を追加する、**変数の追加** ダイアログ ボックス。  
...
## <a name="parameters-in-the-execute-sql-task"></a>パラメーターで、SQL 実行タスク
...
-   [パラメーター名とパラメーター マーカーを使用します。](#Parameter_names_and_markers)  
  
-   [日付と時刻のデータ型でパラメーターの使用](#Date_and_time_data_types)  
  
-   [WHERE 句でのパラメーターの使用](#WHERE_clauses)  
  
-   [ストアド プロシージャでパラメーターの使用](#Stored_procedures)  
  
-   [リターン コードの値の取得](#Return_codes)    
  
###  <a name="Parameter_names_and_markers"></a>パラメーター名とパラメーター マーカー  
 SQL コマンドの構文では、SQL 実行タスクが使用する接続の種類によって、異なるパラメーター マーカーが使用されます。 たとえば、[!INCLUDE[vstecado](../../includes/vstecado-md.md)]接続マネージャーの種類は、SQL コマンドが形式でパラメーター マーカーを使用する必要があります **@varParameter** 、OLE DB 接続の種類には、疑問符 (?) パラメーター マーカーが必要です。  
...
|[!INCLUDE[vstecado](../../includes/vstecado-md.md)]|@\<パラメーター名 >|@\<パラメーター名 >|選択 Person.Contact から FirstName、LastName、タイトル、ContactID =@parmContactID|  
|ODBC|?|1, 2, 3, …|SELECT FirstName, LastName, Title FROM Person.Contact WHERE ContactID = ?|  
|EXCEL および OLE DB|?|0, 1, 2, 3, …|SELECT FirstName, LastName, Title FROM Person.Contact WHERE ContactID = ?|  
  
#### <a name="use-parameters-with-adonet-and-ado-connection-managers"></a>ADO.NET および ADO 接続マネージャーでパラメーターを使用します。  
 [!INCLUDE[vstecado](../../includes/vstecado-md.md)]おり ADO 接続マネージャーがパラメーターを使用する SQL コマンドの特定の要件があります。  
  
-   [!INCLUDE[vstecado](../../includes/vstecado-md.md)]接続マネージャーでは、SQL コマンドが、パラメーター マーカーとしてパラメーター名を使用する必要があります。 これは、変数をパラメーターに直接マップできることを意味します。 たとえば、変数 `@varName` は、 `@parName` という名前のパラメーターにマップされ、パラメーター `@parName`に変数を提供します。  
...
#### <a name="use-parameters-with-excel-odbc-and-ole-db-connection-managers"></a>パラメーターを使用して、EXCEL、ODBC、OLE DB 接続マネージャーと  
...
#### <a name="use-parameters-with-ole-db-connection-managers"></a>OLE DB 接続マネージャーでパラメーターを使用します。  
...
###  <a name="Date_and_time_data_types"></a>日付と時刻のデータ型でパラメーターを使用します。  
  
#### <a name="use-date-and-time-parameters-with-adonet-and-ado-connection-managers"></a>ADO.NET および ADO 接続マネージャーで日付と時刻のパラメーターを使用します。  
...
#### <a name="use-date-and-time-parameters-with-ole-db-connection-managers"></a>OLE DB 接続マネージャーで日付と時刻のパラメーターを使用します。  
...
#### <a name="use-date-and-time-parameters-with-odbc-connection-managers"></a>ODBC 接続マネージャーでの日付と時刻のパラメーターを使用します。  
...
###  <a name="WHERE_clauses"></a>WHERE 句でパラメーターを使用  
...
-   [!INCLUDE[vstecado](../../includes/vstecado-md.md)]接続の種類は、パラメーター名を使用して@parmMinProductIDと@parmMaxProductIDです。  
  
###  <a name="Stored_procedures"></a>ストアド プロシージャでパラメーターを使用します。  
...
### <a name="map-query-parameters-to-variables"></a>クエリ パラメーターを変数にマップします。
このセクションでは、SQL 実行タスクでパラメーター化された SQL ステートメントを使用し、SQL ステートメントで変数とパラメーター間のマッピングを作成する方法について説明します。  
...
##  <a name="Return_codes"></a>リターン コードの値を取得します。  
...
###  <a name="Result_set_type"></a>結果セットの種類を指定します。  
...
###  <a name="Populate_variable_with_result_set"></a>結果セットによる変数を設定します。  
 ...
 単一の値を返す一部のクエリには、列名が含まれないことがあります。 たとえば、ステートメント `SELECT COUNT (*) FROM Production.Product` からは列名が返されません。 返される結果にアクセスするには、結果名として序数位置 0 を使用します。 列名で返される結果にアクセスするには、クエリが、AS を含める必要があります\<エイリアス名 > 句に列名を指定します。 ステートメント `SELECT COUNT (*)AS CountOfProduct FROM Production.Product`では、 **CountOfProduct** 列を指定します。 その後、 **CountOfProduct** 列名または序数位置 0 を使用して、返された結果列にアクセスできます。  
...
## <a name="troubleshoot-the-execute-sql-task"></a>SQL 実行タスクをトラブルシューティングします。  
