---
title: "For ループ コンテナーの |Microsoft ドキュメント"
ms.custom: 
ms.date: 03/14/2017
...
-   ループ カウンターを増減する、省略可能な初期化式。  
 ...
 ![For ループ コンテナーがタスクを 4 回繰り返されます](../../integration-services/control-flow/media/ssis-forloop.gif "A For ループ コンテナーがタスクを 4 回繰り返されます")  
  
 この式は、有効な [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] の式である必要があります。  
  
 初期化式および代入式を作成するには、代入演算子 (=) を使用します。 この演算子は、Integration Services の式文法以外ではサポートされておらず、For ループ コンテナーの初期化式および代入式の種類によってのみ、使用できます。 代入演算子を使用する任意の式の構文が必要`@Var = <expression>`ここで、 **Var**ランタイム変数と\<式 > の規則に従う式です、[!INCLUDE[ssIS](../../includes/ssis-md.md)]式構文があります。 この式には、変数、リテラル、および、SSIS の式文法でサポートされるあらゆる演算子と関数を含めることができます。 この式は、変数のデータ型にキャスト可能なデータ型に評価される必要があります。  
...
## <a name="add-iteration-to-a-control-flow-with-the-for-loop-container"></a>イテレーションを For ループ コンテナーと制御フローに追加します。
  [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]For ループ コンテナーに含まれるを簡単にループを条件付きでは、制御フロー要素は、パッケージの制御フローを繰り返します。 詳細については、「 [For ループ コンテナー](../../integration-services/control-flow/for-loop-container.md)に評価されるまでそのワークフローを繰り返します。  
 ...
 For ループ コンテナーには、複数のタスクを持つ制御フローを含めたり、他のコンテナーを含めることができます。 For ループ コンテナーにタスクとコンテナーを追加する手順は、タスクとコンテナーをドラッグする先がパッケージではなく For ループ コンテナーであること以外は、パッケージに追加する手順と同様です。 For ループ コンテナーに複数のタスクまたはコンテナーが含まれる場合、パッケージの場合と同様に、優先順位制約を使用してそれらを連結できます。 優先順位制約の詳細については、「 [優先順位制約](../../integration-services/control-flow/precedence-constraints.md)」を参照してください。  
  
## <a name="add-a-for-loop-container-in-a-control-flow"></a>制御フローの For ループ コンテナーを追加します。  
...
##  <a name="configure-the-for-loop-container"></a>For ループ コンテナーを構成します。
...
## <a name="use-expressions-with-the-for-loop-container"></a>For ループ コンテナーで式を使用します。  
 ...
 式の内部で変数を指定する場合、変数名をアット マーク (@) で始める必要があります。 たとえば、という名前の変数**カウンター**、入力@CounterFor ループ コンテナーを使用する式。 変数の名前空間のプロパティを含める場合は、変数と名前空間を角かっこで囲む必要があります。 たとえば、**カウンター**に変数が、 **MyNamespace**名前空間、型 [@MyNamespace::Counter] です。  
