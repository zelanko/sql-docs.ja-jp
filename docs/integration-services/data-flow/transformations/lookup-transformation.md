...
helpviewer_keywords: 
  - "参照変換"
  - "列の結合 [Integration Services]"
  - "キャッシュ [Integration Services]"
  - "完全に一致 [Integration Services]"
  - "参照 [Integration Services]"
  - "完全一致 [Integration Services]"
...
-   参照データセット内に一致するエントリがない場合、結合は行われません。 既定では、参照変換で一致するエントリがない行は、エラーとして扱われます。 ただし、それらの行を不一致出力にリダイレクトするように参照変換を構成することもできます。 詳細については、「[[参照変換エディター] &#40;[全般] ページ&#41;](../Topic/Lookup%20Transformation%20Editor%20\(General%20Page\).md)」および「[[参照変換エディター] &#40;[エラー出力] ページ&#41;](../Topic/Lookup%20Transformation%20Editor%20\(Error%20Output%20Page\).md)」をご覧ください。  
