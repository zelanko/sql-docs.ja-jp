...
|最小値|グループ内の最小値を返します。 詳細については、「[MIN (Transact-SQL)](../../../t-sql/functions/min-transact-sql.md)」を参照してください。 Transact-SQL MIN 関数とは異なり、この操作は数値、日付、および時間データ型に対してのみ使用できます。|  
|最大値|グループ内の最大値を返します。 詳細については、「[MAX (Transact-SQL)](../../../t-sql/functions/max-transact-sql.md)」を参照してください。 Transact-SQL MAX 関数とは異なり、この操作は数値、日付、および時間データ型に対してのみ使用できます。|  
