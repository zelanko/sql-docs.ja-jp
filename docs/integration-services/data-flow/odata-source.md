...
> [!NOTE]  
>  OData ソースを使用して、SharePoint リストから読み取りを行うこともできます。 SharePoint サーバーのすべてのリストを表示するには、次の URL を使用します: http://\<server>/_vti_bin/ListData.svc。 SharePoint の URL の規則に関する詳細については、「 [SharePoint Foundation REST インターフェイス](http://msdn.microsoft.com/library/ff521587.aspx)」を参照してください。  OData ソースでは、Microsoft Dynamics AX Online および Microsoft Dynamics CRM Online 製品がサポートされるようになりました。
...
-   [[OData ソース エディター] &#40;[接続] ページ&#41;](../Topic/OData%20Source%20Editor%20\(Connection%20Page\).md)  
