...
[!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)] の他のチュートリアルでは、**[デバッグ]** メニューの **[デバッグ開始]** オプションを使用して、[!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)] の配置環境である [!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)] でパッケージを実行しました。 今度は別の方法でパッケージを実行します。  
...
### SQL Server Management Studio で Integration Services に接続するには  
...
### パッケージ実行ユーティリティを使用してパッケージを実行するには  
