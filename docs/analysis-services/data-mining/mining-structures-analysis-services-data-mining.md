...
helpviewer_keywords: 
  - "属性 [データ マイニング]"
  - "マイニング構造 [Analysis Services], マイニング構造の概要"
  - "マイニング構造 [Analysis Services]"
  - "データ マイニング [Analysis Services], 構造"
  - "Analysis Services オブジェクト, データ マイニング オブジェクト"
  - "データ マイニング [Analysis Services], モデル"
  - "アルゴリズム [データ マイニング]"
  - "データ マイニング [Analysis Services], オブジェクト"
  - "マイニング モデル [Analysis Services]"
  - "マイニング モデル [Analysis Services], データ マイニング モデルの概要"
...
### マイニング構造の表示  
 ...
 `SELECT * FROM <model>.CASES` というステートメントでは同じ列が返されますが、特定のモデルのケースのみです。 詳細については、「[SELECT FROM &#60;structure&#62;.CASES](../Topic/SELECT%20FROM%20%3Cstructure%3E.CASES.md)」および「[SELECT FROM &#60;model&#62;.CASES (DMX)](../Topic/SELECT%20FROM%20%3Cmodel%3E.CASES%20\(DMX\).md)」を参照してください。  
...
|OLAP キューブに基づくマイニング構造の操作|[新規の OLAP マイニング構造の作成](../../analysis-services/data-mining/create-a-new-olap-mining-structure.md)<br /><br /> [マイニング構造のソース キューブのフィルター選択](../Topic/Filter%20the%20Source%20Cube%20for%20a%20Mining%20Structure.md)|  
