...
###  <a name="bkmk_Query1"></a> サンプル クエリ 1: データ マイニング スキーマ行セットを使用してモデル パラメーターを取得する  
 ...
 次のクエリでは、 `[Sequence Clustering]`モデルの作成とトレーニングに使用されたパラメーターが返されます。 このモデルは、「 [Basic Data Mining Tutorial](../Topic/Basic%20Data%20Mining%20Tutorial.md)」のレッスン 5 で作成できます。  
...
```  
 ...
 詳細については、「[SELECT FROM &#60;model&#62;.SAMPLE_CASES (DMX)](../Topic/SELECT%20FROM%20%3Cmodel%3E.SAMPLE_CASES%20\(DMX\).md)」を参照してください。  
