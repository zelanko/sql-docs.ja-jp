...
ms.topic: "article"
helpviewer_keywords: 
  - "データ マイニング [Analysis Services], 入れ子になったテーブル"
  - "テーブル [Analysis Services], 入れ子になった"
  - "nested tables"
...
# 入れ子になったテーブル (Analysis Services - データ マイニング)
 ...
 ![入れ子になったテーブルを使用して結合された 2 つのテーブル](../../analysis-services/data-mining/media/nested-tables.gif "入れ子になったテーブルを使用して結合された 2 つのテーブル")  
...
## マイニング モデルでの入れ子になったテーブル列の使用  
 ケース テーブルのキーは多くの場合、顧客 ID、製品名、連続する日付など、テーブルの行を一意に識別するデータです。 」を参照してください。 しかし、入れ子になったテーブルのキーは、そのリレーショナル キー (外部キー) ではなく、モデル化する属性を表す列であるのが一般的です。  
