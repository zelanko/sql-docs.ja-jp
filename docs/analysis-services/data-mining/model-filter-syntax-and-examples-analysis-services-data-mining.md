...
# モデル フィルターの構文と例 (Analysis Services - データ マイニング)
 ...
 [ケース属性に基づくフィルター](#bkmk_Ex1)  
  
 [入れ子になったテーブルの属性に基づくフィルター](#bkmk_Ex2)  
  
 [入れ子になったテーブルの複数の属性に基づくフィルター](#bkmk_Ex3)  
  
 [入れ子になったテーブルの属性が存在しないことに基づくフィルター処理](#bkmk_Ex4)  
  
 [入れ子になったテーブルの複数の値に基づくフィルター処理](#bkmk_Ex5)  
  
 [入れ子になったテーブルの属性および EXISTS に基づくフィルター処理](#bkmk_Ex6)  
  
 [フィルターの組み合わせ](#bkmk_Ex7)  
  
 [日付に基づくフィルター処理](#bkmk_Ex8)  
...
> [!NOTE]  
>  モデルに列を追加すると、結果に影響を及ぼす場合があることに注意してください。 そのため、系列の計算に列を使用しない場合は、マイニング構造のみに列を追加し、モデルには追加しないでください。 列のモデル フラグを **PredictOnly** または **Ignore** に設定することもできます。 詳細については、「[モデリング フラグ &#40;データ マイニング&#41;](../../analysis-services/data-mining/modeling-flags-data-mining.md)」を参照してください。  
