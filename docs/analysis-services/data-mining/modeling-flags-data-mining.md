 ...
 **MODEL_EXISTENCE_ONLY**  
 列が 2 つの状態を持つものとして扱われますことを示します。**不足している**と**既存**します。 値が **NULL**の場合は Missing として扱われます。 MODEL_EXISTENCE_ONLY フラグは、予測可能な属性に適用され、ほとんどのアルゴリズムでサポートされます。  
  
 実際には、MODEL_EXISTENCE_ONLY フラグを設定**True**が 2 つの状態が存在するよう、値の形式が変わります。**不足している**と**既存**します。 Missing に該当しない状態はすべて単一の **Existing** 値に統合されます。  
