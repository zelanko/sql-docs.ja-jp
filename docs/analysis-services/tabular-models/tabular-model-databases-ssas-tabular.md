...
[!INCLUDE[ssas-appliesto-sqlas-aas](../../includes/ssas-appliesto-sqlas-aas.md)]テーブル モデル データベースは、Analysis Services データベースで作成した[!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)]Analysis Services 表形式モデル プロジェクト テンプレートを使用して、テーブルとリレーションシップがデータを表すため、主な構造です。  
