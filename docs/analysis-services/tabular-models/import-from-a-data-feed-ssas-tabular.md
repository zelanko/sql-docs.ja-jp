---
title: "データ フィードからのインポート (SSAS テーブル) | Microsoft Docs"
ms.custom: ""
ms.date: "12/09/2016"
 ...
 **Atom フィード**  
 ...
 インポート時に、データ フィードからのデータがモデルに 1 回追加されます。 フィードから更新済みデータを取得するには、モデル デザイナーからデータを更新するか、Analysis Services の実稼動インスタンスに配置後にモデルのデータ更新スケジュールを構成するかします。 詳細については、「[データの処理 (SSAS テーブル)](../../analysis-services/tabular-models/process-data-ssas-tabular.md)」を参照してください。  
...
#### パブリックまたは企業データ フィードからデータをインポートするには  
...
4.  [データ フィード URL] で、リスト データ サービスへのアドレスを入力し、\<server-name> を実際の SharePoint サーバーの名前に置き換えます。  
