...
## "行識別子 (ROWID)" プロパティの設定  
 テーブル内の行識別子 (ROWID) は、一意の値のみ含み、空白値を含まない単一列を指定します。 "行識別子 (ROWID)" プロパティはグループ化の変更に使用されるため、グループは行のフィールド構成に基づかず、特定のレポート レイアウトで使用されるフィールドにかかわらず行を一意に識別するために常に使用される固定列に基づきます。  
...
-   マトリックス レポートからの不要な小計の削除。 フィールド レベルでの既定のグループ化によって、各フィールドの小計が作成されます。 行レベルで計算された単一の小計のみ必要な場合は、行識別子 (ROWID) を設定するとこの結果が生成されます。  
 ...
 日付テーブルとしてマークされているテーブルの行識別子 (ROWID) は設定できません。 日付テーブルの場合、行識別子 (ROWID) はテーブルをマークするときに指定します。 詳細については、「[[日付テーブルとしてマーク] ダイアログ ボックス (SSAS)](../Topic/Mark%20as%20Date%20Table%20Dialog%20Box%20\(SSAS\).md)」を参照してください。  
...
### 画像がない場合  
 ...
 ![画像 URL がテキストとしてレポート内に表示される](../../analysis-services/tabular-models/media/ssas-rptprop-noimageurl.gif "画像 URL がテキストとしてレポート内に表示される")  
  
 既定では、モデル内のテキストはレポートでテキストとして解釈されます。 テキスト列がレポート画像への URL アドレスの場合は、[!INCLUDE[ssCrescent](../../includes/sscrescent-md.md)] によって画像ファイルが取得されるように **"画像の URL"** プロパティを忘れずに設定してください。 バイナリの画像の場合は、**"行識別子 (ROWID)"** プロパティを忘れずに設定してください。  
...
### テーブルに 1 つ以上の行が不足している場合  
 ...
 ![既定グループにより 2 つから 1 つに統合](../../analysis-services/tabular-models/media/ssas-jonyang-norowid.gif "既定グループにより 2 つから 1 つに統合")  
 ...
 ![行 ID に基づく重複が格納された行データ](../../analysis-services/tabular-models/media/ssas-jonyang.gif "行 ID に基づく重複が格納された行データ")  
 ...
 **設定前: マトリックス内のフィールドに基づく既定のグループ化**  
 ...
 ![マトリックス レイアウト、行識別子 (ROWID) でのグループ化](../../analysis-services/tabular-models/media/ssas-rptprop-matrixrowid.gif "マトリックス レイアウト、行識別子 (ROWID) でのグループ化")  
 ...
 **設定前: グラフ内のフィールドに基づく既定のグループ化**  
 ...
 ![フィールド レベルの既定のグループ化に基づくグラフ](../../analysis-services/tabular-models/media/ssas-rptprop-chartfieldgroup.gif "フィールド レベルの既定のグループ化に基づくグラフ")  
  
 **設定後: 行識別子 (ROWID) でのグループ化 (行識別子 (ROWID) は軸になります)**  
  
 ![行 ID グループ化に基づくグラフ](../../analysis-services/tabular-models/media/ssas-rptprop-chartrowid.gif "行 ID グループ化に基づくグラフ")  
