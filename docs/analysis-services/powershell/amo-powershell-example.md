---
title: "AMO PowerShell の例 | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
---
# AMO PowerShell の例
  以下の例は、Analysis Services 管理オブジェクト (AMO) 型を呼び出して、SQL Server リレーショナル データベースに基づく表形式データベース (互換性レベル 1200) を作成する PowerShell サンプル スクリプトです。  
  
 このサンプルは、デモンストレーションのみを目的としています。 テーブルのリレーションシップは定義されておらず、データのインポートは含まれません。  このため、スクリプトは実行可能なコードというよりも、スニペット形式となっています。 スクリプトを実行するには、それらの処理を行う他のヘルパー関数やその他のコマンドが必要です。  
  
## DatabaseGenerate.ps1 サンプル  
...
## 参照  
 ...
 [Analysis Services PowerShell リファレンス](../../analysis-services/powershell/analysis-services-powershell-reference.md)   
 [Analysis Services 管理オブジェクト (AMO)](../Topic/Analysis%20Services%20Management%20Objects%20\(AMO\).md)   
