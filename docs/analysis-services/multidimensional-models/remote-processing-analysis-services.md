...
## SQL Server エージェント サービスを使用したリモート処理のスケジュール設定  
...
-   **sysadmin** 権限を付与することができない場合、[SQL Server エージェントのコンポーネント](../../ssms/agent/sql-server-agent.md#Components)は代替の固定サーバー ロールを提示します。  
  
 アカウントの権限を構成したら、以下の手順を続行します。  
  
#### SSAS に対する管理者権限を SQL Server エージェント アカウントに付与する  
...
## 参照  
 [SQL Server エージェントのコンポーネント](../../ssms/agent/sql-server-agent.md#Components)   
