...
[!INCLUDE[ssas-appliesto-sqlas](../../../includes/ssas-appliesto-sqlas.md)]場合は、名前付きセットは、のみ 1 つの多次元式 (MDX) クエリに必要なを定義できます WITH キーワードを使用して名前付きセットです。 WITH キーワードを使用して作成した名前付きセットは、そのクエリの実行が終了した時点で存在しなくなります。  
