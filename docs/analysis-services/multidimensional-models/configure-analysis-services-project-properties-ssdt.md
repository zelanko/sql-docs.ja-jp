...
helpviewer_keywords: 
  - "プロジェクト [Analysis Services], プロパティ"
...
|配置 / 対象サーバー|localhost|既定では、[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] プロジェクト内のデータベース オブジェクトは、[!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)] を使用しているローカル コンピューター上の [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] の既定のインスタンスに配置されます。 ローカル コンピューター上の特定のインスタンスを指定するか、[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] のオブジェクトを作成する権限のあるリモート コンピューター上の任意のインスタンスを指定するには、この既定を変更します。|  
