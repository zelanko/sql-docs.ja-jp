...
helpviewer_keywords: 
  - "ユーザー アクセス権 [Analysis Services], キューブ"
  - "キューブ [Analysis Services], セキュリティ"
  - "読み取り/書き込み権限"
  - "権限 [Analysis Services], キューブ"
...
2.  次の手順に進み、キューブ権限を設定します。  
     ...
     データ ソース ペインをスキップしていることに注意してください。 Analysis Services データの通常のコンシューマーのほとんどには、データ ソース オブジェクトに対する権限は必要ありません。 この権限を設定する場合の詳細については、「[データ ソース オブジェクトに対する権限の付与 (Analysis Services)](../../analysis-services/multidimensional-models/grant-permissions-on-a-data-source-object-analysis-services.md)」をご覧ください。  
