...
---
# SharePoint 2013 への Power Pivot の移行
...
-   **コンテンツ データベースをマウントする:**  
     ...
     移行したコンテンツ データベースをマウントするには、SharePoint 2013 管理シェルで PowerShell コマンドレットを使用します。 マウントする必要があるのはコンテンツ データベースのみで、サービス アプリケーション データベースをマウントする必要はありません。 ![PowerShell 関連コンテンツ](../../../analysis-services/instances/install-windows/media/rs-powershellicon.png "PowerShell 関連コンテンツ")  
...
-   **スケジュールを移行するオプション 1: SharePoint ファームの管理者**  
    ...
    1.  SharePoint 2013 の管理作業で、`Set-PowerPivotServiceApplication` スイッチを指定して `-StartMigratingRefreshSchedules` コマンドレットを実行し、要求に応じて自動実行されるスケジュールの移行を有効にします![PowerShell 関連コンテンツ](../../../analysis-services/instances/install-windows/media/rs-powershellicon.png "PowerShell 関連コンテンツ")。 次の Windows PowerShell スクリプトは、 [!INCLUDE[ssGemini](../../../includes/ssgemini-md.md)] サービス アプリケーションが 1 つだけであることを前提としています。  
