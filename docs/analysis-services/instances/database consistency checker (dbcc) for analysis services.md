---
title: "Analysis Services の表形式および多次元データベース用 Database Consistency Checker (DBCC) | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/04/2017"
...
manager: "erikre"
---
# Analysis Services の表形式および多次元データベース用 Database Consistency Checker (DBCC)
...
-   互換性レベル 1200 の新しい表形式モデル データベースのメタデータは、 **TableName** や **PartitionName**のような記述で構成されます。  
...
## 表形式の 1200 データベースのコマンド構文  
 1200 互換性レベルの表形式データベースは、オブジェクト定義に表形式メタデータを使用します。 次の例は、SQL Server 2016 機能レベルで作成した表形式データベースの完全な DBCC 構文です。  
  
 2 つの構文の主な違いは、新しい XMLA 名前空間、\<Object> 要素がない、\<Model> 要素がない (データベースごとに 1 つのモデルのみがある) などの点です。  
...
### Management Studio で DBCC コマンドを実行する  
...
### SQL Server Profiler 2016 で DBCC の出力をトレースする  
...
4.  SQL Server Profiler で、DBCC アクティビティは、DBCC のイベント サブクラスを持つ **Command** イベントで示されます。  
     ...
     ステータス メッセージは、"Checking consistency of \<object>"、"Started checking \<object>"、または "Finished checking \<object>" から始まります。  
  
    > [!NOTE]  
    >  CTP 3.0 では、オブジェクトは内部名で識別されます。 たとえば、Categories 階層は、H$Categories-\<objectID> と表現されます。 内部名は、今後の CTP でユーザー フレンドリ名で置き換えられます。  
  
     エラー メッセージを以下に示します。  
  
### SSMS の xEvent セッションで DBCC 出力をトレースする  
...
|Table|テーブルが Dimension テーブルか Fact テーブルかを決定するために使用される内部値をチェックします。  既知の範囲を外れる値は、破損していることを示します。|テーブル統計のチェック中に、データベース整合性チェック (DBCC) が失敗しました。|  
|Table|テーブルのセグメント マップ内のパーティション数が、テーブルに定義されているパーティション数と同じかどうかをチェックします。|格納層が破損しています。 '%{parent/}' テーブルのパーティションのコレクションが破損しています。|  
|Table|Excel 2010 用 PowerPivot で作成またはインポートした表形式データベースで、パーティション数が 2 以上の場合、エラーが発生します。パーティションのサポートが以降のバージョンで追加され、それが破損を示すためです。|セグメント マップのチェック中に、データベース整合性チェック (DBCC) が失敗しました。|  
...
|階層|階層の並べ替え順序が認識可能な値ではない場合、エラーが発生します。|'%{hier/}' 階層のチェック中に、データベース整合性チェック (DBCC) が失敗しました。|  
|階層|階層に対して実行されるチェックは、使用する階層マッピング スキームの内部型に応じて変わります。<br /><br /> すべての階層について、処理済みの状態が正しいこと、データ ID から階層位置への変換に使用される階層ストアが存在することがチェックされます。<br /><br /> これらすべてのチェックに合格すると、階層構造全体につい、階層内の各位置が正しいメンバーを指していることを検証します。<br />いずれかのテストに失敗すると、エラーが発生します。|'%{hier/}' 階層のチェック中に、データベース整合性チェック (DBCC) が失敗しました。|  
...
|DBCC コマンド|DBCC コマンドは、DBCC 処理の進行中に複数のステータス メッセージをレポートします。  オブジェクトのデータベース、テーブル、または列の名前を含める処理を開始する前、各オブジェクトのチェックが完了した後に、ステータス メッセージがレポートされます。|\<objectname> \<objecttype> の整合性をチェックします。 フェーズ: 確認前処理。<br /><br /> \<objectname> \<objecttype> の整合性をチェックします。 フェーズ: 確認後処理。|  
...
|**メタデータ マネージャーのエラー**<br /><br /> ID が '\<objectID>' の \<object> が ID が '\<parentobjectID>' の \<parentobject> に存在しないか、ユーザーがオブジェクトにアクセスするアクセス許可を持っていません。|インデックスの破損 (多次元)|オブジェクトと依存するすべてのオブジェクトを再処理します。|  
|**パーティションの整合性チェック中に発生したエラー**<br /><br /> \<database-name> データベースの \<cube-name> キューブについて、\<measure-group-name> メジャー グループの \<partition-name> パーティションの整合性をチェックしているときに、エラーが発生しました。 破損を修復するには、パーティションまたはインデックスを再処理してください。|インデックスの破損 (多次元)|オブジェクトと依存するすべてのオブジェクトを再処理します。|  
...
|**テーブル メタデータの破損**<br /><br /> テーブル \<table-name> メタデータ ファイルが破損しています。 メイン テーブルが DataFileList ノードに見つかりません。|メタデータの破損 (表形式のみ)|プロジェクトを削除してから再配置するか、バックアップから復元して再処理します。<br /><br /> 手順については、ブログの投稿「[How to handle corruption in Analysis Services database](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)」(Analysis Services データベースで破損を処理する方法) を参照してください。|  
|**格納層の破損**<br /><br /> 格納層の破損: \<parent-name> \<parent-type> の \<type-name> のコレクションが破損しています。|メタデータの破損 (表形式のみ)|プロジェクトを削除してから再配置するか、バックアップから復元して再処理します。<br /><br /> 手順については、ブログの投稿「[How to handle corruption in Analysis Services database](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)」(Analysis Services データベースで破損を処理する方法) を参照してください。|  
|**システム テーブルが見つからない**<br /><br /> システム テーブル \<table-name> が見つかりません。|オブジェクトの破損 (表形式のみ)|オブジェクトと依存するすべてのオブジェクトを再処理します。|  
|**テーブルの統計情報の破損**<br /><br /> システム テーブル \<table-name> の統計情報が見つかりません。|メタデータの破損 (表形式のみ)|プロジェクトを削除してから再配置するか、バックアップから復元して再処理します。<br /><br /> 手順については、ブログの投稿「[How to handle corruption in Analysis Services database](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)」(Analysis Services データベースで破損を処理する方法) を参照してください。|  
  
## msmdsrv.ini 構成ファイルによって、データベースの読み込み操作時に自動整合性チェックを無効にします。  
