...
|**ContextualNameRule**|Enum|プロパティ名をそれが使用されるコンテキストに基づいて変更する必要があるかどうか、および変更する方法を示す値。 設定可能な値は、次のとおりです。**None**、**ロール**、**マージ**します。|  
|**アラインメント**|Enum|表形式の表示でフィールドの値を配置する方法を示す値。 指定できる値は**既定**、 **Center**、**左**、**右**します。 省略した場合、既定値は、フィールドのデータ型に基づいて配置を決定します。|  
...
|**Width**|整数型|表形式の表示でフィールドの値を表示するために予約する文字の幅。 省略した場合、既定の幅は、フィールドのデータ型に基づきます。|  
