...
ms.topic: "article"
helpviewer_keywords: 
  - "プロセス ノード [SQL Server Profiler]"
  - "プロファイラー [SQL Server Profiler], デッドロック"
  - "デッドロック [SQL Server], 原因の特定"
  - "リソース ノード [SQL Server Profiler]"
  - "グラフ [SQL Server Profiler]"
  - "SQL Server Profiler, デッドロック"
  - "イベント [SQL Server], デッドロック"
  - "エッジ [SQL Server Profiler]"
...
# SQL Server Profiler を使用したデッドロックの分析
 ...
 デッドロック イベントをトレースするには、**Deadlock Graph** イベント クラスをトレースに追加します。 このイベント クラスにより、デッドロックに関連するプロセスとオブジェクトに関する XML データが、トレースの **TextData** データ列に設定されます。 [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] では、この XML ドキュメントをデッドロック XML (.xdl) ファイルに抽出して、後で SQL Server Management Studio で表示できます。 **Deadlock Graph** イベントを、すべての **Deadlock Graph** イベントが含まれた 1 つのファイルに抽出するか、または個別のファイルに抽出するように [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] を構成できます。 この抽出は、次の方法のいずれかを使用して実行できます。  
