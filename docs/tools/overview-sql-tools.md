...
(クエリ、モニターなど) を管理するツールを必要とするデータベース。 いくつかのデータベース ツールを使用できますがあります。 Windows では、または、クラウド内に、データベースを実行していることができます、 [Linux](../linux/sql-server-linux-overview.md)ツールで、データベースと同じプラットフォーム上で実行する必要はありません。 

この記事では、SQL データベースを操作するために使用できるツールに関する情報を提供します。 
...
|[mssql-cli](mssql-cli.md)|mssql cli は、SQL Server を照会するための対話的なコマンド ライン ツールです。 **mssql cli は、Windows、macOS、および Linux で実行されます。**|
...
- SQL Server インスタンスまたはデータベースで、Windows、Linux または Mac 上の軽量なエディターで管理しますか? 選択します。 [[!INCLUDE[name-sos](../includes/name-sos.md)]](../sql-operations-studio/download.md)
...
- 機能の IntelliSense、構文の高い照明をコマンド ライン ツールを使用して SQL Server のクエリを実行するとしますか? 選択[mssql cli](mssql-cli.md)
...
|[RS.exe ユーティリティ &#40;です。SSRS &#41;](../reporting-services/tools/rs-exe-utility-ssrs.md)|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] レポート サーバーを管理するために作成されたスクリプトを実行します。|[!INCLUDE[ssInstallPathVar](../includes/ssinstallpathvar-md.md)]Tools\Binn|  
|[rsconfig ユーティリティ &#40;です。SSRS &#41;](../reporting-services/tools/rsconfig-utility-ssrs.md)|レポート サーバー接続を構成します。|[!INCLUDE[ssInstallPathVar](../includes/ssinstallpathvar-md.md)]Tools\Binn|  
