...
- SQL Server インスタンスまたはデータベースで、Windows、Linux または Mac 上の軽量なエディターで管理しますか? [[!INCLUDE[name-sos](../includes/name-sos.md)]](../sql-operations-studio/download.md) を選択します。
- SQL Server インスタンスまたは完全な GUI サポートと Windows 上のデータベースを管理しますか? [SQL Server Management Studio (SSMS)](../ssms/download-sql-server-management-studio-ssms.md) を選択します。
- 作成またはコンパイル時の検証を含む、データベースのコードを保守する、Windows 上でのリファクタリングおよびデザイナー サポートしますか。 [SQL Server Data Tools (SSDT)](../ssdt/download-sql-server-data-tools-ssdt.md) を選択します。
