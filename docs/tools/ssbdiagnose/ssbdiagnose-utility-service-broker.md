...
ms.topic: "article"
helpviewer_keywords: 
  - "Service Broker, ランタイム レポート"
  - "Service Broker, コマンド プロンプト ユーティリティ"
  - "トラブルシューティング [Service Broker], メッセージ交換"
  - "トラブルシューティング [Service Broker], 構成"
  - "コマンド プロンプト ユーティリティ [Service Broker]"
  - "Service Broker, トラブルシューティング"
  - "Service Broker, 構成レポート"
  - "Service Broker, ツール"
  - "トラブルシューティング [Service Broker], ランタイム"
  - "メッセージ交換 [Service Broker], トラブルシューティング"
  - "トラブルシューティング [Service Broker], ssbdiagnose ユーティリティ"
  - "ツール [Service Broker], ssbdiagnose"
  - "Service Broker, ssbdiagnose ユーティリティ"
  - "ssbdiagnose"
 ...
 **イベント**  
 **RUNTIME** レポートの実行中に監視されるメッセージ交換で問題が検出されたことを示す [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] イベントを報告します。 **ssbdiagnose** は、イベントが生成されるたびにイベントを報告します。 複数のメッセージ交換で問題が検出される場合は、イベントが複数回報告されます。  
  
 **問題**  
 **ssbdiagnose** で構成分析を完了したり、メッセージ交換を監視したりすることができない場合に、その原因となる問題を報告します。  
  
## sqlcmd 環境変数  
 **ssbdiagnose** ユーティリティでは、**sqlcmd** ユーティリティでも使用されている SQLCMDSERVER、SQLCMDUSER、SQLCMDPASSWORD、および SQLCMDLOGINTIMOUT の各環境変数がサポートされています。 これらの環境変数を設定するには、コマンド プロンプトの SET コマンドを使用するか、**sqlcmd** を使用して実行する [!INCLUDE[tsql](../../includes/tsql-md.md)] スクリプトの **setvar** コマンドを使用します。 **sqlcmd** で **setvar** を使用する方法の詳細については、「[sqlcmd でのスクリプト変数の使用](../../relational-databases/scripting/use-sqlcmd-with-scripting-variables.md)」をご覧ください。  
