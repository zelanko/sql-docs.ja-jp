...
- **Off**:手段**拡張保護**は無効です。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] のインスタンスは、クライアントが保護されているかどうかに関係なく、任意のクライアントからの接続を許可します。 **[オフ]** は、古いオペレーティング システムや修正プログラムの適用が解除されたオペレーティング システムと互換性がありますが、安全性は低くなります。 この設定は、クライアント オペレーティング システムで拡張保護がサポートされていないことがわかっている場合にのみ使用してください。
...
- **必須**: 許可される接続の場合、必要があります保護されたアプリケーションから保護されているオペレーティング システムのことを意味します。 この設定は最も安全な 3 つのオプションです。 サポートしないオペレーティング システムからの接続が、**拡張保護**に接続することはできません[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]します。
