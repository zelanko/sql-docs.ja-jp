...
### <a name="a-using-general-operators"></a>A. 一般的な演算子の使用  
 ...
 このクエリでは**=** 、比較演算子を使用します。 **=** 演算子の右側にあるシーケンスの各ノードは、左側にあるシーケンスの各ノードと比較されます。 ノードが一致する場合、ノードの比較は**TRUE**になります。 その後、int に変換され、1と比較され、クエリから顧客 ID が返されます。  
 ...
 **Eq**演算子のオペランドは両方ともアトミック値であるため、値演算子がクエリで使用されます。 一般的な比較演算子 ( **=** ) を使用して、同じクエリを記述できます。  
