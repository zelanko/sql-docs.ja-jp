...
---
# [サーバーのプロパティ] ([履歴] ページ)
  [!INCLUDE[ssManStudioFull_md](../../includes/ssmanstudiofull-md.md)]この [!INCLUDE[ssRSnoversion_md](../../includes/ssrsnoversion-md.md)] ページを使用すると、保持されるレポート履歴のコピー数の既定値を設定できます。 既定値には、すべてのレポートのレポート履歴の制限を規定する初期設定が用意されています。 これらの設定は、レポートごとに変えることができます。  
