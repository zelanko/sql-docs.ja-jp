...
ms.topic: "article"
helpviewer_keywords: 
  - "レポート サーバー [Reporting Services], ネットワーク負荷分散"
...
---
# ネットワーク負荷分散クラスターにおけるレポート サーバーの構成
...
|手順|Description|詳細情報|  
|----------|-----------------|----------------------|  
|1|NLB クラスター内のサーバー ノードに Reporting Services をインストールする前に、スケールアウト配置の要件を確認します。|[スケールアウト配置 - Reporting Services のネイティブ モード (構成マネージャー)](../Topic/Scale-out%20Deployment%20%20-%20Reporting%20Services%20Native%20mode%20\(Configuration%20Manager\).md) ([!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] オンライン ブック)|  
...
3.  **UrlRoot** を見つけます。 この要素は構成ファイルには指定されていませんが、使用される既定値は http:// または https://\<*computername*>/\<*reportserver*> という形式の URL です。\<*reportserver*> はレポート サーバー Web サービスの仮想ディレクトリ名です。  
...
##  <a name="Verify"></a> レポート サーバーへのアクセスの確認  
 ...
 レポート サーバーのログ ファイルを参照するか、または RS の実行ログを確認して、どのノードが実際にレポートを処理しているのかを確認できます (実行ログ テーブルには、**InstanceName** という列があり、特定の要求がどのインスタンスによって処理されたのかを示しています)。 詳細については、[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] オンライン ブックの「[Reporting Services のログ ファイルとソース](../../reporting-services/report-server/reporting-services-log-files-and-sources.md)」を参照してください。  
