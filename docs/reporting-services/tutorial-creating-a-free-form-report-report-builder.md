...
4.  接続の種類が Microsoft SQL Server であることを確認し、**接続文字列**ボックスに「:**データ ソース =\<サーバー名 >**  
  
    **\<サーバー名 >**、report001 の例は、SQL Server データベース エンジンのインスタンスがインストールされているコンピューターを指定します。 このレポートのデータは SQL Server のデータベースから抽出されるのではないので、データベース名を含める必要はありません。 指定したサーバー上の既定のデータベースを使用し、クエリが解析されます。  
...
6.  行グループ ペインで右クリックし、**詳細**グループ >**グループの削除**です。  
...
2.  **挿入**タブ >**テキスト ボックス**です。 先に追加した四角形の内側で、一覧の左上隅をクリックし、ドラッグして幅が約 3.45 インチ、高さが約 5 インチのテキスト ボックスを作成します。  
...
22. 選択、`[Sales]`フィールドに、フィールドを右クリックして >**式**です。  
...
29. `[Sum(Sales)]`にオンのまま、**ホーム**タブ >**数**グループ >**通貨**です。  
...
1.  **挿入**タブ >**データ領域**領域 >**テーブル** > **テーブル ウィザード**です。  
...
12. [SalesDate] フィールドを選択します。 **ホーム**タブ >**数**、変更**既定**に**日付**です。

13. [Sum(Sales)] フィールドを選択します。 **ホーム**タブ >**数**、変更**既定**に**通貨**です。
...
1.  **挿入**タブ >**データの視覚エフェクト** > **グラフ** > **グラフ ウィザード**です。  
...
12. **ホーム**タブ >**フォント**、タイトルします。
    * **[フォント]** **Segoe UI Semibold**」を参照してください。
    * **サイズ** **12 pt**」を参照してください。
    * **[色]** **黒**」を参照してください。  

13. 凡例を右クリックして >**凡例のプロパティ**です。
...
2.  選択した両方のグラフを右クリックして >**レイアウト** > **ように同じ幅**です。  
