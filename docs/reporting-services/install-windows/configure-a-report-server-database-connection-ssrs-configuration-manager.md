...
-   レポート サーバー スケールアウト配置を構成する。 スケールアウト配置を構成するには、レポート サーバー データベースへの複数の接続を作成する必要があります。 この複数段階に操作を実行する方法の詳細については、次を参照してください[ネイティブ モード レポート サーバー スケール アウト配置 &#40; を構成します。。SSRS 構成マネージャー &#41;](../../reporting-services/install-windows/configure-a-native-mode-report-server-scale-out-deployment.md).  
