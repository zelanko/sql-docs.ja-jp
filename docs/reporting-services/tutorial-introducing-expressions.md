...
13. **[カテゴリ]** ボックスの一覧で **[日付と時刻]** をもう一度クリックし、**[アイテム]** ボックスの一覧で **[Now]** をダブルクリックします。  
    ...
    > [!WARNING]  
    > 運用環境のレポートでは、レポートのレンダリングごとに何度も評価される式に **Now** 関数を使用しないでください (レポートの詳細行内など)。 **Now** の値が行ごとに変わり、それが式の評価に影響して、微妙に一貫性に欠ける結果を招きます。 これを回避するには、[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] で提供されている `ExecutionTime` グローバル変数を使用してください。  
...
22. `Parameters!RowColor.Value` のすぐ後に「**, “White”)**」を入力します。  
    ...
    完成した式は、次のようになります。 `=IIF(RowNumber(Nothing) MOD 2, Parameters!RowColor.Value, “White”)`  
