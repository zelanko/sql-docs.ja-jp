...
manager: "erikre"
---
# RowNumber 関数 (レポート ビルダーおよび SSRS)
...
## 解説  
 **RunningValue** では、集計関数の実行中の値が返されますが、 [RowNumber](../../reporting-services/report-design/runningvalue-function-report-builder-and-ssrs.md) では、指定したスコープ内の行数の実行中の値が返されます。 スコープを指定すると、行数を 1 にリセットするタイミングが指定されます。  
