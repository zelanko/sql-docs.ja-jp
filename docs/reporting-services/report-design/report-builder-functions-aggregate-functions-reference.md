...
|[Max (最大値)](../../reporting-services/report-design/report-builder-functions-max-function.md)|指定されたスコープのコンテキストで、式で指定された NULL 以外のすべての数値の中から最大値を返します。 グラフの軸の最大値を指定してスケールを制御する場合は、この関数を使用できます。|  
|[Min (最小値)](../../reporting-services/report-design/report-builder-functions-min-function.md)|指定されたスコープのコンテキストで、式で指定された NULL 以外のすべての数値の中から最小値を返します。 グラフの軸の最小値を指定してスケールを制御する場合は、この関数を使用できます。|  
...
|[Union (結合)](../../reporting-services/report-design/report-builder-functions-union-function.md)|式で指定された NULL 以外のすべての空間データの値 ( **SqlGeometry** 型または **SqlGeography** 型) の和集合を、指定されたスコープで評価して返します。|  
