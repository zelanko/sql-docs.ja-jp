...
description: レポート ビルダーでユーザーがマップのデータの視覚化を解釈できるように、マップの凡例を変更する方法について説明します。
ms.date: 03/07/2017
...
1.  デザイン ビューで凡例を右クリックし、 _[\<report item>_ **のプロパティ]** ページを開きます。  
  
2.  **[位置]** で、ビューポートを基準とする凡例の相対的な表示位置をクリックします。  
  
3.  **[ビューポートの外に\<report item>を表示する]** を選択し、凡例をビューポートの外に表示します。  
...
2.  必要なデータが存在するレイヤーを右クリックし、 _[\<map element type>_ **の色ルール]** をクリックします。  
