---
title: "式 (レポート ビルダーおよび SSRS) |Microsoft ドキュメント"
ms.custom: 
ms.date: 09/06/2016
...
-   **複合** 複合式には、複数の組み込み参照、演算子、および関数呼び出しへの参照が含まれます。 複雑な式が表示されます <\<Expr >> 式の値が単純な参照が複数含まれる場合。 式を表示するには、その上にカーソルを合わせ、ツールヒントを使用します。 式を編集するには、 **[式]** ダイアログ ボックスでそれを開きます。  
