...
-   [!INCLUDE[vstecasp](../../includes/vstecasp-md.md)] で Windows 認証を構成する必要があります。 既定では、レポート サーバー Web サービスの Web.config ファイルが含まれて、\<認証モード ="Windows"> 設定です。 変更した場合\<認証モード ="Forms"> の Windows 認証[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]は失敗します。  
