---
title: "レポート サーバー Web サービスのエンドポイント |Microsoft ドキュメント"
ms.custom: 
ms.date: 03/03/2017
...
> [!IMPORTANT]  
>  レポート サーバーが SharePoint 統合モードで構成されている場合、 <xref:ReportService2005> Api が返されます、 **rsOperationNotSupportedSharePointMode**エラーです。 レポート サーバーがネイティブ モードで構成されている場合、 <xref:ReportService2006> Api が返されます、 **rsOperationNotSupportedNativeMode**エラーです。 同様に、<xref:ReportService2010> のモード固有 API を不適切なモードで使用した場合も、それぞれエラーが返されます。  
...
```  
 ...
 詳細については、次を参照してください。 [SOAP API にアクセスする](../../../reporting-services/report-server-web-service/accessing-the-soap-api.md)です。  
...
```  
 ...
 詳細についてを参照してください[SOAP API にアクセスする](../../../reporting-services/report-server-web-service/accessing-the-soap-api.md)です。  
...
|--------------------|-----------------|  
|<xref:ReportService2006>|SharePoint 統合モード用に構成されたレポート サーバーを管理するための API を提供します。<br /><br /> 注: このエンドポイントは非推奨で[!INCLUDE[ssKilimanjaro](../../../includes/sskilimanjaro-md.md)]です。|  
