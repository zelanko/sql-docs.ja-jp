---
title: "チュートリアル: テキストの書式設定 (レポート ビルダー) | Microsoft Docs"
ms.custom: ""
ms.date: "06/15/2016"
...
### レポートを設定する  
1. [空のレポートとデータ ソースおよびデータセットを作成する](#CreateReport)  
  
2. [レポート デザイン画面にフィールドを追加する (正しくない方法と正しい方法)](#AddField)  
  
3. [レポート デザイン画面にテーブルを追加する](#AddTable)  
  
### 選択する  
* [レポートにハイパーリンクを追加する](#AddHyperlink)  
  
* [レポート内のテキストを回転する](#RotateText)  
  
* [通貨の書式設定](#FormatCurrency)  
  
* [HTML 形式のテキストを表示する](#FormatHTML)  
...
### レポートにフィールドを追加する (うまくいかない方法を試す) には  
...
9. **[LinkText]** 列の **[合計]** セルを選択し、Shift キーを押しながらこのセルの右にある 2 つのセル (**[Product]** 列の [合計] セルと **[Sales]** 列の [`[Sum(Sales)]`] セル) を選択します。  
...
## 次の手順  
レポート ビルダーでは、さまざまな方法でテキストの書式を設定できます。 詳細な例については、「[チュートリアル: 自由形式のレポートの作成(レポート ビルダー)](../reporting-services/tutorial-creating-a-free-form-report-report-builder.md)」を参照してください。  
