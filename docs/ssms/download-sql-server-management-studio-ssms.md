...
SSMS 18.x のインストールでは、17.x 以前のバージョンの SSMS がアップグレードまたは置き換えられることはありません。 SSMS 18.x は以前のバージョンとは別にサイド バイ サイドでインストールされるので、両方のバージョンを使用できます。 ただし、"***プレビュー***" 版の SSMS 18.x がインストールされている場合は、それを**アンインストール**してから SSMS 18.4 をインストールしてください。 *プレビュー*版がインストールされているかどうかを確認するには、[ヘルプ]、[バージョン情報] の順に移動します。
