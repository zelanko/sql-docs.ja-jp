---
title: SSMS のリリース ノート | Microsoft Docs
ms.custom: ''
ms.date: 03/30/2019
ms.prod: sql
...
この記事では、SSMS の現在と以前のバージョンの更新、機能強化、およびバグの修正に関する詳細を提供します。 [こちらをクリックして、以前のバージョンの SSMS をダウンロード](#previous-ssms-releases)できます。
...
ダウンロード:&nbsp; &nbsp; [SSMS 18.0 (RC1) のダウンロード](download-sql-server-management-studio-ssms.md)<br/>
ビルド番号:&nbsp; &nbsp; 15.0.18098.0<br/>
リリース日:&nbsp; &nbsp; 2019 年 3 月 28 日

このセクションでは、SSMS 18.0 RC1 の新機能を挙げます。

リリース候補 1 (RC1) は、SSMS 18.0 のパブリック プレビューです。 バージョン 18 の一般提供 (GA) リリースまでは、バージョン 17.\* が GA としてリリースされた最新のバージョンです。

> [!NOTE]
> SSMS 17.9.1 以降の完全な変更ログについては、[SSMS 18.0 プレビュー - RC1 までの累積的な変更ログ](#180-preview---cumulative-changelog-through-rc1)に関するページを参照してください。
...
| **SMO:**&nbsp; &nbsp; データ分類の "_読み書き_" のアクセス許可のサポートを追加しました。 | &nbsp; |
| **監査ファイル:**&nbsp; &nbsp; 既知の監査アクションの一覧を更新しました。 | 一覧に FEATURE RESTRICTION ADD/CHANGE GROUP/DROP が含まれるようになりました。 |
| &nbsp; | &nbsp; |
...
| バグの修正 | 詳細 |
| :------ | :------ |
| **SSMS 全般:**&nbsp; &nbsp; ユーザー識別子が複数のテナントに属していた場合に多要素認証 (MFA) を妨げていた問題を修正しました。 | &nbsp; |
| **SSMS 全般:**&nbsp; &nbsp; パフォーマンス ダッシュボード レポートで、サブレポートにはない PAGELATCH および PAGEIOLATCH による待機がレポートされる問題を修正しました。 | &nbsp; |
| **SSMS エディター:**&nbsp; &nbsp; いくつかのシステム ビューおよびテーブル値関数が正しく色付けされない問題を修正しました。 | &nbsp; |
| **オブジェクト エクスプローラー:** オブジェクト エクスプローラーのフィルター処理に適切なエスケープを追加しました。 | [Azure フィードバック 36678803](https://feedback.azure.com/forums/908035/suggestions/36678803) に関するページをご覧ください。 |
| **SMO:**&nbsp; &nbsp; Azure のすべてのデータベースで `GetDbComparer` が不正に CI への照合順序を既定としていた問題を修正しました。 | &nbsp; |
| **Managed Instance のサポート:**&nbsp; &nbsp; Managed Instance サーバー固有のプロパティの表示を改善しました。 | これらのプロパティには、ハードウェアの世代、サービス層、および使用、予約の両方の対象となるストレージが含まれます。 |
| **Azure SQL Database:**&nbsp; &nbsp; SSMS が新しい SQL Azure SLO をサポートするのを困難にしていた、サービス レベルの目標 (SLO) のハードコードの問題を修正しました。 | これでユーザーは Azure にサインインし、SSMS が該当するすべての SLO データを取得できるようになりました。<br/>該当する SLO データ項目はエディションと最大サイズです)。 |
| **結果グリッド:**&nbsp; &nbsp; グリッドをクリックすると "_インデックスが範囲外です_" となる問題を修正しました。 | &nbsp; |
| **結果グリッド:**&nbsp; &nbsp; グリッド結果の背景色が無視されていた問題を修正しました。 | [Azure フィードバック 32895916](https://feedback.azure.com/forums/908035/suggestions/32895916) に関するページをご覧ください。 |
| **Profiler:**&nbsp; &nbsp; Windows 7 SP1 で SQL Profiler が起動できない問題を修正しました。 | &nbsp; |
| **プラン表示:**&nbsp; &nbsp; 実際の実行プラン xml の `RunTimeCountersPerThread` に、4 つの属性を追加しました。 | &bull; &nbsp; &nbsp; `HpcRowCount` - hpc デバイスで処理された行数。<br/><br/>&bull; &nbsp; &nbsp; `HpcKernelElapsedUs` - 使用中のカーネルの実行の待機の経過時間。<br/><br/>&bull; &nbsp; &nbsp; `HpcHostToDeviceBytes` - ホストからデバイスに転送されたバイト数。<br/><br/>&bull; &nbsp; &nbsp; `HpcDeviceToHostBytes` - デバイスからホストに転送されたバイト数。 |
| **データ層アプリケーションのウィザード:**&nbsp; &nbsp; サーバーへのアクセス制限のために、ユーザーがデータ層アプリケーション (.dacpac) をインポートできなかった問題を修正しました。 | たとえば、ユーザーが同じサーバー上のすべてのデータベースにアクセスできなかった場合、.dacpac ファイルをインポートできませんでした。 |
| **データ層アプリケーションのウィザード:**&nbsp; &nbsp; 多数のデータベースがたまたま同じ Azure SQL サーバー上でホストされていた場合に、インポートが非常に遅くなる原因となっていた問題を修正しました。 | &nbsp; |
| &nbsp; | &nbsp; |
...
## <a name="180-preview---cumulative-changelog-through-rc1"></a>18.0 プレビュー - RC1 までの累積的な変更ログ

*プレビュー 5*、*プレビュー 6*、または*プレビュー 7* または *RC1* ラベルがない場合は、SSMS 18.0 の最初のパブリック プレビュー (SSMS 18.0 *プレビュー 4*) での変更を示しています。
...
- **SSMS**
  ...
  - SSMS GUI から、危険な 'lightweight pooling' および 'priority boost' オプションを削除しました (詳細については次のページを参照してください。 https://blogs.msdn.microsoft.com/arvindsh/2010/01/26/priority-boost-details-and-why-its-not-recommended/)
...
- **SMO**
  ...
  - (RC1 の新機能) SMO と SSMS の両方に "エッジ制約" の連鎖削除のサポートを追加しました。
  - (RC1 の新機能) データ分類の "読み書き" のアクセス許可のサポートを追加しました。
...
- **監査ファイル**
  ...
  - (RC1 の新機能) 既知の監査アクションの一覧に FEATURE RESTRICTION ADD/CHANGE GROUP/DROP を含め更新しました。
...
- **SSMS 全般**
  ...
  - (RC1 の新機能) ユーザー ID が複数のテナントのものであった場合に多要素認証ができなかった問題を修正しました。
  - (RC1 の新機能) パフォーマンス ダッシュボード レポートに、サブレポートにはない PAGELATCH および PAGEIOLATCH による待機がレポートされる問題を修正しました。
...
- **オブジェクト エクスプローラー**
  ...
  - (RC1 の新機能) オブジェクト エクスプローラーのフィルター処理に適切なエスケープを追加しました。 詳細については、[Azure フィードバック 36678803](https://feedback.azure.com/forums/908035/suggestions/36678803) のページを参照してください。
...
- **SMO**
  ...
  - (RC1 の新機能) SMO の Azure データベースが、文字列のすべての比較で、データベースに指定された照合順序を使用する代わりに、大文字と小文字が区別される照合順序を既定としていた問題を修正しました。
...
- **Azure SQL データベース**
  ...
  - (RC1 の新機能) SSMS が新しい SQL Azure SLO をサポートするのを困難にしていた、サービス レベルの目標 (SLO) のハードコードの問題を修正しました。 これでユーザーは Azure にサインインし、SSMS が該当するすべての SLO データ (エディションおよび最大サイズ) を取得できるようにできます。
...
- **結果グリッド**
  ...
  - (RC1 の新機能) グリッドをクリックすると "インデックスが範囲外です" となる問題を修正しました。
  - (RC1 の新機能) グリッド結果の背景色が無視される問題を修正しました。 詳細については、[Azure フィードバック 32895916](https://feedback.azure.com/forums/908035/suggestions/32895916) のページを参照してください。
...
- **データ層アプリケーションのウィザード**
  - (RC1 の新機能) サーバーへのアクセス制限のために、ユーザーがデータ層アプリケーション (.dacpac) をインポートできない問題を修正しました (例: 同じサーバーにおいてすべてのデータベースへのアクセスがない)。
  - (RC1 の新機能) 多数のデータベースが同じ Azure の SQL server にホストされている場合に、インポートが非常に遅くなる問題を修正しました。
...
- (RC1 の新機能) 静的データ マスク (プレビュー) の機能を削除しました。
