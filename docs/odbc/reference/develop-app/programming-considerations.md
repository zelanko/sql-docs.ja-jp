---
title: "プログラミングに関する考慮事項 |Microsoft ドキュメント"
ms.custom: 
ms.date: 01/19/2017
ms.prod: sql-non-specified
ms.prod_service: drivers
ms.service: 
ms.component: reference
ms.reviewer: 
ms.suite: sql
ms.technology:
- drivers
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 6a1b7b7e-5d08-4df8-aa4a-688da7da1b30
caps.latest.revision: 9
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.workload: Inactive
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: ce7fa26983200ba8857c44e6d085410e337e8df8
ms.contentlocale: ja-jp
ms.lasthandoff: 09/09/2017

---
# <a name="programming-considerations"></a>プログラミングの考慮事項
このセクションでは、ODBC アプリケーションおよびドライバーの書き込みに関連するトピックの数について簡単に説明します。  
  
 このセクションでは、次のトピックを扱います。  
  
-   [マルチ スレッド](../../../odbc/reference/develop-app/multithreading.md)  
  
-   [配置](../../../odbc/reference/develop-app/alignment.md)  
  
-   [Unicode](../../../odbc/reference/develop-app/unicode.md)  
  
-   [翻訳の Dll](../../../odbc/reference/develop-app/translation-dlls.md)  
  
-   [診断ツール](../../../odbc/reference/develop-app/diagnostic-tools.md)  
  
-   [Visual Studio Analyzer](../../../odbc/reference/develop-app/visual-studio-analyzer.md)  
  
-   [ドライバー固有のデータ型、記述子の種類、情報の種類、診断型、および属性](../../../odbc/reference/develop-app/driver-specific-data-types-descriptor-information-diagnostic.md)  
  
-   [旧バージョンとの互換性と標準への準拠](../../../odbc/reference/develop-app/backward-compatibility-and-standards-compliance.md)  
  
-   [Windows で ODBC](../../../odbc/reference/develop-app/odbc-in-windows.md)

