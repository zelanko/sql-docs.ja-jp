---
title: "SQL ステートメント |Microsoft ドキュメント"
ms.custom: 
ms.date: 01/19/2017
ms.prod: sql-non-specified
ms.reviewer: 
ms.suite: 
ms.technology:
- drivers
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 78133ee0-1e2f-4dc8-81f6-3bac8eb21332
caps.latest.revision: 5
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.workload: Inactive
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: 4ff92186a7523904cf9db8f1b5a8ac1814384354
ms.contentlocale: ja-jp
ms.lasthandoff: 09/09/2017

---
# <a name="sql-statements"></a>SQL ステートメント
ODBC アプリケーションでは、SQL ステートメントを実行することによってほとんどすべてのデータベースへのアクセスを実行します。 これらのステートメントの形式:、ハードコーディングまたは実行時に、相互運用可能なデータやデータ ソースの特定などに構築された、アプリケーションのニーズによって異なります。  
  
 このセクションでは、次のトピックを扱います。  
  
-   [SQL ステートメントの作成](../../../odbc/reference/develop-app/constructing-sql-statements.md)  
  
-   [SQL ステートメントの相互運用性](../../../odbc/reference/develop-app/interoperability-of-sql-statements.md)  
  
-   [Odbc エスケープ シーケンス](../../../odbc/reference/develop-app/escape-sequences-in-odbc.md)

