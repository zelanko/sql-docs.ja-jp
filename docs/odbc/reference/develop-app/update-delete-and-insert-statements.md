...
SQL ベースのアプリケーションでは、テーブルに変更を加えるを実行して、 **UPDATE**、**DELETE**、および**INSERT**ステートメント。 これらのステートメントでは、Minimum SQL 文法の準拠レベルの一部であるし、すべてのドライバーとデータ ソースでサポートする必要があります。  
 ...
 **INSERT INTO** _テーブル名_[ **(** _列識別子_[ **、** _列識別子_]... **)** ]  
 ...
 などの他の SQL ステートメント**UPDATE**、**DELETE**と**INSERT**ステートメントは、多くの場合は詳細はパラメーターを使用するときに効率的です。 たとえば、次のステートメントを準備および Orders テーブルに複数の行を挿入するために繰り返し実行します。  
