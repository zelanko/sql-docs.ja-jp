...
-   位置指定更新と削除ステートメントは、次の場合にのみ使用できます: ときに、**SELECT**ステートメントには、結果セットが生成されたときに、**SELECT**ステートメントに、結合が含まれていない、 **UNION**句、または**GROUP BY**句; ときに、選択リストで、別名または式を使用するすべての列にバインドされていないと**SQLBindCol**します。  
