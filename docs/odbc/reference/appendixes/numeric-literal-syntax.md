---
title: "数値リテラルの構文 |Microsoft ドキュメント"
ms.custom: 
ms.date: 01/19/2017
ms.prod: sql-non-specified
ms.reviewer: 
ms.suite: 
ms.technology:
- drivers
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- ODBC literals [ODBC], numeric
- numeric literals [ODBC]
- literals [ODBC], numeric
ms.assetid: fb17498d-4f1d-4b3d-b33d-1e62c7d3c32d
caps.latest.revision: 5
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: 64a772d8a04015060aa717e4153bf6f36e5b74de
ms.contentlocale: ja-jp
ms.lasthandoff: 09/09/2017

---
# <a name="numeric-literal-syntax"></a>数値リテラルの構文
次の構文は、ODBC での数値リテラルに使用します。  
  
 *数値リテラル*:: =*符号付き数値リテラル (&) #124; 符号なし数値リテラル*  
  
 *符号付き数値リテラル*:: = [*記号*]*符号なし数値リテラル*  
  
 *符号なし数値リテラル*:: =*正確な数値リテラル (&) #124; おおよその数値リテラル*  
  
 *正確な数値リテラル*:: =*符号なし整数*[*期間*[*符号なし整数*] *(& a) #124; 期間符号なし整数*  
  
 *サインオン*:: =*プラス記号 (&) #124; マイナス記号*  
  
 *おおよその数値リテラル*:: =*仮数 E の指数部*  
  
 *仮数*:: =*正確な数値リテラル*  
  
 *指数部*:: =*符号付き整数*  
  
 *符号付き整数*:: = [*記号*]*符号なし整数*  
  
 *符号なし整数*:: =*桁しています.*  
  
 *プラス記号*:: =*+*  
  
 *マイナス記号*:: = -  
  
 *桁*:: 1 (&) #124; 2 &#124; = 3 &#124; 4 &#124; 5 &#124; 6 &#124; 7 (&) #124; 8 &#124; 9 &#124; 0  
  
 *期間*:: = です。
