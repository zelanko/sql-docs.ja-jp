...
|特権 (ODBC 1.0)|6|NULL 以外の Varchar|テーブルの特権。 または、次のデータ ソース固有の権限の 1 つがあります。<br /><br /> 選択: 権限付与対象ユーザーは、テーブルの 1 つまたは複数の列のデータを取得を許可します。<br /><br /> 挿入します。 テーブルに 1 つまたは複数の列のデータを含む行を挿入する、権限付与対象ユーザーが許可されます。<br /><br /> 更新: テーブルの 1 つまたは複数の列のデータを更新する、権限付与対象ユーザーが許可されます。<br /><br /> 削除: テーブルから行のデータを削除する権限が許可されます。<br /><br /> 参照: 制約内のテーブルの 1 つまたは複数の列を参照する、権限付与対象ユーザーが許可されている (たとえば、一意な参照、またはテーブルが check 制約)。<br /><br /> 特定のテーブル特権によって、権限付与対象ユーザーを許可されているアクションのスコープは、データ ソースによって異なります。 たとえば、UPDATE 特権は、1 つのデータ ソースではテーブルのすべての列と、権限の許可者が別のデータ ソースに対する UPDATE 特権を持つ対象の列のみを更新する権限付与対象ユーザーを許可する可能性があります。|  
