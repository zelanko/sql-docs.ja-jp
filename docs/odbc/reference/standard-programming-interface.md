...
ms.locfileid: "47622560"
---
# <a name="standard-programming-interface"></a>標準のプログラミング インターフェイス
プログラミング インターフェイスが標準化のための最も明らかな候補ではおそらくです。 実際には、ODBC の開発中、時に ANSI および ISO 既に提供されている標準 embedded SQL および SQL モジュール。 データベース CLI, SQL アクセス グループの基準が存在しないが、データベース ベンダーに業界コンソーシアム —; を作成するかどうかを考えていたODBC の後の部分では、作業の基礎となりました。  
