...
-   SQL_FILE_QUALIFIER-1 層ドライバーは、データソース内のファイルを修飾子として扱います。  
 ...
 ODBC ドライバーは、各ファイルが完全なデータベースであるため SQL_FILE_QUALIFIER を返します。  
  
## <a name="sql_bookmark_persistence"></a>SQL_BOOKMARK_PERSISTENCE  
 SQL_BP_SCROLL &#124; SQL_BP_UPDATE [1]  
  
 [1] の場合、ブックマークはコミット後も保持されますが、ロールバック後は保持されません。  
...
> [!NOTE]  
>  バージョン1.0 および1.1 はサポートされていません。 また、Microsoft Access バージョン3.0、7.0、および97では、データ形式に違いはありません。  
