---
title: SQLGetInfo (Visual FoxPro ODBC ドライバー) |Microsoft ドキュメント
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
...
>  このトピックには、Visual FoxPro ODBC ドライバー固有の情報が含まれています。 この関数の概要については、下の該当するトピックを参照してください。 [ODBC API リファレンス](../../odbc/reference/syntax/odbc-api-reference.md)です。  
 ...
 Visual FoxPro ODBC ドライバーと、接続ハンドルに関連付けられているデータ ソースに関する全般情報を返します*hdbc*です。 次の一覧は、各 Visual FoxPro ODBC ドライバーによって返される値を示します*fInfoType*引数と戻り値に関するコメントです。  
  
 詳細については、次を参照してください。 [SQLGetInfo](../../odbc/reference/syntax/sqlgetinfo-function.md)で、 *ODBC プログラマ リファレンス*です。  
  
## <a name="a"></a>A  
 SQL_ACCESSIBLE_PROCEDURES 返します 'N' です。  
  
 SQL_ACCESSIBLE_TABLES では、'Y' を返します。  
...
## <a name="c"></a>C  
 SQL_COLUMN_ALIAS では、'Y' を返します。  
  
 SQL_CONCAT_NULL_BEHAVIOR SQL_CB_ を返します。  
  
 SQL_CONVERT_BIGINT では、0 を返します。 Visual FoxPro ODBC ドライバーがサポートしていません*BigInt*です。  
...
## <a name="d"></a>D  
 SQL_DATA_SOURCE_NAME を DSN として渡された値が返されます[SQLConnect](../../odbc/microsoft/sqlconnect-visual-foxpro-odbc-driver.md)、または[SQLDriverConnect](../../odbc/microsoft/sqldriverconnect-visual-foxpro-odbc-driver.md); DSN が指定されていない場合は、空の文字列を返します。  
  
 SQL_DATA_SOURCE_READ_ONLY 返します 'N' です。  
  
 SQL_DATABASE_NAME は、データ ソースがある場合、現在のデータベースを完全な UNC パスを返します、[データベース](../../odbc/microsoft/visual-foxpro-terminology.md)です。 データ ソースのディレクトリに接続する場合[テーブル](../../odbc/microsoft/visual-foxpro-terminology.md)ディレクトリへのパスを返します。  
  
 SQL_DBMS_NAME では、"Visual FoxPro"を返します。  
  
 SQL_DBMS_VER では、「03.00.0000」を返します。  
 ...
 SQL_DRIVER_NAME では、"vfpodbc.dll"を返します。  
  
 SQL_DRIVER_ODBC_VER では、「02.50」(SQL_SPEC_MAJOR、SQL_SPEC_MINOR) を返します。  
  
 SQL_DRIVER_VER では、「01.00.0000」を返します。  
  
## <a name="e"></a>E  
 SQL_EXPRESSIONS_IN_ORDERBY 返します 'N' です。  
  
## <a name="f"></a>F  
 SQL_FETCH_DIRECTION を返します。  
...
-   SQL_FD_FETCH_BOOKMARK です。  
 ...
 SQL_FILE_USAGE を使用して、データベース (.dbc ファイル) の両方の SQL_FILE_QUALIFIER を返します、無料テーブル (.dbf ファイル) のデータ ソース。  
  
## <a name="g-h"></a>G H  
 SQL_GETDATA_EXENSIONS を返します。  
...
## <a name="i-j"></a>I J  
 ...
 SQL_IDENTIFIER_QUOTE_CHAR 返します ' です。  
...
## <a name="m"></a>M  
 ...
 SQL_MAX_CHAR_LITERAL_LEN は 254 を返します。  
 ...
 SQL_MAX_COLUMNS_IN_SELECT は 254 を返します。  
  
 SQL_MAX_COLUMNS_IN_TABLE は 254 を返します。  
  
 SQL_MAX_CURSOR_NAME_LEN は 254 を返します。  
  
 SQL_MAX_INDEX_SIZE では、0 を返します。  
  
 SQL_MAX_OWNER_NAME_LEN では、0 を返します。  
  
 SQL_MAX_PROCEDURE_NAME_LEN では、0 を返します。 Visual FoxPro ODBC ドライバーでは、Visual FoxPro がストアド プロシージャに直接アクセスをできません。  
  
 SQL_MAX_QUALIFIER_NAME_LEN では、最新のオペレーティング システムのパスの長さを返します。  
  
 SQL_MAX_ROW_SIZE 返します 254 ^2 です。  
 ...
 SQL_MULT_RESULT_SETS では、'Y' を返します。  
  
 SQL_MULTIPLE_ACTIVE_TXN では、'Y' を返します。 複数の接続には、複数のトランザクションが、一度に開くことができます。  
...
## <a name="n"></a>×  
 ...
 SQL_NUMERIC_FUNCTIONS では、Visual FoxPro ODBC ドライバーでサポートされていない SQL_FN_NUM_POWER を除くすべての関数を返します。 次の関数がサポートされています。  
...
## <a name="o"></a>O  
 ...
 SQL_ODBC_SQL_CONFORMANCE を返します以上でなければなりません。 最小 SQL 構文がサポートされています。  
...
## <a name="q"></a>Q  
 ...
 SQL_QUALIFIER_NAME_SEPARATOR 返します '!' または '\\' です。 データベースとテーブル間の区切り記号は、'!' のデータ ソースに接続されている[データベース](../../odbc/microsoft/visual-foxpro-terminology.md)、および '\\' のディレクトリにあるデータ ソースの[テーブルの空き](../../odbc/microsoft/visual-foxpro-terminology.md)です。  
  
 SQL_QUALIFIER_TERM は、"database"または"directory"を返します。 修飾子は、「データベース」データ ソースに接続[データベース](../../odbc/microsoft/visual-foxpro-terminology.md)、および「ディレクトリ」のディレクトリにあるデータ ソースの[テーブルの空き](../../odbc/microsoft/visual-foxpro-terminology.md)です。  
...
## <a name="r"></a>R  
 SQL_ROW_UPDATES"N"を返します。 Visual FoxPro ODBC ドライバーには静的カーソルと順方向カーソルがサポートしています。  
...
## <a name="s"></a>S  
 ...
 SQL_SEARCH_PATTERN_ESCAPE 返します"\\"です。  
  
 SQL_SERVER_NAME 返します""です。  
  
 SQL_SPECIAL_CHARACTERS 返します"~ @# $% ^"です。  
...
-   SQL_SQ_QUANTIFIED です。  
  
 SQL_SYSTEM_FUNCTIONS を返します。  
...
## <a name="t"></a>T  
 ...
 SQL_TIMEDATE_ADD_INTERVALS を返します。  
...
-   SQL_FN_TSI_QUARTER  
 ...
 SQL_TIMEDATE_DIFF_INTERVALS を返します。  
...
## <a name="u-z"></a>U Z  
 SQL_U_UNION、または、SQL_U_UNION_ALL SQL_UNION を返します。  
