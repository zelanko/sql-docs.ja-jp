...
##  <a name="Createaknowledgebase"></a> ナレッジ ベースの作成  
...
4.  **[次の場所からナレッジ ベースを作成]**で、ナレッジ ベースを作成する際に基にするナレッジ ベースを選択します。  
    ...
    -   新しいナレッジ ベースを作成する際に、 **で既に作成されているナレッジ ベースまたは既定のナレッジ ベースを基にする場合は、** [既存のナレッジ ベース] [!INCLUDE[ssDQSServer](../includes/ssdqsserver-md.md)]を選択します。 ナレッジ ベースを選択、 **サポート技術情報の選択** ドロップダウン リスト、またはクリック **参照** を表示する、 **ナレッジ ベースを選択** ダイアログ ボックスで、既存のナレッジ ベースを基本新しいナレッジ ベースを選択して、をクリックして **[ok]**します。 ナレッジ ベースをタブレットから選択する場合は、ナレッジ ベースのドメインと照合ルールがダイアログ ボックスの右側のペインに表示されます。 選択することも、 **DQS データ** 共通の既定のドメインと米国の会社、アドレス、およびパーティのデータに関連するナレッジを含む既定のナレッジ ベースはナレッジ ベースです。  
...
##  <a name="FollowUp"></a> 補足情報: ナレッジ ベースを作成した後  
 ナレッジ ベースを作成した後は、ナレッジ検出を実行するためのウィザード、照合ポリシーを作成するためのウィザード、またはドメイン管理を実行するためのページが表示されます。 ナレッジ検出、ドメイン管理、または照合ポリシーの詳細については、次を参照してください。 [ナレッジ検出の実行](../data-quality-services/perform-knowledge-discovery.md), 、[ドメインの管理](../data-quality-services/managing-a-domain.md), 、または [照合ポリシーを作成](../data-quality-services/create-a-matching-policy.md)します。  
