...
|値が正規表現と一致する|オペランド内の正規表現と一致する値のみが有効と見なされます。<br /><br /> "^"、"$" などのアンカーを正規表現に含めないでください。これらのアンカーは、この条件を含む句に自動的に追加されます。 (または、"^" アンカーと "$" アンカーを含む正規表現をかっこで囲むこともできます)。正規表現の詳細については、「 [正規表現言語要素](/dotnet/standard/base-types/regular-expression-language-quick-reference)」を参照してください。|オペランドの例: [1-5]+ (1 ～ 5 の数字の 1 回以上の繰り返し)<br /><br /> 有効な値: 123、12345、14352<br /><br /> 無効な値: 456、ABC|  
