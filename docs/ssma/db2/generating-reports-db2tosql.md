...
1.  指定して、**書き込みで概要レポート-を**パラメーター。 (指定した場合)、ファイル名と関連するレポートが格納されているフォルダーにするかを指定します。 ファイル名は、where 句、次の表で説明したようにシステム定義済み **&lt; n &gt;** 同じコマンドの実行ごとに 1 桁の数字で増加する一意のファイル数です。  
    ...
    コマンド オプションの出力からのレポート (Sl です。 不可。 上記の 2 ~ 4) を参照してください、 [SSMA コンソール (&) #40";"DB2ToSQL"&"#41; を実行する](../../ssma/db2/executing-the-ssma-console-db2tosql.md)セクションです。  
...
[SSMA コンソールを実行します。](http://msdn.microsoft.com/en-us/ce63f633-067d-4f04-b8e9-e1abd7ec740b)  
