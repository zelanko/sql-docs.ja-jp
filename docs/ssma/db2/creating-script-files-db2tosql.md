...
7.  **停止操作:** 、ユーザーがその後、操作を停止する場合は、中間操作中に **' Ctrl + C'** ホットキーを使用することができます。 SSMA for DB2 コンソールは、操作の完了を待機し、コンソールの実行を終了します。  
...
たとえば、DB2 データベースの特定のテーブルの一般的なデータ移行は次の階層: スキーマ -&gt;テーブル。  
