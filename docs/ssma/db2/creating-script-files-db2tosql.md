---
title: スクリプト ファイル (DB2ToSQL) の作成 |Microsoft Docs
ms.prod: sql
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
...
7.  **操作を停止します。** ユーザーがその後、操作を停止する場合は、中間操作中に **' Ctrl + C'** ホットキーを使用することができます。 SSMA for DB2 コンソールは、操作の完了を待機し、コンソールの実行を終了します。  
...
たとえば、DB2 データベースの特定のテーブルの一般的なデータ移行では、階層を次に示します。スキーマ -&gt;テーブル。  
...
スクリプト コマンドの完全な一覧が記載されて[SSMA コンソールの実行&#40;DB2ToSQL&#41;](../../ssma/db2/executing-the-ssma-console-db2tosql.md)  
