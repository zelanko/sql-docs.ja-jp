...
**フル モード**: 空の文字列と 0 個の数値  
  
**NULL の連結**  
この設定では、文字列の連結で NULL に変換する方法を指定します。 この特定の設定は、次のオプションを設定できます。  
  
-   **ISNULL 関数でラップ:** かどうかは、このオプションは設定され、ISNULL(string_expression) ですべての非定数の連結では、' string_expression' が折り返される null 値は空の文字列に置き換えられます。  
