...
4.  **プロバイダーを再接続するには:** これにより、ユーザーは、接続エラーが発生した場合の再接続の設定を設定します。 これは、ソースとターゲットの両方のサーバーを設定できます。  
...
7.  **停止操作:** 、ユーザーがその後、操作を停止する場合、中間の操作中に **' Ctrl + C'** ホット キーを使用することができます。 Access のコンソールの SSMA では、操作が完了するが待機し、コンソールの実行を終了します。  
...
スクリプト ファイル、またはサーバーの接続ファイルには、サーバー接続パラメーターを定義することができます。 参照してください、[サーバー接続ファイルを作成する&#40;AccessToSQL&#41; ](../../ssma/access/creating-the-server-connection-files-accesstosql.md)詳細についてはします。  
...
スクリプト コマンドの完全な一覧は含まれて[SSMA コンソールを実行する&#40;AccessToSQL&#41;](../../ssma/access/executing-the-ssma-console-accesstosql.md)  
...
コンソールの運用には、次の手順は[変数値のファイルを作成する&#40;AccessToSQL&#41;](../../ssma/access/creating-variable-value-files-accesstosql.md)です。  
