---
title: コメント付き XSD スキーマの概要 (SQLXML)
description: XML スキーマ定義 (XSD) 言語 (SQLXML 4.0) を使用してリレーショナル データの XML ビューを作成する方法について説明します。
ms.date: 01/11/2019
ms.prod: sql
...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]
 ...
 XSD スキーマでは**\<、xsd:schema>** 要素はスキーマ全体を囲みます。すべての要素宣言は**\<、xsd:schema>** 要素内に含まれている必要があります。 スキーマが存在する名前空間を定義する属性と**\<、xsd:schema>** 要素のプロパティとしてスキーマで使用される名前空間を記述できます。  
  
 有効な XSD スキーマには、次のように定義されている**\<xsd:schema>** 要素が含まれている必要があります。  
 ...
 ** \<xsd:schema>** 要素は、 の XML スキーマ名前空間http://www.w3.org/2001/XMLSchemaの仕様から派生します。  
...
>  [!INCLUDE[msCoName](../../../includes/msconame-md.md)] SQLXML 4.0 の XSD スキーマ言語では、[!INCLUDE[ssVersion2000](../../../includes/ssversion2000-md.md)] の注釈付き XML-Data Reduced (XDR) スキーマ言語で挿入された注釈がサポートされます。 ただし、注釈付き XDR は、SQLXML 4.0 では非推奨とされます。  
 ...
 リレーショナル データベースの場合、任意の XSD スキーマをリレーショナル ストアにマップすると便利です。 これを実行する 1 つの方法は、XSD スキーマに注釈を付けることです。 注釈を持つ XSD スキーマは *、XML*データをリレーショナル ストアにマップする方法に関する情報を提供するマッピング スキーマと呼ばれます。 マッピング スキーマは、実質的にはリレーショナル データの XML ビューです。 これらのマッピングを使用して、リレーショナル データを XML ドキュメントとして取得できます。  
  
## <a name="namespace-for-annotations"></a>注釈の名前空間  
 XSD スキーマでは、注釈は名前空間**urn:スキーマ -microsoft-com:マッピング スキーマ**を使用して指定されます。 次の例に示すように、名前空間を指定する最も簡単な方法は**\<、xsd:schema>** タグで指定することです。  
...
```  
 ...
 使用される名前空間プレフィックスは任意です。 このドキュメントでは **、sql**プレフィックスを使用してアノテーション名前空間を示し、この名前空間の注釈を他の名前空間の注釈と区別します。  
  
## <a name="example-of-an-annotated-xsd-schema"></a>注釈付き XSD スキーマの例  
 次の例では、XSD スキーマは**\<Person.Contact>** 要素で構成されています。 従業員>要素には、**取引先担当者 ID**属性**\<と名>** と**\<姓>** 子要素があります。 ** \<**  
...
```  
 ...
 マッピング スキーマでは、**\<連絡先>** 要素は **、sql:relation**アノテーションを使用してサンプル AdventureWorks データベースの Person.Contact テーブルにマップされます。 属性 ConID、FName、および LName は **、sql:フィールド**の注釈を使用して、Person.Contact テーブルの連絡先 ID、名、および姓の列にマップされます。  
...
-   XML スキーマパート 0: プライマー、W3C 勧告 (https://www.w3.org/TR/xmlschema-0/)  
  
-   XML スキーマパート 1: 構造、W3C 勧告 (https://www.w3.org/TR/xmlschema-1/)  
...
## <a name="see-also"></a>参照  
 [SQLXML 4.0&#41;&#40;の、アセトされたスキーマ セキュリティに関する考慮事項](../../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/security/annotated-schema-security-considerations-sqlxml-4-0.md)   
 [コメント化された XDR スキーマ &#40;SQLXML 4.0&#41;で非推奨](../../../relational-databases/sqlxml/annotated-xsd-schemas/annotated-xdr-schemas-deprecated-in-sqlxml-4-0.md)  
