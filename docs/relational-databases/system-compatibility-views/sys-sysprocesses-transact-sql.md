...
|sql_handle|**binary(20)**|現在実行されているバッチまたはオブジェクトを表します。<br /><br /> **メモ**この値は、オブジェクトのバッチまたはメモリアドレスから取得されます。 この値は、ハッシュベースの[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]アルゴリズムを使用して計算されません。|  
