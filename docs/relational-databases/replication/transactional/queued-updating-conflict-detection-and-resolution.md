---
title: "キュー更新における競合の検出と解決 | Microsoft Docs"
ms.custom: ""
ms.date: "12/09/2016"
...
## 競合の検出  
 ときに、パブリケーションを作成および有効化キュー更新、レプリケーションを追加、 **uniqueidentifier** 列 (**msrepl_tran_version**) 既定値は **newid()** を基になるテーブルにします。 パブリッシュされたデータがパブリッシャーまたはサブスクライバーで変更されると、行は新しいグローバルな一意識別子 (GUID) を受け取り、新しい行が存在することを示します。 キュー リーダー エージェントは同期のとき、この列を使って競合の有無を確認します。  
...
-   [!含める [ssManStudioFull] (../Token/ssManStudioFull_md.md)]: [Set Queued Updating Conflict Resolution Options &#40;SQL Server Management Studio&#41;](../../../relational-databases/replication/publish/set-queued-updating-conflict-resolution-options-sql-server-management-studio.md)  
  
-   レプリケーション TRANSACT-SQL プログラミング: [トランザクション パブリケーションのサブスクリプションの更新を有効にします。](../../../relational-databases/replication/publish/enable-updating-subscriptions-for-transactional-publications.md)  
