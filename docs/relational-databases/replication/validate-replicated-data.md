...
ms.topic: "article"
helpviewer_keywords: 
  - "インライン データの検証 [SQL Server レプリケーション]"
  - "レプリケーションの管理、データの検証"
  - "レプリケーション [SQL Server]、データの検証"
  - "transactional replication, validating data"
  - "データの検証"
  - "マージ レプリケーションのデータ検証 [SQL Server レプリケーション]"
  - "マージ レプリケーションのデータ検証 [SQL Server レプリケーション], データ検証について"
  - "レプリケートされたデータの検証"
...
-   行数とバイナリ チェックサム。 パブリッシャーとサブスクライバーの行数に加え、チェックサム アルゴリズムを使用して、すべてのデータのチェックサムが計算されます。 行数の検証で失敗となった場合、チェックサムは実行されません。  
 ...
 サブスクライバーとパブリッシャーのデータが一致するかどうかの検証に加え、マージ レプリケーションでは、各サブスクライバーに対してデータが正しくパーティション分割されているかどうかを検証する機能も用意されています。 詳細については、次を参照してください。 [マージ サブスクライバーのパーティション情報の検証](../../relational-databases/replication/validate-partition-information-for-a-merge-subscriber.md)します。  
  
 **データを検証するには**  
  
 サブスクリプションのすべてのアーティクルを検証するには、[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]、ストアド プロシージャ、またはレプリケーション管理オブジェクト (RMO) を使用します。 詳しくは、「 [Validate Data at the Subscriber](../../relational-databases/replication/validate-data-at-the-subscriber.md)」をご覧ください。 スナップショット パブリケーションおよびトランザクション パブリケーションの個々のアーティクルを検証するには、ストアド プロシージャを使用する必要があります。  
...
-   という名前のレプリケーションの警告を構成する **レプリケーション: サブスクライバーでは、データの検証が失敗した** 、エラーの場合に通知されるようにします。 詳細については、次を参照してください。 [[定義済みのレプリケーションの警告と #40 です。SQL Server Management Studio と #41;](../../relational-databases/replication/administration/configure-predefined-replication-alerts-sql-server-management-studio.md)します。  
  
-   検証の失敗はアプリケーションにとって問題となりますか?  検証の失敗が問題となる場合、データを手動で更新して同期するか、サブスクリプションを再初期化してください。  
  
    -   使用してデータを更新できる、 [tablediff ユーティリティ](../../tools/tablediff-utility.md)します。 詳細については、このユーティリティを使用して、次を参照してください [相違点と #40; のレプリケートされたテーブルの比較。レプリケーション プログラミングと #41;](../../relational-databases/replication/administration/compare-replicated-tables-for-differences-replication-programming.md)します。  
  
    -   再初期化の詳細については、次を参照してください。 [サブスクリプションの再初期化](../../relational-databases/replication/reinitialize-subscriptions.md)します。  
...
-   レプリケーションの検証を使用して、 **チェックサム** と **binary_checksum** 関数です。 その動作については、次を参照してください。  [チェックサムと #40 です。Transact SQL と #41;](../../t-sql/functions/checksum-transact-sql.md)  [BINARY_CHECKSUM & #40 です。Transact SQL と #41;](../../t-sql/functions/binary-checksum-transact-sql.md)します。  
...
## データ検証の動作  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 行の数またはパブリッシャーでのチェックサムを計算して、行数またはチェックサムは、サブスクライバーで計算にそれらの値を比較して、データを検証します。 1 つの値はパブリケーション テーブル全体に対して計算され、1 つの値はサブスクリプション テーブル全体ですが内のデータに対して計算 **テキスト**, 、**ntext**, 、または **イメージ** 列が計算に含まれません。  
