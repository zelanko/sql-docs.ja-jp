...
---
# キャッシュ、更新、およびレプリケーション モニターのパフォーマンス
  [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] レプリケーション モニターは、実稼働システムで多数のコンピューターを効率的に監視設計されています。 計算の実行、およびデータの収集のためにレプリケーション モニターによって使用されるクエリは、定期的にキャッシュおよび更新されます。 キャッシュによってレプリケーション モニターでさまざまなページを表示する際に必要なクエリと計算の数が削減され、監視のスケーラビリティが向上し、複数のユーザーに対応できるようになります。  
  
 によってキャッシュの更新を処理、 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] エージェント ジョブ、 **レプリケーションの監視の配布リフレッシャー**します。 このジョブは継続的に実行されますが、キャッシュの更新スケジュールは、前回の更新後の待機時間に基づいて設定されます。  
...
## レプリケーション モニターのユーザー インターフェイスの更新  
...
-   F5 キーを押すか、レプリケーション モニターのツリー内のノードを右クリックして、すべてのウィンドウを手動で更新することができます **更新**します。 手動の更新を行うと、キャッシュは強制的に更新されます。  
  
 詳細については、次を参照してください。 [レプリケーション モニターでデータをリフレッシュ](../../../relational-databases/replication/monitor/refresh-data-in-replication-monitor.md)します。  
