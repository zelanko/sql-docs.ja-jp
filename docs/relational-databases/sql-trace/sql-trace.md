---
title: "SQL トレース (SQL Trace) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
## SQL トレース アーキテクチャ  
 ...
 ![データベース エンジンのイベント追跡処理](../../relational-databases/sql-trace/media/tracarch.gif "データベース エンジンのイベント追跡処理")  
...
|**IndexID**|24|イベントの影響を受けるオブジェクトに付けられたインデックス用の ID。 オブジェクトのインデックス ID を調べるには、**indid** システム テーブルの **sysindexes** 列を使用します。|  
...
|**LineNumber**|5|エラーを含む行の番号が格納されます。 **SP:StmtStarting** のような [!INCLUDE[tsql](../../includes/tsql-md.md)] ステートメントを含むイベントの場合、**LineNumber** にはストアド プロシージャまたはバッチでのステートメントの行番号が格納されます。|  
