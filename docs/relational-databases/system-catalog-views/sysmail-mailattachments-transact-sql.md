...
|**ファイル名**|**nvarchar (520)**|添付ファイルのファイル名。 ときに**attach_query_result**は 1 と**query_attachment_filename**が NULL の場合、データベース メールは、任意のファイル名を作成します。|  
|**ファイル サイズ**|**int**|添付ファイルのサイズ (バイト単位)。|  
|**添付ファイル**|**varbinary(max)**|添付ファイルの内容。|  
