...
|**stale_query_threshold_days**|**bigint**|ポリシー設定がないクエリがクエリストアに保持される日数。 既定値は**30**です。 保持ポリシーを無効にするには0に設定します。<br />
  [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)] Basic エディションの場合、既定の日数は 7 日です。<br /><br /> `ALTER DATABASE <database> SET QUERY_STORE ( CLEANUP_POLICY = ( STALE_QUERY_THRESHOLD_DAYS = <value> ) )`ステートメントを使用してを変更します。|  
