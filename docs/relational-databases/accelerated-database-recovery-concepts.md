...
この設計に基づき、データベース エンジンが予想外の再起動から復旧するために必要な時間は、クラッシュ時のシステムで最も大きいアクティブ トランザクションのサイズに (おおよそ) 比例します。 復旧するには、不完全なトランザクションをすべてロールバックする必要があります。 必要な時間の長さは、トランザクションで実行された作業とトランザクションがアクティブになっていた時間に比例します。 そのため、実行時間の長いトランザクション (大量の一括挿入操作やインデックス作成操作など) が存在するとき、SQL Server 復旧プロセスに長時間かかることがあります。
...
- 最も古いアクティブなトランザクションの最初を起点/終点としてログをスキャンすることを回避し、プロセスにかかる時間を一定または一瞬にします。 ADR を利用することで、トランザクション ログの処理は、最後に成功したチェックポイント (あるいは、最も古いダーティ ページのログ シーケンス番号 (LSN)) からに限定されます。 結果として、復旧時間は実行時間の長いトランザクションの影響を受けません。
- トランザクション全体のログを処理する必要がないため、必須のトランザクション ログ領域が最小限に抑えられます。 結果として、チェックポイントやバックアップが行われるたびに、トランザクション ログを積極的に切り捨てることができます。

大まかに言えば、ADR を利用することで、物理データベースのあらゆる変更がバージョン管理され、(数に限りがあり、ほとんど一瞬で元に戻せる) 論理操作のみを元に戻すことでデータベース復旧が速くなります。 クラッシュ時にアクティブになっていたトランザクションがあれば、それには "中止" の印が付けられ、そのようなトランザクションによって生成されたバージョンは同時実行ユーザー クエリで無視されます。

ADR 復旧プロセスには、現行の復旧プロセスと同じく 3 つのフェーズがあります。 それらのフェーズと ADR が連動するしくみを示したものが次の図です。

![ADR 復旧プロセス](./media/accelerated-database-recovery-concepts/adr-recovery-process.png)
...
- **再実行**フェーズ
  ...
  - サブフェーズ 1
      ...
      sLog から再実行します (最も古いコミット前のトランザクションから最後のチェックポイントまで)。 sLog から少数のレコードを処理するだけでよいため、再実行は速くなります。
