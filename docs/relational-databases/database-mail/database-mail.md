...
ms.topic: "article"
helpviewer_keywords: 
  - "アーキテクチャ [SQL Server], データベース メール"
  - "データベース メール [SQL Server], アーキテクチャ"
  - "データベース メール [SQL Server], コンポーネント"
...
-   [データベース メールの使用の利点](#Benefits)  
  
-   [データベース メールのアーキテクチャ](#VisualElement)  
  
-   [データベース メールのコンポーネントの概要](#ComponentsAndConcepts)  
...
-   プロセスの分離。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] への影響を最小限にするために、電子メールを配信するコンポーネントは [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 外部の別個のプロセスで実行されます。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 外部プロセスが停止または失敗しても、電子メールのキューを続行します。 キューに登録されたメッセージは、外部プロセスまたは SMTP サーバーがオンラインになると送信されます。  
