...
###  <a name="HowToEnableCreate"></a> 方法: フルテキスト インデックスがない場合にセマンティック インデックスを作成する  
...
###  <a name="HowToEnableAlter"></a> 方法: 既存のフルテキスト インデックスがある場合にセマンティック インデックスを作成する  
 ...
 **[フルテキスト インデックスのプロパティ]** ダイアログ ボックスの **[フルテキスト インデックスの列]** ページで、セマンティック インデックス作成とフルテキスト インデックス作成が有効な列を変更できます。 詳細については、「[フルテキスト インデックスの管理](../Topic/Manage%20Full-Text%20Indexes.md)」をご覧ください。  
...
###  <a name="drophow"></a> 方法: セマンティック インデックスを削除する  
...
###  <a name="HowToCheckEnabled"></a> 方法: データベース オブジェクトでセマンティック検索が有効かどうかを確認する  
...
###  <a name="doctypes"></a> 方法: どのドキュメントの種類でインデックス作成ができるかを判断する  
