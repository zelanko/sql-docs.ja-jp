...
helpviewer_keywords: 
  ...
  - "SQL Server インストール ウィザード, 認証モード ページ"
  - "認証モードの選択"
  - "認証 [SQL Server], モードの選択"
...
-   [パスワード ポリシーを適用する]  
     ...
     コンピューターの Windows のパスワード ポリシーが [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ログインに適用されます。 これには、パスワードの長さおよび複雑さが含まれます。 この機能は、[!INCLUDE[winxpsvr](../../includes/winxpsvr-md.md)] 以降のバージョンのみで使用できる `NetValidatePasswordPolicy` API に依存します。  
