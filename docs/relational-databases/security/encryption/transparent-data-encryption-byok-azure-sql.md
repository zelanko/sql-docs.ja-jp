...
- SQL Database の TDE プロテクター専用の 1 つのサブスクリプションに Azure Key Vault を構成します。  論理サーバーに関連付けられているデータベースはすべて、同じ TDE プロテクターを使用するため、論理サーバーに対するデータベースのグループ化を検討する必要があります。 
...
- Azure Active Directory (AAD) の ID を使用して、キー コンテナーへのアクセス権を論理サーバーに付与します。  ポータルの UI を使用すると、AAD の ID が自動的に作成され、キー コンテナーのアクセス許可がそのサーバーに付与されます。  BYOK で TDE を構成するために PowerShell を使用すると、AAD の ID が作成され、完了が確認されます。 PowerShell を使用するときの詳しいステップバイステップのガイダンスについては、「[BYOK での TDE の構成](transparent-data-encryption-byok-azure-sql-configure.md)」を参照してください。

  >[!NOTE]
  >AAD の ID **が誤って削除されたり、キー コンテナーのアクセス ポリシーを使用してサーバーのアクセス許可が取り消されたりした**場合、そのサーバーはキー コンテナーへのアクセス権を失います。
  >
