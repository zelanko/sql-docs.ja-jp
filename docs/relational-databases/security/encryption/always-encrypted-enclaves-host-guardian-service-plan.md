---
title: ホスト ガーディアン サービスの構成証明の計画 | Microsoft Docs
ms.custom: ''
ms.date: 10/12/2019
ms.prod: sql
ms.reviewer: vanto
...
[!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] を実行しているコンピューターをドメインに参加させる必要はありませんが、その場合は、HGS サーバーで使用されているものとは別のドメインにする必要があります。
...
SQL クライアントと [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] は両方とも、HTTP 経由で HGS と通信できる必要があります。 必要に応じて、暗号化された HTTPS 接続をサポートするように、TLS 証明書を使用して HGS を構成することもできます。
...
TPM 構成証明の場合は、セキュリティで保護された起動が必要になります。これは、Microsoft が署名した正当なブートローダーが UEFI によって確実に読み込まれ、ルートキットまたはブートキットによってハイパーバイザー起動プロセスがインターセプトされないようにするためです。
...
同様に、[!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] を実行しているコンピューターに TPM 2.0 モジュールがインストールされていないシナリオ、またはセキュリティが最重要ではない開発/テストシナリオでは、ホスト キーの構成証明が依然として有用です。
...
| CPU | 2 コア (最小)、4 コア (推奨) |
| RAM | 8 GB (最小) |
| NIC | NIC は 2 つとすることを推奨 (1 つはクラスター トラフィック用、もう 1 つは HGS サービス用) |
...
- 仮想化テクノロジの CPU サポート:
  ...
  - VM で [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] を実行する場合、ハイパーバイザーおよび物理 CPU には入れ子になった仮想化機能が用意されている必要があります。 VM で VBS エンクレーブを実行する場合の保証については、「[信頼モデル](#trust-model)」セクションを参照してください。
    - Hyper-V 2016 以降では、[VM プロセッサ上で入れ子にされた仮想化拡張機能を有効](https://docs.microsoft.com/virtualization/hyper-v-on-windows/user-guide/nested-virtualization#configure-nested-virtualization)にします。
    ...
    - 他のハイパーバイザーおよびパブリック クラウドでは、VBS エンクレーブが設定された Always Encrypted を有効にする入れ子になった仮想化機能がサポートされている場合もあります。 互換性と構成手順については、ご利用の仮想化ソリューションのドキュメントを確認してください。
