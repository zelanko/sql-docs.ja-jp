...
ms.topic: "article"
helpviewer_keywords: 
  - "EKM, Key Vault のセットアップ"
  - "SQL Server コネクタ, セットアップ"
  - "SQL Server コネクタ"
...
## パート I: Azure Active Directory サービス プリンシパルをセットアップする  
...
2.  アプリケーションを Azure Active Directory に登録します。 アプリケーションを登録する詳しい手順については、[Azure Key Vault のブログ記事](https://blogs.technet.microsoft.com/kv/2015/06/02/azure-key-vault-step-by-step/)の「**Get an identity for the application (アプリケーションの ID を取得する)**」セクションを参照してください。  
...
4.  **Azure Active Directory サービス プリンシパルが Key Vault にアクセスするための権限を付与する**  
    ...
    >  Azure Active Directory サービス プリンシパルには、Key Vault に対して少なくとも `get`、`list`、`wrapKey`、`unwrapKey` の権限が必要です。  
     ...
     `ServicePrincipalName` パラメーターには、以下のように、パート I でコピーした**クライアント ID** を使用します。 `Set-AzureRmKeyVaultAccessPolicy` は、メッセージを伴わずに実行されます。正常に実行されたとしても何も出力されません。  
...
5.  **Key Vault に非対称キーを生成する**  
    ...
    ##### 例:  
    新しい暗号化キーを Azure Key Vault に直接作成し、ソフトウェアまたは HSM で保護することもできます。 この例では、ソフトウェアによって保護されるキーを `Add-AzureRmKeyVaultKey cmdlet`で作成します。  
...
## パート III: [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] コネクタをインストールする  
...
>  1.0.0.440 以前のバージョンは置き換えられ、実稼働環境ではサポートされなくなりました。 [Microsoft ダウンロード センター](https://www.microsoft.com/download/details.aspx?id=45344)にアクセスし、[[SQL Server コネクタのメンテナンスとトラブルシューティング]](../../../relational-databases/security/encryption/sql-server-connector-maintenance-troubleshooting.md) ページの「SQL Server コネクタのアップグレード」に示されている手順を使用して、バージョン 1.0.1.0 以降にアップグレードしてください。
 ...
 コネクタ用のインターフェイスはありませんが、正常にインストールされた場合はコンピューターに **Microsoft.AzureKeyVaultService.EKM.dll** がインストールされます。 これは、`CREATE CRYPTOGRAPHIC PROVIDER` ステートメントを使用して [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] に登録する必要のある暗号化 EKM プロバイダー DLL です。  
...
## パート IV: 構成する [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]  
