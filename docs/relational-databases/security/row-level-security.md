...
-   述語関数で使用されているどの列も変更されていない場合、オプティマイザーは AFTER UPDATE ブロック述語をチェックしません。 たとえば、Alice は給与を変更して 100,000 を超える値にすることはできませんが、給与が既に 100,000 を超えている (給与が既に述語に違反しているため) 従業員の住所は変更できます。  
