...
helpviewer_keywords: 
  - "db_denydatareader ロール"
  - "ユーザー [SQL Server], データベース ロール"
  - "データベース レベルのロール [SQL Server]"
  - "db_denydatawriter ロール"
  - "ロール [SQL Server], データベース"
  - "プリンシパル [SQL Server], データベース レベル"
  - "db_backupoperator ロール"
  - "資格情報 [SQL Server], ロール"
  - "db_accessadmin ロール"
  - "スキーマ [SQL Server], ロール"
  - "権限 [SQL Server], ロール"
  - "データベース ロール [SQL Server], 一覧表示"
  - "db_datareader ロール"
  - "db_ddladmin ロール"
  - "db_datawriter ロール"
  - "db_securityadmin ロール"
  - "db_owner ロール"
  - "データベース ロール [SQL Server]"
  - "固定データベース ロール [SQL Server]"
  - "認証 [SQL Server], ロール"
  - "グループ [SQL Server]、ロール"
...
データベース ロールに対するユーザーの追加および削除を行うには、[ALTER ROLE](../../../t-sql/statements/alter-role-transact-sql.md) ステートメントの `ADD MEMBER` と `DROP MEMBER` のオプションを使用します。 `ALTER ROLE` では、[!INCLUDE[ssPDW_md](../../../includes/sspdw-md.md)] のこのような使用はサポートされません。 代わりに、以前の [sp_addrolemember](../../../relational-databases/system-stored-procedures/sp-addrolemember-transact-sql.md) と [sp_droprolemember](../../../relational-databases/system-stored-procedures/sp-droprolemember-transact-sql.md) プロシージャを使用してください。
...
## <a name="fixeddatabase-roles"></a>固定データベース ロール
...
## <a name="special-roles-for-includesssdsmdtokensssdsmdmd-and-includesssdwmdtokensssdwmdmd"></a>特別なロール - [!INCLUDE[ssSDS_md](../../../includes/sssds-md.md)] と [!INCLUDE[ssSDW_md](../../../includes/sssdw-md.md)]
...
## <a name="working-with-databaselevel-roles"></a>データベース レベルのロールの操作  
