...
# [データベースのプロパティ] ([ファイル グループ] ページ)
 ...
 FILESTREAM が有効になっていない場合、**Filestream** のセクションは使用できません。 FILESTREAM ストレージを有効にするには、[[サーバーのプロパティ] ([詳細設定] ページ)](../Topic/Server%20Properties%20\(Advanced%20Page\).md) を使用します。  
...
## ROW および FILESTREAM データ ファイル グループのオプション  
 **名前**  
 ファイル グループの名前を入力します。  
