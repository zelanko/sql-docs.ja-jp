...
11. **[転送元のデータベース ファイルの場所]** ページ: **転送元のサーバーでのファイル共有**の省略記号ボタンをクリックし、指定したフォルダーの場所に移動します。  たとえば、フォルダーの場所 `D:\MSSQL13.MSSQLSERVER\MSSQL\DATA` は、 `\\Server1\D$\MSSQL13.MSSQLSERVER\MSSQL\DATA` 転送元のサーバーでのファイル共有 **の**を使用します。  [**次へ**] をクリックします。
