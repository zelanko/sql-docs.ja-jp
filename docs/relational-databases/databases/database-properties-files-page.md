 ...
 **[ファイルの種類]**  
 ...
 FILESTREAM データ ファイル グループにファイル (コンテナー) を追加するには、FILESTREAM を有効にする必要があります。 FILESTREAM を有効にするには、[[サーバーのプロパティ] ([詳細設定] ページ)](../Topic/Server%20Properties%20\(Advanced%20Page\).md) ダイアログ ボックスを使用します。  
