...
>   
>  また、リモート処理プロキシ/スタブ dll (MDAC 2.8) は、Native Client [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]の**ISSAsynchStatus**インターフェイスを呼び出すことができません。 **ISSAsynchStatus** インターフェイスは、リモート処理経由では公開されません。  
>   
