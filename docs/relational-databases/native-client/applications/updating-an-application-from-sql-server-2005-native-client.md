...
|**sp_columns** ストアド プロシージャによって IS_NULLABLE 列に **"NO "** ではなく **"NO"** が返される。|Native Client [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 10.0 ([!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]) 以降では、 **sp_columns**ストアドプロシージャは、IS_NULLABLE 列に対して " **no** " ではなく **"no"** を返すようになりました。|  
