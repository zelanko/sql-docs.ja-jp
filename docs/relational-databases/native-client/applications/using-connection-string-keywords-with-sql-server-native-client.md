...
|**[ドライバー]**|によって返されるドライバー名[SQLDrivers](../../../relational-databases/native-client-odbc-api/sqldrivers.md)します。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC ドライバーに対するこのキーワードの値は "{SQL Server Native Client 11.0}" です。 **サーバー**場合は、キーワードが必要です**ドライバー**が指定されて、 **DriverCompletion**を SQL_DRIVER_NOPROMPT に設定されます。<br /><br /> ドライバー名の詳細については、[SQL Server Native Client ヘッダーとライブラリ ファイルを使用して](../../../relational-databases/native-client/applications/using-the-sql-server-native-client-header-and-library-files.md)を参照してください。|  
