...
---
# Azure でのデータベース ファイルのファイル スナップショット バックアップ
...
-   Azure アカウントをすでにお持ちですか?  既にお持ちの場合は、**[こちら](https://azure.microsoft.com/marketplace/partners/microsoft/sqlserver2016ctp3evaluationwindowsserver2012r2/?wt.mc_id=sqL16_vm)**にアクセスして、[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] がインストール済みの仮想マシンをすぐにご利用いただけます。  
  
## Azure のスナップショットを使用した Azure に格納されているデータベース ファイルのバックアップ  
  
### [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ファイル スナップショット バックアップとは  
...
## [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ファイル スナップショット バックアップからの復元  
...
## [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ファイル スナップショット バックアップからの特定の時点への復元  
...
## データベース ファイル スナップショット バックアップ セットを削除する  
 ファイル スナップショットのバックアップ セットを削除するには、**sys.sp_delete_backup** システム ストアド プロシージャを使用します。 データベース名を指定すると、指定されたファイル スナップショット バックアップ セットが実際に指定したデータベースのバックアップであることをシステムが検証します。 データベース名を指定しないと、このような検証が行われることなく、指定したバックアップ セットとそのファイル スナップショットが削除されます。 詳細については、「[sp_delete_backup &#40;Transact-SQL&#41;](../Topic/sp_delete_backup%20\(Transact-SQL\).md)」を参照してください。  
...
## この記事は役に立ちましたか? フィードバックをお待ちしております。  
 どのような情報をお探しでしたか? お探しの情報は見つかりましたか? コンテンツ改善のため、フィードバックをお待ちしています。  [sqlfeedback@microsoft.com](mailto:sqlfeedback@microsoft.com?subject=Your%20feedback%20about%20the%20File-Snapshot%20Backups%20for%20Database%20Files%20in%20Azure%20page)にコメントをご送信ください  
  
## 参照  
 [Tutorial: Using the Microsoft Azure Blob storage service with SQL Server 2016 databases (チュートリアル: Microsoft Azure BLOB ストレージ サービスと SQL Server 2016 データベースの使用)](https://msdn.microsoft.com/library/dn466438.aspx)  
