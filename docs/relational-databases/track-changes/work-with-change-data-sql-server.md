...
helpviewer_keywords: 
  - "変更データ [SQL Server]"
  - "変更データ キャプチャ [SQL Server], クエリ関数のシナリオ"
  - "変更データ キャプチャ [SQL Server], LSN 境界"
  - "変更データ キャプチャ [SQL Server], クエリ関数"
...
-   [cdc.fn_cdc_get_all_changes_<capture_instance>](../Topic/cdc.fn_cdc_get_all_changes_%3Ccapture_instance%3E%20%20\(Transact-SQL\).md) 関数は、指定した期間に発生したすべての変更を返します。 この関数は常に生成されます。 エントリは、必ず並べ替えて返されます。まず変更のトランザクション コミット LSN で並べ替えられ、次にそのトランザクション内の変更のシーケンス値で並べ替えられます。 選択した行フィルター オプションに応じて、更新について最後の行が返される (行フィルター オプションが "all" の場合) か、新しい値と古い値の両方が返されます (行フィルター オプションが "all update old" の場合)。  
  
-   [cdc.fn_cdc_get_net_changes_<capture_instance>](../Topic/cdc.fn_cdc_get_net_changes_%3Ccapture_instance%3E%20\(Transact-SQL\).md) 関数は、ソース テーブルが有効な場合に @supports_net_changes パラメーターが 1 に設定されていると生成されます。  
...
### datetime ラッパー関数を使用したキャプチャ インスタンスの切り替え  
