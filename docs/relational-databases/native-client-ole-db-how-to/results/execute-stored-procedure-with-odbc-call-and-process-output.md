...
> [!IMPORTANT]  
>  可能な場合は、Windows 認証を使用します。 Windows 認証が使用できない場合は、実行時に資格情報を入力するようユーザーに求めます。 資格情報をファイルに保存するのは避けてください。 資格情報を保持する必要がある場合は[、Win32 Crypto API](https://go.microsoft.com/fwlink/?LinkId=64532)を使用して資格情報を暗号化する必要があります。  
