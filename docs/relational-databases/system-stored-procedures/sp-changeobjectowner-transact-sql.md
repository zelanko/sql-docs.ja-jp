...
> [!IMPORTANT]
>  このストアドプロシージャは、で[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)]使用可能なオブジェクトに対してのみ機能します。 [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)]代わりに、 [ALTER SCHEMA](../../t-sql/statements/alter-schema-transact-sql.md)または[alter AUTHORIZATION](../../t-sql/statements/alter-authorization-transact-sql.md)を使用してください。 **sp_changeobjectowner**によってスキーマと所有者の両方が変更されます。 以前のバージョンの [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] との互換性を維持するために、現在の所有者と新しい所有者がデータベースのユーザー名と同じ名前のスキーマを所有している場合は、このストアド プロシージャはオブジェクトの所有者だけを変更します。  
> 
> [!IMPORTANT]
...
`[ @newowner = ] 'owner_ '`オブジェクトの新しい所有者となるセキュリティアカウントの名前を指定します。 *owner*は**sysname**,、既定値はありません。 *所有者*は、現在のデータベースにアクセスできる有効[!INCLUDE[msCoName](../../includes/msconame-md.md)]なデータベースユーザー、サーバーロール、windows ログイン、または windows グループである必要があります。 新しい所有者が、対応するデータベースレベルのプリンシパルが存在しない Windows ユーザーまたは Windows グループである場合は、データベースユーザーが作成されます。  
