...
||**オプション**|アーティクルに対してインタラクティブ競合回避モジュールの使用を無効にします。|  
...
||**オプション**|列レベルの追跡を無効にし、競合検出を行レベルのままにします。 テーブル アーティクルにのみ適用されます。|  
...
||**オプション**|DELETE ステートメントはレプリケートされません。<br /><br /> **\*\*重要な\* \*** 設定**delete_tracking**に**false**非収束、および削除された行の結果を手動で削除する必要があります。|  
...
||**オプション**|バイナリ ラージ オブジェクトの列をレプリケートするときに、最適化が使用されません。|  
