...
|**pre_creation_command**|**[なし]**|サブスクライバーでテーブルが既に存在する場合、アクションは行われません。|  
...
|**status**|**active**|テーブルをパブリッシュする初期処理スクリプトを実行します。|  
