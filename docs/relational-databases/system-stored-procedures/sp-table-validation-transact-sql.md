 ...
 **0**の場合は、rowcount と[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 7.0 と互換性のあるチェックサムを実行します。  
 ...
 **sp_table_validation**は、すべての種類のレプリケーションで使用されます。 **sp_table_validation**は、Oracle パブリッシャーではサポートされていません。  
 ...
 文字モードの**bcp**を使用した場合、浮動小数点値によってチェックサムの違いが生じる可能性があります[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 。これは、パブリケーションに以外のサブスクライバーが含まれている場合に発生します。 これは、文字モード間で変換を実行するときの、わずかではあるが避けられない有効桁数の違いに基づきます。  
