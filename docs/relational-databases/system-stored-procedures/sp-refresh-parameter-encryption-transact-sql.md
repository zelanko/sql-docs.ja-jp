---
title: sp_refresh_parameter_encryption (TRANSACT-SQL) |マイクロソフトのドキュメント
ms.custom: ''
ms.date: 01/11/2017
ms.prod: sql
...
[  **@namespace =** ] **'** < クラス > **'**   
指定されたモジュールのクラスです。 ときに*module_name* 、DDL トリガーは、`<class>`が必要です。 `<class>` `nvarchar(20)`します。 有効な入力は`DATABASE_DDL_TRIGGER`と`SERVER_DDL_TRIGGER`します。    
