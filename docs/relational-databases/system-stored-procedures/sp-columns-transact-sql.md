...
## <a name="arguments"></a>引数  
 ...
 [ **@table_qualifier****=**] *qualifier*  
 オブジェクト識別子の名前です。 *修飾子*は**sysname**、既定値は NULL です。 さまざまな DBMS 製品で、3 つの部分名オブジェクト (*修飾子 ***.*** 所有者 ***.*** 名前*)。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]、この列は、データベース名を表します。 製品によっては、オブジェクトのデータベース環境のサーバー名を表す場合があります。  
