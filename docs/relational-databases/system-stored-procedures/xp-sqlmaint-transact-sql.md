...
## <a name="remarks"></a>解説  
 SQL Server 認証でログオンするユーザーによってこのプロシージャが呼び出される場合、 **-u"***login_id***"**と**-p"** *パスワード***"**スイッチは、前に付加*switch_string*実行前にします。 Windows 認証では、ユーザーがログオンしている場合*switch_string*を変更せずに渡される**sqlmaint**です。  
