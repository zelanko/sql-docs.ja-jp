...
|**重要度**|**int**|重大度レベル (から**9**を通じて**25**、 **110**、 **120**、 **130**、または**140**)警告を定義するとします。|  
|**有効になっています。**|**tinyint**|アラートが現在有効になっているかどうかの状態 (**1**) か (**0**)。 有効でない警告は送信されません。|  
...
|**このパラメーター**|**nvarchar(512)**|電子メールやポケットベルによる通知の一部としてオペレーターに送信する追加メッセージ (省略可)。|  
...
|**フラグ**|**int**|[!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]|  
...
|**型**|**int**|イベントの種類です。<br /><br /> **1**  =  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]イベントのアラート<br /><br /> **2**  =  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]パフォーマンス警告<br /><br /> **3** = WMI イベント警告|  
...
|**event_id**|**int**|[!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]|  
|**message_id**|**int**|警告を定義するメッセージ エラー番号。 (通常はエラー番号に対応、 **sysmessages**テーブル)。 重要度を使用して、警告を定義する場合**message_id**は**0**または NULL。|  
|**重要度**|**int**|重大度レベル (から**9**を通じて**25**、 **110**、 **120**、 **130**、または 1**40**)警告を定義するとします。|  
|**有効になっています。**|**tinyint**|アラートが現在有効になっているかどうかの状態 (**1**) か (**0**)。 有効でない警告は送信されません。|  
|**delay_between_responses**|**int**|警告に対する 1 つの応答から次の応答までの待ち時間 (秒単位)。|  
|**last_occurrence_date**|**int**|警告が最後に発生した日付。|  
|**last_occurrence_time**|**int**|警告が最後に発生した時刻。|  
...
|**フラグ**|**int**|[!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]」をご覧ください。|  
...
|**型**|**int**|警告の種類。<br /><br /> **1**  =  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]イベントのアラート<br /><br /> **2**  =  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]パフォーマンス警告<br /><br /> **3** = WMI イベント警告|  
