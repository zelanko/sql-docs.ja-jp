 ...
 **sp_expired_subscription_cleanup**を検出し、24 時間ごとにパブリケーション データベースから期限切れのサブスクリプションを削除、期限切れサブスクリプション クリーンアップのジョブによって実行します。 保有期間内にパブリッシャーと同期されなかった古いサブスクリプションが含まれている場合、パブリケーションは期限切れとして宣言され、サブスクリプションのトレースがパブリッシャーでクリーンアップされます。 詳細については、「 [Subscription Expiration and Deactivation](../../relational-databases/replication/subscription-expiration-and-deactivation.md)」を参照してください。  
