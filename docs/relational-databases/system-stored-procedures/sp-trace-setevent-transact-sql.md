...
|12|SQL:BatchCompleted|@No__t 0 のバッチが完了したときに発生します。|  
...
|39|非推奨|非推奨|  
|40|SQL:StmtStarting|@No__t-0 ステートメントが開始されたときに発生します。|  
|41|SQL:StmtCompleted|@No__t-0 ステートメントが完了したときに発生します。|  
|42|SP:Starting|ストアドプロシージャが開始された時点を示します。|  
|43|SP:Completed|ストアド プロシージャが完了したことを示します。|  
|44|SP:StmtStarting|ストアドプロシージャ内の @no__t 0 ステートメントが実行を開始したことを示します。|  
...
|48|予約済み||  
|49|予約済み||  
|50|SQL トランザクション|@No__t-0 BEGIN、COMMIT、SAVE、および ROLLBACK TRANSACTION ステートメントを追跡します。|  
...
|70|CursorPrepare|@No__t-0 ステートメントのカーソルが、ODBC、OLE DB、または DB-LIBRARY で使用できるように準備された時点を示します。|  
|71|Prepare SQL|ODBC、OLE DB、または DB-LIBRARY では、使用する @no__t 0 のステートメントが準備されています。|  
|72|Exec Prepared SQL|ODBC、OLE DB、または DB-Library によって、準備された 1 つ以上の [!INCLUDE[tsql](../../includes/tsql-md.md)] ステートメントが実行されたことを示します。|  
|73|Unprepare SQL|ODBC、OLE DB、または DB-LIBRARY が、準備された @no__t 0 ステートメントまたはステートメントを準備解除 (削除) しました。|  
...
|76|CursorImplicitConversion|@No__t-0 ステートメントのカーソルは、@no__t によって1つの型から別の型に変換されます。<br /><br /> ANSI および ANSI 以外のカーソルに対してトリガーされます。|  
|77|CursorUnprepare|[!INCLUDE[tsql](../../includes/tsql-md.md)] ステートメントで準備されたカーソルが、ODBC、OLE DB、または DB-Library によって削除されたことを示します。|  
|78|CursorClose|ODBC、OLE DB、または DB-LIBRARY によって @no__t 0 ステートメントで以前に開かれたカーソルは閉じられます。|  
|79|列の統計情報がありません|オプティマイザーに有効な列の統計が利用できません。|  
|80|Missing Join Predicate|結合述語がないクエリが実行されています。 クエリの終了に時間がかかる可能性があります。|  
|81|Server Memory Change|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] メモリ使用量は、1メガバイト (MB) または最大サーバーメモリの5% のいずれか大きい方によって増加または減少しています。|  
...
|104|Audit AddLogin イベント|@No__t 0 のログインが追加または削除されたときに発生します。**sp_addlogin**と**sp_droplogin**の場合。|  
|105|Audit Login GDR Event|Windows ログイン権限が追加または削除されたときに発生します。**sp_grantlogin**、 **sp_revokelogin**、および**sp_denylogin**の場合。|  
|106|Audit Login Change Property Event|パスワード以外のログインのプロパティが変更された場合に発生します。**sp_defaultdb**と**sp_defaultlanguage**の場合。|  
|107|Audit Login Change Password イベント|@No__t 0 のログインパスワードが変更されたときに発生します。<br /><br /> パスワードは記録されません。|  
|108|Audit Add Login to Server Role イベント|固定サーバーロールに対してログインが追加または削除されたときに発生します。**sp_addsrvrolemember**と**sp_dropsrvrolemember**の場合。|  
|109|Audit Add DB User Event|データベースユーザー (Windows または @no__t 0) としてログインが追加または削除されたときに発生します。**sp_grantdbaccess**、 **sp_revokedbaccess**、 **sp_adduser**、および**sp_dropuser**の場合。|  
|110|Audit Add Member to DB ロールイベント|データベースに対してデータベースユーザー (固定またはユーザー定義) としてログインが追加または削除されたときに発生します。**sp_addrolemember**、 **sp_droprolemember**、および**sp_changegroup**の場合。|  
...
|124|Broker:Conversation|@No__t 0 のメッセージ交換の進行状況を報告します。|  
|125|廃止に関するお知らせ|@No__t-0 の将来のバージョンから削除される機能を使用すると発生します。|  
...
|142|Broker:Transmission|@No__t-0 トランスポート層でエラーが発生したことを示します。 エラー番号と状態の値で、エラーの原因を確認できます。|  
|143|Broker: キューが無効です|@No__t 0 キューで連続して5つのトランザクションロールバックが発生したため、有害メッセージが検出されたことを示します。 イベントには、有害メッセージを含むキューのデータベース ID とキュー ID が含まれます。|  
|144-145|予約済み||  
...
|158|Audit Broker Conversation|@No__t-0 ダイアログセキュリティに関連する監査メッセージを報告します。|  
|159|Audit Broker Login|@No__t-0 トランスポートセキュリティに関連する監査メッセージを報告します。|  
|160|Broker: メッセージを配信できませんでした|@No__t-0 が、サービスに配信される必要がある受信メッセージを保持できない場合に発生します。|  
|161|Broker:Corrupted Message|@No__t-0 が破損したメッセージを受信したときに発生します。|  
...
|181|TM: トランザクションの開始開始|BEGIN TRANSACTION 要求が開始されたときに発生します。|  
|182|TM: 開始トランザクションの完了|BEGIN TRANSACTION 要求が完了したときに発生します。|  
|183|TM: トランザクションの昇格を開始します|トランザクションの昇格要求が開始されたときに発生します。|  
|184|TM: プロモーショントランザクションの完了|PROMOTE TRANSACTION 要求が完了したときに発生します。|  
|185|TM: Commit Tran を開始しています|COMMIT TRANSACTION 要求が開始されたときに発生します。|  
|186|TM: コミットトランザクションの完了|COMMIT TRANSACTION 要求が完了したときに発生します。|  
|187|TM: ロールバックトランザクションを開始しています|ROLLBACK TRANSACTION 要求が開始されたときに発生します。|  
|188|TM: ロールバックトランザクション完了|ROLLBACK TRANSACTION 要求が完了したときに発生します。|  
|189|Lock: Timeout (タイムアウト > 0)|ページなどのリソースのロック要求がタイムアウトしたときに発生します。|  
|190|Progress Report: オンラインインデックス操作|ビルドプロセスの実行中に、オンラインのインデックス構築操作の進行状況を報告します。|  
...
|213|Database Suspect Data Page|**Msdb**の**suspect_pages**テーブルにページが追加されたことを示します。|  
...
`[ @columnid = ] column_id` は、イベントに追加する列の ID です。 *column_id*は**int**,、既定値はありません。  
...
|3|**DatabaseID**|USE *database*ステートメントで指定されたデータベースの ID、または特定の接続に対して use *database*ステートメントが実行されていない場合は既定のデータベースの ID となります。<br /><br /> データベースの値を確認するには、DB_ID 関数を使用します。|  
...
|19|**権限**|アクセス許可のビットマップを表します。セキュリティ監査によって使用されます。|  
...
|26|**ServerName**|トレースされている [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ( *servername*または*servername\instancename*のいずれか) のインスタンスの名前。|  
...
|31|**Error**|エラー番号。|  
|32|**Mode**|取得したロックのロックモード。 この列は、 **Lock: Released**イベントによって設定されません。|  
 ...
 *On*が**1**に設定され、 *column_id*が NULL の場合、イベントは on に設定され、すべての列がクリアされます。 *Column_id*が null でない場合は、そのイベントに対して列が ON に設定されます。  
  
 *On*が**0**に設定され、 *column_id*が NULL の場合、イベントはオフになり、すべての列がクリアされます。 *Column_id*が null でない場合、列は無効になります。  
  
 次の表は、と**の \@** の間の相互作用と **@no__t 3columnid**を示しています。  
...
-   **xp_trace_seteventclassrequired**  
 ...
 ユーザーは、各イベントに追加された各列に対して**sp_trace_setevent**を実行する必要があります。 **Sp_trace_setevent**が**1**に設定され**て @no__t**いる場合は、実行のたびに、指定したイベントがトレースのイベントの一覧に追加されます。 **@No__t-1on**が**0**に設定されている場合、 **sp_trace_setevent**は指定されたイベントを一覧から削除します。  
