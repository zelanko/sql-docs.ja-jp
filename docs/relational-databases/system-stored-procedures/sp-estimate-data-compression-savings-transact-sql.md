...
## <a name="remarks"></a>コメント  
 を`sp_estimate_data_compression_savings`使用すると、行、ページ、列ストア、または列ストアアーカイブの圧縮に対してテーブルまたはパーティションを有効にした場合に発生する節約量を見積もることができます。 たとえば、行の平均サイズを 40% 削減できれば、オブジェクトのサイズを 40% 削減できる可能性があります。 これは FILL FACTOR と行サイズに左右されるため、領域を削減できない場合もあります。 たとえば、8000バイトの長さの行があり、そのサイズを 40% 削減した場合でも、データページには1つの行だけを収めることができます。 領域は削減されません。  
  
 を実行`sp_estimate_data_compression_savings`した結果、テーブルが大きくなることを示している場合は、テーブル内の多くの行でデータ型の精度がほぼすべて使用されていることを意味します。また、圧縮形式に必要な少量のオーバーヘッドを追加することは、機能. このようなまれなケースでは、圧縮を使用しないでください。  
  
 テーブルで圧縮が有効になっている`sp_estimate_data_compression_savings`場合は、を使用して、テーブルが圧縮されていない場合の行の平均サイズを推定します。  
...
> 行ストアのソースオブジェクト (クラスター化インデックス、非クラスター化インデックス、またはヒープ) から列ストア圧縮を推定するときに、列ストアインデックスでサポートされていないデータ型を持つ列がソースオブジェクトに存在する場合、sp_estimate_compression_savingsはエラーで失敗します。
 ...
 同様に、 `@data_compression`パラメーターが`ROW`、、また`NONE`は`PAGE`に設定され、ソースオブジェクトが列ストアインデックスの場合、使用される参照オブジェクトの概要を次の表に示します。
