...
> [!IMPORTANT]  
> [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] @no__t のロックに関する情報を取得するには、動的管理ビューを[ロック](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md)します。  
...
`[ @spid1 = ] 'session ID1'` は、ユーザーがロック情報を必要とする **_exec_sessions**からの @no__t セッション ID 番号です。 *SESSION ID1*は**int**で、既定値は NULL です。 セッションに関するプロセス情報を取得するには、 **sp_who**を実行します。 *SESSION ID1*が指定されていない場合は、すべてのロックに関する情報が表示されます。  
  
`[ @spid2 = ] 'session ID2'` は、ID1**のセッション**ID 番号です。これ @no__t は、*セッション*と同時にロックが設定されている可能性があり、ユーザーが情報を必要とする場合もあります。 *SESSION ID2*は**int**で、既定値は NULL です。  
  
## <a name="return-code-values"></a>リターン コードの値  
 成功した場合は 0 を返します。  
  
## <a name="result-sets"></a>結果セット  
 **Sp_lock** result セットには、2spid1 パラメーター **@no__t と 4spid2**パラメーター **@no__t**で指定したセッションによって保持されているロックごとに1つの行が含まれています。 1spid1 も **@no__t**も指定されていない **@no__t**場合、結果セットは、[!INCLUDE[ssDE](../../includes/ssde-md.md)] のインスタンスで現在アクティブになっているすべてのセッションのロックを報告します。  
  
|列名|データ型|説明|  
|-----------------|---------------|-----------------|  
|**spid**|**smallint**|ロックを要求しているプロセスの @no__t 0 のセッション ID 番号。|  
|**dbid**|**smallint**|ロックが保持されているデータベースの ID 番号です。 DB_NAME () 関数を使用すると、データベースを識別できます。|  
