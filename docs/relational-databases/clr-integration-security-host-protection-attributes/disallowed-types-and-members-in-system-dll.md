---
title: System.dll での許可されていない型とメンバー |マイクロソフトドキュメント
description: SQL Server CLR プログラミングでは、ホストプロテクションリソース列挙型の特定の値を持つ型またはメンバーを許可しません。 この資料では、System.dll が許可されていない値の一覧を示します。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
