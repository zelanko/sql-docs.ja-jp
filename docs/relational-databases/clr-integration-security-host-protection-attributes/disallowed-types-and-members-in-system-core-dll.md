---
title: System.Core.dll の型とメンバーが許可されていません |マイクロソフトドキュメント
description: SQL Server CLR プログラミングでは、ホストプロテクションリソース列挙型の値を持つ型またはメンバーを許可しません。 この記事では、System.Core.dll の許可されていない値を一覧表示します。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
