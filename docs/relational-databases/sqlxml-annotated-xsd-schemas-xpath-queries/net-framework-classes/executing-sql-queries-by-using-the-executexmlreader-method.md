...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]ExecuteToStream メソッドを使用する代わりにコマンドを実行するのに SqlXmlCommand オブジェクトの ExecuteXmlReader メソッドを使用することができます。 このメソッドは、さらに、結果は (この例では、要素または属性の名前と値の印刷) の処理のために使用する XmlReader オブジェクトを返します。  
