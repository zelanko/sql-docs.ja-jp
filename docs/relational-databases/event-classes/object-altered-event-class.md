...
|IntegerData|**int**|対応する開始イベントのイベント シーケンス番号。 この列は、コミット型またはロールバック型のイベント サブクラスのみに使用できます。|25|[ユーザー アカウント制御]|  
|IsSystem|**int**|イベントがシステム プロセスとユーザー プロセスのどちらで発生したか。 1 = システム、NULL = ユーザーです。|60|[ユーザー アカウント制御]|  
...
|ObjectID2|**bigint**|パーティション スキーマが変更されたときはパーティション関数 ID、サービスが変更されたときはキュー ID、XML スキーマが変更されたときはコレクション スキーマ ID。|56|[ユーザー アカウント制御]|  
...
|RequestID|**int**|ステートメントを含んでいるバッチ要求の ID。|49|[ユーザー アカウント制御]|  
