...
|Handle|**int**|準備済み Transact-SQL ステートメントのハンドル。|33|可|  
...
|LoginSid|**image**|ログイン ユーザーのセキュリティ ID 番号 (SID)。 この情報は、sys.server_principals カタログで参照できます。 各 SID はサーバーのログインごとに一意です。|41|可|  
