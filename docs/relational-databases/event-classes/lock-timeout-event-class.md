...
|DatabaseID|**int**|ロックのタイムアウトが発生したデータベースの ID。 [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] では、ServerName データ列がトレースにキャプチャされ、そのサーバーが利用可能な場合、データベースの名前が表示されます。 データベースに対応する値は、DB_ID 関数を使用して特定します。|3|可|  
...
|ObjectID|**int**|タイムアウトしたオブジェクトの ID (使用可能かつ適用可能な場合)。|22|可|  
...
|TextData|**ntext**|タイムアウトが発生したときに取得されていたロックの種類に依存するテキスト値。|1|可|  
