...
|ObjectName|**nvarchar**|ストアド プロシージャの名前。 ObjectName に値が設定された場合、TextData には値が設定されません。|34|可|  
...
|TextData|**ntext**|キャッシュされている SQL コードのテキスト。 TextData に値が設定された場合、ObjectName には値が設定されません。|1|可|  
