...
## SQL:StmtCompleted イベント クラスのデータ列  
...
|CPU|**int**|イベントに使用された CPU 時間 (ミリ秒単位)。|18|可|  
...
|NestLevel|**int**|ステートメントがストアド プロシージャ内で実行された場合のストアド プロシージャの入れ子レベル。|29|可|  
...
|TextData|**ntext**|実行されたステートメントのテキスト。|1|可|  
