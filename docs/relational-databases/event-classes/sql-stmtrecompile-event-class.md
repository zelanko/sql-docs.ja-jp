...
|EventSubClass|**int**|再コンパイルの原因を示します。<br /><br /> 1 = スキーマの変更<br /><br /> 2 = 統計の変更<br /><br /> 3 = コンパイルの遅延<br /><br /> 4 = Set オプションの変更<br /><br /> 5 = Temp テーブルの変更<br /><br /> 6 = リモート行セットの変更<br /><br /> 7 = 参照権限の変更<br /><br /> 8 = クエリ通知環境の変更<br /><br /> 9 = パーティション ビューの変更<br /><br /> 10 = カーソル オプションの変更<br /><br /> 11 = オプション (再コンパイル) の要求|21|[ユーザー アカウント制御]|  
|GroupID|**int**|SQL トレース イベントが発生したワークロード グループの ID。|66|[ユーザー アカウント制御]|  
|HostName|**nvarchar**|このステートメントを送信したクライアントが実行されているコンピューターの名前。 このデータ列にはクライアントからホスト名が提供されている場合に値が格納されます。 ホスト名を指定するには、HOST_NAME 関数を使用します。|8|[ユーザー アカウント制御]|  
...
|IsSystem|**int**|イベントがシステム プロセスとユーザー プロセスのどちらで発生したか。<br /><br /> 1 = システム<br /><br /> 0 = ユーザー|60|[ユーザー アカウント制御]|  
|LineNumber|**int**|バッチ内のこのステートメントのシーケンス番号 (該当する場合)。|5|[ユーザー アカウント制御]|  
|LoginName|**nvarchar**|このバッチを送信したログインの名前。|11|[ユーザー アカウント制御]|  
|LoginSid|**image**|現在ログインしているユーザーのセキュリティ識別子 (SID)。 この情報は、sys.server_principals カタログ ビューで参照できます。 各 SID はサーバーのログインごとに一意です。|41|[ユーザー アカウント制御]|  
|NestLevel|**int**|ストアド プロシージャ コールの入れ子レベル。 たとえば、my_proc_a ストアド プロシージャは my_proc_b を呼び出します。 この場合、my_proc_a の NestLevel は 1 で、my_proc_b の NestLevel は 2 です。|29|[ユーザー アカウント制御]|  
|NTDomainName|**nvarchar**|ユーザーが所属する Windows ドメイン。|7|[ユーザー アカウント制御]|  
|NTUserName|**nvarchar**|接続しているユーザーの Windows ユーザー名。|6|[ユーザー アカウント制御]|  
|ObjectID|**int**|再コンパイルの原因となったステートメントを含むオブジェクトのシステムによって割り当てられた識別子。 このオブジェクトは、ストアド プロシージャ、トリガー、またはユーザー定義関数です。 アドホック バッチまたは準備された SQL の場合、ObjectID および ObjectName は NULL 値を返します。|22|[ユーザー アカウント制御]|  
|ObjectName|**nvarchar**|ObjectID によって識別されるオブジェクトの名前。|34|[ユーザー アカウント制御]|  
...
|SPID|**int**|接続のサーバー プロセス ID。|12|[ユーザー アカウント制御]|  
...
|TextData|**ntext**|再コンパイルされた Transact-SQL ステートメントのテキスト。|1|[ユーザー アカウント制御]|  
