...
|EventSubClass|**int**|イベント サブクラスの種類。<br /><br /> 1 = 単一パス。 並べ替えテーブルをディスクに書き込むとき、並べ替え出力を取得するためにデータを 1 回だけパスする必要がありました。<br /><br /> 2 = 複数パス。 並べ替えテーブルをディスクに書き込むとき、並べ替え出力を取得するためにデータを複数回パスする必要がありました。|21|可|  
...
|IsSystem|**int**|イベントがシステム プロセスとユーザー プロセスのどちらで発生したか。 1 = システム、NULL = ユーザーです。|60|可|  
