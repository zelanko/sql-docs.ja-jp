...
|**ObjectType**|**int**|イベントに関係するオブジェクトの種類を表す値。 この列に返される値は、 **sys.objects** カタログ ビューの **type** 列の対応する値と、 [ObjectType トレース イベント列](../../relational-databases/event-classes/objecttype-trace-event-column.md)に示された値の組み合わせになります。|28|[ユーザー アカウント制御]|  
|**OwnerName**|**nvarchar**|オブジェクト所有者のデータベース ユーザー名。|37|[ユーザー アカウント制御]|  
|**ParentName**|**nvarchar**|オブジェクトが含まれているスキーマの名前。|59|[ユーザー アカウント制御]|  
