...
|BigintData1|**bigint**|パーティション ID (ロック リソースがパーティション分割されている場合)。|52|[ユーザー アカウント制御]|  
...
|Duration|**bigint**|ロック要求が発行されてからロックが取得されたときまでの時間 (マイクロ秒)。|13|[ユーザー アカウント制御]|  
...
|ObjectID|**int**|ロックを取得したオブジェクトの ID (使用可能かつ適用可能な場合)。|22|[ユーザー アカウント制御]|  
...
|TextData|**ntext**|取得したロックの種類に依存するテキスト値。 この値は、sys.dm_tran_locks の resource_description 列と同じです。|1|[ユーザー アカウント制御]|  
