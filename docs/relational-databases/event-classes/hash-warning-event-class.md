...
|ClientProcessID|**int**|クライアント アプリケーションが実行されているプロセスに対し、ホスト コンピューターが割り当てた ID。 クライアントによってクライアント プロセス ID が指定された場合、このデータ列にデータが格納されます。|9|[ユーザー アカウント制御]|  
...
|EventSubClass|**int**|イベント サブクラスの種類。<br /><br /> 0 = 再帰<br /><br /> 1 = 保留|21|[ユーザー アカウント制御]|  
...
|IntegerData|**int**|再帰レベル (ハッシュの再帰のみ)。|25|[ユーザー アカウント制御]|  
...
|ObjectID|**int**|パーティション再分割に関係するハッシュ チームのルートのノード ID。 Showplan のノード ID と一致します。|22|[ユーザー アカウント制御]|  
