...
|BinaryData|**image**|ANSI NULL、ANSI PADDING、コミット時のカーソル クローズ、NULL 連結、引用符で囲まれた識別子を含むセッション レベル設定など、オプション フラグのバイナリ ダンプ。|2|[ユーザー アカウント制御]|  
...
|DatabaseID|**int**|ユーザー接続の現在のデータベース ID。 USE *database* ステートメントで指定されたデータベースの ID、または特定のインスタンスについて USE *database* ステートメントが実行されていない場合は既定のデータベースの ID となります。 データベースに対応する値は、DB_ID 関数を使用して特定します。|3|[ユーザー アカウント制御]|  
...
|IntegerData|**int**|接続に使用されるネットワーク パケット サイズ。|25|[ユーザー アカウント制御]|  
|IsSystem|**int**|イベントがシステム プロセスとユーザー プロセスのどちらで発生したか。 1 = システム、NULL = ユーザーです。 このイベントの場合は、常に NULL です。|60|[ユーザー アカウント制御]|  
...
|StartTime|**datetime**|ユーザーがこの接続を開いた日時 (ログイン日時)。|14|[ユーザー アカウント制御]|  
|TextData|**ntext**|接続に固有の設定オプション。|1|[ユーザー アカウント制御]|  
