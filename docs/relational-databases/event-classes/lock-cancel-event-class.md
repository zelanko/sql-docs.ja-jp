...
|**DatabaseName**|**nvarchar**|ロックの取得が試みられたデータベースの名前。|35|[ユーザー アカウント制御]|  
|**Duration**|**bigint**|ロック要求が発行されてからロックが取り消されたときまでの時間 (マイクロ秒)。|13|[ユーザー アカウント制御]|  
...
|**Exchange Spill**|**int**|ロックが取り消されたオブジェクトの ID (使用可能かつ適用可能な場合)。|22|[ユーザー アカウント制御]|  
