...
|**DatabaseName**|**nvarchar**|ロックの取得が試みられたデータベースの名前。|35|可|  
...
|**ObjectID**|**int**|ロックが取り消されたオブジェクトの ID (使用可能かつ適用可能な場合)。|22|可|  
...
|**TextData**|**ntext**|取得したロックの種類に依存するテキスト値。 この値は、**sys.dm_tran_locks** の **resource_description** 列と同じです。|1|可|  
