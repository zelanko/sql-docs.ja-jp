...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)] **CursorUnprepare** イベント クラスは、アプリケーション プログラミング インターフェイス (API) カーソルで発生するカーソル準備解除イベントについての情報を提供します。 カーソル準備解除イベントは、 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)] によって実行プランが破棄されたときに発生します。  
