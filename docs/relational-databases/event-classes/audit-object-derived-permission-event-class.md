...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)] **Audit Object Derived Permission** イベント クラスにより、指定されたオブジェクトに対して CREATE コマンド、ALTER コマンド、または DROP コマンドが実行された時点が記録されます。 このイベントは、オブジェクトに権限または直接関連付けられた所有者がない場合にのみ発生します。  
