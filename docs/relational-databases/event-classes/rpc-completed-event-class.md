...
|CPU|**int**|イベントによって使用された CPU 時間。 [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]からは、マイクロ秒単位。 それより前のバージョンではミリ秒単位。|18|[ユーザー アカウント制御]|  
...
|Duration|**bigint**|イベントによって使用された時間。 [!INCLUDE[ssKilimanjaro](../../includes/sskilimanjaro-md.md)]からは、マイクロ秒単位。 それより前のバージョンではミリ秒単位。|13|[ユーザー アカウント制御]|  
|EndTime|**datetime**|リモート プロシージャ コールの終了時刻。|15|[ユーザー アカウント制御]|  
|[エラー]|**int**|特定のイベントのエラー番号。<br /><br /> 0 = OK<br /><br /> 1 = エラー<br /><br /> 2 = 中止<br /><br /> 3 = スキップ|31|[ユーザー アカウント制御]|  
...
|Reads|**bigint**|リモート プロシージャ コールによって実行されたページ読み取りの回数。|16|[ユーザー アカウント制御]|  
|RequestID|**int**|ステートメントが含まれている要求の ID。|49|[ユーザー アカウント制御]|  
|RowCounts|**bigint**|RPC バッチに含まれる行数。|48|[ユーザー アカウント制御]|  
...
|TextData|**ntext**|リモート プロシージャ コールのテキスト。|1|[ユーザー アカウント制御]|  
|TransactionID|**bigint**|システムによって割り当てられたトランザクション ID。|4|[ユーザー アカウント制御]|  
|Writes|**bigint**|リモート プロシージャ コールによって実行されたページ書き込みの回数。|17|[ユーザー アカウント制御]|  
