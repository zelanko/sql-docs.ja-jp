...
ms.locfileid: "81301080"
---
# <a name="data-type-support-for-ole-db-date-and-time-improvements"></a>OLE DB の日付/時刻の強化に対するデータ型のサポート
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]

  このトピックでは、日付/時刻型[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]をサポート[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]する OLE DB ( ネイティブ クライアント ) 型について説明します。  
...
```  
 ...
 sql_variant**を使用し**、**日時**の制限された精度に依存する Native Client に[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]移行するアプリケーションは、基になるスキーマが**datetime**ではなく**datetime2**を使用するように更新された場合に更新する必要があります。  
...
|----------------------------------|-----------------------------------------|-----------|  
|DBTYPE_DBDATE|date||  
|DBTYPE_DBTIMESTAMP|**datetime2**(p)|ネイティブ[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]クライアント OLE DB プロバイダーは、秒の小数部の精度を決定するために DBCOLUMDESC *bScale*メンバーを検査します。|  
|DBTYPE_DBTIME2|**time**(p)|ネイティブ[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]クライアント OLE DB プロバイダーは、秒の小数部の精度を決定するために DBCOLUMDESC *bScale*メンバーを検査します。|  
|DBTYPE_DBTIMESTAMPOFFSET|**datetimeoffset**(p)|ネイティブ[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]クライアント OLE DB プロバイダーは、秒の小数部の精度を決定するために DBCOLUMDESC *bScale*メンバーを検査します。|  
  
 アプリケーションで *wType* に DBTYPE_DBTIMESTAMP を指定した場合、*pwszTypeName* に型名を指定することで、**datetime2** へのマッピングをオーバーライドします。 **datetime** を指定する場合、*bScale* は 3 にする必要があります。 **smalldatetime** を指定する場合、*bScale* は 0 にする必要があります。 *bScale*が wType および*pwszTypeName*と一致しない場合、DB_E_BADSCALEが返されます。 *wType*  
