...
-   トランザクション レプリケーション  
     ...
     SPARSE 属性のレプリケーションは、[sp_addarticle](../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md) または [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] の **[アーティクルのプロパティ]** ダイアログ ボックスで指定されるスキーマ オプションによって決まります。 以前のバージョンの [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] では、スパース列がサポートされません。 以前のバージョンにデータをレプリケートする必要がある場合は、SPARSE 属性をレプリケートしないように指定する必要があります。  
