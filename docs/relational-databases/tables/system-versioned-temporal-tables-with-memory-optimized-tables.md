...
## 概要  
 ...
 次の図に、このアーキテクチャを示します。![Temporal In-Memory Architecture](../../relational-databases/tables/media/temporal-in-memory-architecture.png "Temporal In-Memory Architecture")  
...
-   データ フラッシュを適用するには、[sp_xtp_flush_temporal_history](../Topic/sp_xtp_flush_temporal_history.md) を呼び出します。  
...
## データ フラッシュ タスク  
 ...
 データ フラッシュを強制的に実行するには、[sp_xtp_flush_temporal_history](../Topic/sp_xtp_flush_temporal_history.md) を呼び出して、スキーマとテーブル名    
(**sys.sp_xtp_flush_temporal_history  @schema_name, @object_name**) を呼び出します。 このユーザー実行コマンドを使用すると、内部スケジュールでシステムによってデータ フラッシュ タスクが呼び出されるときと同じデータ移動プロセスが呼び出されます。  
  
## この記事は役に立ちましたか? フィードバックをお待ちしております。  
 どのような情報をお探しでしたか? お探しの情報は見つかりましたか? コンテンツ改善のため、フィードバックをお待ちしています。  [sqlfeedback@microsoft.com](mailto:sqlfeedback@microsoft.com?subject=Your%20feedback%20about%20the%20System-Versioned%20Temporal%20Tables%20with%20Memory-Optimized%20Tables%20page)にコメントをお送りください  
