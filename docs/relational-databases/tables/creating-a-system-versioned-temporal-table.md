...
- 最小限のメンテナンス コスト
 ...
 既存のテーブルを変換するときは、列名が明示的に指定されておらず (たとえば、SELECT * や、列リストのない INSERT)、新しい列を処理するように設計されていない、既存のアプリケーションに影響しないように、**HIDDEN** 句を使用して新しい **PERIOD** 列 (datetime2 列の **SysStartTime** と **SysEndTime**) を非表示にすることを検討します。
