...
description: CLR と SQL Server との統合により、.NET Framework 言語を使用して SQL Server サーバー側モジュールとしていくつかの機能を実装できます。
...
Azure SQL データベースマネージ インスタンスで CLR を使用する方法を示すこの 6 分間のビデオを見ることもできます。
...
## <a name="when-to-use-clr-modules"></a>CLR モジュールを使用するタイミング

CLR 統合を使用すると、正規表現、外部リソース (サーバー、Web サービス、データベース) にアクセスするためのコード、カスタム暗号化などの .Net Framework で使用できる複雑な機能を実装できます。サーバー側の CLR 統合の利点の一部を次に示します。
...
-   **安全性とセキュリティの強化。** マネージ コードは、データベース エンジンによってホストされる共通言語ランタイム環境で実行されます。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ではこれを利用して、旧バージョンの [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 拡張ストアド プロシージャに代わる、より安全で確実な機能を提供します。  
...
## <a name="see-also"></a>参照  
 [.NET フレームワークのインストール](https://technet.microsoft.com/library/ms166014\(v=SQL.105\).aspx)([!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]のみ)   
