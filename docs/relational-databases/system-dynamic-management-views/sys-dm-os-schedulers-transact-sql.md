...
> [!NOTE]  
>  これから[!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)]または[!INCLUDE[ssPDW](../../includes/sspdw-md.md)]、名前を使用して**sys.dm_pdw_nodes_os_schedulers**です。  
...
### <a name="a-monitoring-hidden-and-nonhidden-schedulers"></a>A. 非表示スケジューラと表示スケジューラを監視する  
 次のクエリは、ワーカーの状態を出力し、内のタスク[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]すべてスケジューラ間でします。 このクエリは、次の要件を満たすコンピューター システムで実行されました。  
...
-   スケジューラは 5 つで、 そのうちの 2 つの ID の値は 1048576 未満です。 ID が 1048576 以上のスケジューラは非表示スケジューラです。 スケジューラ`255`専用管理者接続 (DAC) を表します。 インスタンスごとに 1 つの DAC スケジューラが存在します。 メモリの負荷を調整するリソース モニターは、スケジューラを使用して`257`およびスケジューラ`258`、NUMA ノードごとに 1 つ  
