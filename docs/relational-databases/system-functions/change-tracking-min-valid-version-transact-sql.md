...
## <a name="examples"></a>使用例  
 次の例では、指定されたバージョンが有効なバージョンであるかどうかを判断します。 この例では、 `dbo.Employees`テーブルの有効な最小バージョンを取得し、これを`@last_sync_version`変数の値と比較します。 `@last_sync_version` の値が `@min_valid_version` の値より小さい場合、変更された行の一覧は有効ではありません。  
