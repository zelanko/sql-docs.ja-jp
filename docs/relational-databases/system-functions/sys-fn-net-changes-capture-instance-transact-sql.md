...
-   行を挿入または更新する必要がある場合は M  
 ...
 変更を適用するために挿入と更新のどちらが必要であるかを特定するロジックでは、クエリが複雑になってしまいます。 このオプションは、挿入操作と更新操作を区別する必要がない場合に、パフォーマンスを向上させるために使用します。 この方法は、 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]環境など、マージ操作を直接利用できるターゲット環境で最適に機能します。  
 ...
 **Net changes**ラッパー関数によって返される結果セットは、ラッパーが生成された@column_listときにに含まれていた追跡対象列のみを返します。 が@column_list NULL の場合、追跡対象のすべてのソース列が返されます。 ソース列に続いて、操作列 __CDC_OPERATION が返されます。これは、操作を表す 1 文字か 2 文字の列です。  
  
 次に、パラメーター @update_flag_listで識別された各列の結果セットに、ビットフラグが追加されます。 **Net changes**ラッパーの場合、ラッパー関数の呼び出しで使用さ@row_filter_optionれるが ' all ' または ' all with merge ' である場合、ビットフラグは常に NULL になります。 @row_filter_optionが "all with mask" に設定されていて、__CDC_OPERATION が "d" または "I" の場合、フラグの値も NULL になります。 _CDC_OPERATION \_が ' UN ' の場合、 **net**更新操作によって列が変更されたかどうかに応じて、フラグが1または0に設定されます。  
