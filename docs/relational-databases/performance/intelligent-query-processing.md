...
行モード メモリ許可フィードバック以降では、実際の実行後プランのために 2 つの新しいクエリ プラン属性 (**_IsMemoryGrantFeedbackAdjusted_* _ および _*_LastRequestedMemory_*_) が表示されます。これらは _MemoryGrantInfo* クエリ プランの XML 要素に追加されます。 
