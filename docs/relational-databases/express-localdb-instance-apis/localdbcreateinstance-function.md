...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]新しい SQL Server Express LocalDB インスタンスを作成します。  
...
## <a name="remarks"></a>解説  
 ...
 既存のインスタンスが破損したそれ以降の呼び出し時の場合、 **LocalDBCreateInstance** API メソッドは失敗します。 破損したインスタンスは、手動で修正するか明示的に削除しないと、再度使用できるようになりません。  
