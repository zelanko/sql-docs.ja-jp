...
|**type**|**tinyint**|アーティクルの種類。次のいずれかになります。<br /><br /> **10** = テーブル。<br /><br /> **32** = ストアドプロシージャ (スキーマのみ)。<br /><br /> **64** = ビューまたはインデックス付きビュー (スキーマのみ)。<br /><br /> **128** = ユーザー定義関数 (スキーマのみ)。<br /><br /> **160** = シノニム (スキーマのみ)。|  
...
|**before_image_objid**|**int**|追跡テーブルのオブジェクト ID。 追跡テーブルには、パブリケーションが作成されるときに特定のキー列の値が含まれています。 *\@* の場合は、 =  を**true**に変更します。|  
