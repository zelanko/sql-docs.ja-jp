...
|演算子 (operator)|**nvarchar**|パッケージを実行したユーザーのユーザー名は、ログ エントリを生成します。|  
...
|開始時刻|**datetime**|パッケージの実行が開始された時刻。|  
|終了時刻|**datetime**|パッケージが完了した時刻。<br /><br /> この機能は実装されていません。 Endtime 列の値は、starttime 列の値と同じでは常にします。|  
