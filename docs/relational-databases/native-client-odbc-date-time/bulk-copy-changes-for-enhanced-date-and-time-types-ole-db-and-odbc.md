...
## <a name="character-data-files"></a>文字データ ファイル  
 文字データファイルでは、日付と時刻の値は、odbc のデータ型のサポート、odbc のデータ型の[サポート](../../relational-databases/native-client-odbc-date-time/data-type-support-for-odbc-date-and-time-improvements.md)、および[OLE DB の日付と時刻の機能強化のためのデータ型のサポートに関するページの「データ形式: 文字列とリテラル」で説明されているように表されます。](../../relational-databases/native-client-ole-db-date-time/data-type-support-for-ole-db-date-and-time-improvements.md)OLE DB の場合。  
  
 Native data ファイルでは、4つの新しい型の日付と時刻の値は、小数点以下桁数が7の TDS 表現として表されます (これは [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] でサポートされる最大値であり、bcp データファイルにこれらの列の小数点以下桁数が格納されていないため)。 既存の **datetime** 型および **smalldatetime** 型や、それらの TDS (表形式のデータ ストリーム) 表現のストレージに変更はありません。  
...
|ファイル ストレージ型|ホスト ファイル データ型|IBCPSession:: BCPColFmt と共に使用するには、sqlncli に「」と入力します。|ReplTest1|  
|-----------------------|-------------------------|-----------------------------------------------------------|-----------|  
...
|------------------------|----------|----------|-------------------|--------------|---------------|--------------------|----------|-----------|  
|[日付]|@shouldalert|-|1,6|1,6|1,6|1,5,6|1,3|1,3|  
...
|2|時刻部分は無視されます。|  
|3|ODBC の場合、データ損失を伴う切り捨てが発生すると、"文字列データの右側が切り捨てられました" というメッセージで SQLSTATE 22001 の診断レコードが生成されます。秒の小数部の桁数 (小数点以下桁数) は、変換先の列のサイズによって決まります。次の表に示します。 テーブルの範囲よりサイズが大きい列の場合、7 桁と見なされます。 この変換では、最大9桁の小数点以下桁数 (ODBC で許容される最大値) を使用できます。<br /><br /> **種類:** DBTIME2<br /><br /> **暗黙の小数点以下桁数 0** 8<br /><br /> **暗黙の位取り 1.. 7** 10、16<br /><br /> <br /><br /> **種類:** DBTIMESTAMP<br /><br /> **暗黙の小数点以下桁数 0:** 19<br /><br /> **暗黙の小数点以下桁数 1.. 7:** 21.. 27<br /><br /> <br /><br /> **型:** DBTIMESTAMPOFFSET<br /><br /> **暗黙の小数点以下桁数 0:** 26<br /><br /> **暗黙の小数点以下桁数 1.. 7:** 28.. 34<br /><br /> OLE DB の場合は、データの損失を伴う切り捨てが行われると、エラーが通知されます。 datetime2 に関しては、次の表に示すように、秒の小数点以下桁数 (スケール) は変換先の列のサイズによって決まります。 テーブルの範囲よりサイズが大きい列の場合は、9 桁と見なされます。 この変換では、秒の小数点以下桁数が 9 桁まで許容されます。これは、OLE DB で許容される最大桁数です。<br /><br /> **種類:** DBTIME2<br /><br /> **暗黙の小数点以下桁数 0** 8<br /><br /> **暗黙の小数点以下桁数 1..9** 1..9<br /><br /> <br /><br /> **種類:** DBTIMESTAMP<br /><br /> **暗黙の小数点以下桁数 0:** 19<br /><br /> **暗黙の小数点以下桁数 1..9:** 21..29<br /><br /> <br /><br /> **型:** DBTIMESTAMPOFFSET<br /><br /> **暗黙の小数点以下桁数 0:** 26<br /><br /> **暗黙の小数点以下桁数 1..9:** 28..36|  
|4|日付部分は無視されます。|  
...
## <a name="see-also"></a>参照  
 [日付と時刻の&#40;機能&#41;強化 ODBC](../../relational-databases/native-client-odbc-date-time/date-and-time-improvements-odbc.md)   
