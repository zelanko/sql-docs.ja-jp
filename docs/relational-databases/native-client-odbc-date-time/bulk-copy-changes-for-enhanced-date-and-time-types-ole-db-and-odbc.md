...
## <a name="character-data-files"></a>文字データ ファイル  
 文字データ ファイルでは、ODBC の ODBC の日付と時刻の改善、または OLE DB の[日付と時刻の向上のデータ型のサポート](../../relational-databases/native-client-ole-db-date-time/data-type-support-for-ole-db-date-and-time-improvements.md)の[「データ](../../relational-databases/native-client-odbc-date-time/data-type-support-for-odbc-date-and-time-improvements.md)形式: 文字列とリテラル」のセクションで説明されているとおり、日付と時刻の値が表されます。  
  
 ネイティブ・データ・フレスでは、4 つの新しいタイプの日時値は、スケール 7 の TDS 表現として表されます (これは、[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]サポートされる最大のデータ・ファイルであり、bcp データ・ファイルはこれらの列のスケールを格納しないため)。 既存の **datetime** 型および **smalldatetime** 型や、それらの TDS (表形式のデータ ストリーム) 表現のストレージに変更はありません。  
...
|ファイル ストレージ型|ホスト ファイル データ型|IBCP セッションで使用するために sqlncli.h を入力します。:BCPColFmt|[値]|  
|-----------------------|-------------------------|-----------------------------------------------------------|-----------|  
...
|2|時刻部分は無視されます。|  
|3|ODBC では、データ損失による切り捨てが発生した場合、診断レコードが生成され、SQLSTATE 22001 とメッセージ '文字列データ、右切り捨て' 小数秒桁数 (スケール) は、次の表に従って、変換先列のサイズから決定されます。 テーブルの範囲よりサイズが大きい列の場合、7 桁と見なされます。 この変換では、最大 9 桁の秒の小数部 (ODBC で許容される最大桁) を使用できます。<br /><br /> **タイプ:** DBTIME2<br /><br /> **暗黙の小数点以下桁数 0** 8<br /><br /> **インプライド・スケール 1..7** 10,16<br /><br /> <br /><br /> **タイプ:** DBタイムスタンプ<br /><br /> **暗黙の小数点以下桁数 0:** 19<br /><br /> **インプライド・スケール 1..7:** 21..27<br /><br /> <br /><br /> **型:** DBTIMESTAMPOFFSET<br /><br /> **暗黙の小数点以下桁数 0:** 26<br /><br /> **インプライド・スケール 1..7:** 28..34<br /><br /> OLE DB の場合は、データの損失を伴う切り捨てが行われると、エラーが通知されます。 datetime2 に関しては、次の表に示すように、秒の小数点以下桁数 (スケール) は変換先の列のサイズによって決まります。 テーブルの範囲よりサイズが大きい列の場合は、9 桁と見なされます。 この変換では、秒の小数点以下桁数が 9 桁まで許容されます。これは、OLE DB で許容される最大桁数です。<br /><br /> **タイプ:** DBTIME2<br /><br /> **暗黙の小数点以下桁数 0** 8<br /><br /> **暗黙の小数点以下桁数 1..9** 1..9<br /><br /> <br /><br /> **タイプ:** DBタイムスタンプ<br /><br /> **暗黙の小数点以下桁数 0:** 19<br /><br /> **暗黙の小数点以下桁数 1..9:** 21..29<br /><br /> <br /><br /> **型:** DBTIMESTAMPOFFSET<br /><br /> **暗黙の小数点以下桁数 0:** 26<br /><br /> **暗黙の小数点以下桁数 1..9:** 28..36|  
|4|日付部分は無視されます。|  
