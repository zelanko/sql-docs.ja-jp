...
## <a name="down-level-client-behavior"></a>下位クライアントの動作  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] より前のバージョンの [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] Native Client を使用してコンパイルされたクライアント アプリケーションでは、新しい日付型と時刻型が nvarchar 列と見なされます。 列の内容は[、ODBC 日付と時刻の改善のためのデータ型サポート](../../relational-databases/native-client-odbc-date-time/data-type-support-for-odbc-date-and-time-improvements.md)の「データ形式: 文字列とリテラル」セクションで説明されているように、リテラル表現です。 列のサイズは、列に指定された秒の小数部の有効桁数に対するリテラルの最大長です。  
 ...
 SQLDescribeCol、SQLDescribe パラム、SQGetDescField、および SQLColAttribute によって返されるステートメント メタデータは、型名を含むすべての点でダウンレベルの型と一致するメタデータを返します。 このようなダウンレベル型の例としては、 **nvarchar**があります。  
...
|||SQL_C_TYPE_TIMESTAMP|日付フィールドは現在の日付に設定されます。|OK (2)<br /><br /> 日付は無視されます。 小数秒がゼロ以外の場合は失敗します。 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] で機能します。|  
||Time(7)|SQL_C_TIME|失敗 - 無効な時間リテラル。|OK (1)|  
|||SQL_C_TYPE_TIMESTAMP|失敗 - 無効な時間リテラル。|OK (1)|  
||日時2(3)|SQL_C_TYPE_TIMESTAMP|[OK]|OK (1)|  
||日時2(7)|SQL_C_TYPE_TIMESTAMP|[OK]|クライアントでの変換により、値は 1/300 秒単位に丸められます。|  
