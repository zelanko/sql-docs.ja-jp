---
title: 日付と時刻の種類のsql_variantサポート |マイクロソフトドキュメント
ms.custom: ''
ms.date: 03/03/2017
ms.prod: sql
...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
 ...
 新しい型SQL_SS_TIME2およびSQL_SS_TIMESTAMPOFFSETは、SQLColAttribute によって設定できます。 SQL_CA_SS_VARIANT_SQL_TYPEは、SQLGetDescField によって返すことができます。  
  
 結果列については、ドライバーによってバリアント型から日付型または時刻型に変換されます。 詳細については[、SQL から C への変換を](../../relational-databases/native-client-odbc-date-time/datetime-data-type-conversions-from-sql-to-c.md)参照してください。SQL_C_BINARY にバインドする場合、バッファーの長さは、SQL 型に対応する構造体を受け取るのに十分な大きさである必要があります。  
  
 SQL_SS_TIME2およびSQL_SS_TIMESTAMPOFFSETパラメーターの場合、ドライバーは、次の表に示すように、C 値を**sql_variant**値に変換します。 パラメーターが SQL_C_BINARY としてバインドされ、かつ、サーバー型が SQL_SS_VARIANT である場合、アプリケーションで SQL_CA_SS_VARIANT_SQL_TYPE が別の SQL 型に設定されていない限り、バイナリ値として扱われます。 この場合は、SQL_CA_SS_VARIANT_SQL_TYPE が優先されます。つまり、SQL_CA_SS_VARIANT_SQL_TYPE が設定されている場合、C 型からバリアントの SQL 型を推定するという既定の動作がオーバーライドされます。  
...
|SQL_C_BINARY|time|SQL_CA_SS_VARIANT_SQL_TYPE = SQL_SS_TIME2<br /><br /> スケールはSQL_DESC_PRECISIONに設定されています **(SQL バインドパラメーター**の*10 進数パラメーター* )。|  
|SQL_C_BINARY|datetimeoffset|SQL_CA_SS_VARIANT_SQL_TYPE = SQL_SS_TIMESTAMPOFFSET<br /><br /> スケールはSQL_DESC_PRECISIONに設定されています **(SQL バインドパラメーター**の*10 進数パラメーター* )。|  
|SQL_C_TYPE_DATE|date|SQL_CA_SS_VARIANT_SQL_TYPEは無視されます。|  
|SQL_C_TYPE_TIME|time(0)|SQL_CA_SS_VARIANT_SQL_TYPEは無視されます。|  
|SQL_C_TYPE_TIMESTAMP|datetime2|スケールはSQL_DESC_PRECISIONに設定されています **(SQL バインドパラメーター**の*10 進数パラメーター* )。|  
|SQL_C_NUMERIC|decimal|有効桁数はSQL_DESC_PRECISIONに設定されています **(SQLBindParameter**の*ColumnSize*パラメーター)。<br /><br /> スケールセットはSQL_DESC_SCALE (SQL バインドパラメーターの*10 進数のパラメーター* ) に設定されます。|  
