...
ms.topic: "article"
helpviewer_keywords: 
  - "暗号化 [SQL Server], DML トリガー"
  - "名前の遅延解決では、DML トリガー"
  - "WITH ENCRYPTION 句"
  - "IF UPDATE"
  - "SET ステートメント, DML トリガー"
  - "DML トリガー, プログラミング"
  - "列の変更のテスト"
  - "結果 [SQL Server], DML トリガー"
...
---
# DML トリガーの作成
  このトピックでは、[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] および [!INCLUDE[tsql](../../includes/tsql-md.md)] の CREATE TRIGGER ステートメントを使用して、[!INCLUDE[tsql](../../includes/tsql-md.md)] DML トリガーを作成する方法について説明します。  
