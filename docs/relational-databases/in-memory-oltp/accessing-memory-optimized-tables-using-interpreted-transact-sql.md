...
- レポート クエリ。このクエリでは通常、ネイティブ コンパイル ストアド プロシージャでは使用できない構造 (*ウィンドウ* 関数など) を使用します (ウィンドウ関数は [OVER](../Topic/OVER%20Clause%20\(Transact-SQL\).md) 関数とも呼ばれます)。  
...
テーブル ヒントの詳細については、 [テーブル ヒント &#40;Transact-SQL&#41;](../Topic/Table%20Hints%20\(Transact-SQL\).md). [!INCLUDE[hek_2](../../includes/hek-2-md.md)] をサポートするために SNAPSHOT が追加されました。  
...
- データベース オプション [MEMORY_OPTIMIZED_ELEVATE_TO_SNAPSHOT](../Topic/ALTER%20DATABASE%20SET%20Options%20\(Transact-SQL\).md) を ON に設定する。  
  
[自動コミット モード](http://msdn.microsoft.com/ja-jp/c8de5b60-d147-492d-b601-2eeae8511d00)で実行されるクエリでアクセスするメモリ最適化テーブルの場合、分離レベルのテーブル ヒントは必要ありません。  
