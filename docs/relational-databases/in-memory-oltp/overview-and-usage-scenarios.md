...
- `INSERT INTO <disk-based table> SELECT FROM <memory-optimized table>` を実行するジョブを使用して、[クラスター化列ストア インデックス](https://msdn.microsoft.com/library/gg492088.aspx)が指定されたディスクベースのテーブルに定期的にデータを一括オフロードします。または、
