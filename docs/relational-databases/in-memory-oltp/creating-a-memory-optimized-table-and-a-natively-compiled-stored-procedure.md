...
description: このサンプルでは、メモリ最適化テーブルとネイティブ コンパイル ストアド プロシージャを作成するためのインメモリ OLTP の構文について説明します。
