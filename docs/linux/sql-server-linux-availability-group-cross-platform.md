...
2. Linux で SQL Server 2017 をインストールします。手順については、「[SQL Sever のインストール](sql-server-linux-setup.md)」を参照してください。mssql-conf を使用して `hadr` を有効にします。
...
3. 両方のサーバー上の hosts ファイルを構成するか、サーバー名を DNS に登録します。

4. Windows と Linux の両方で TPC 1433 と 5022 のファイアウォール ポートを開きます。

5. プライマリ レプリカで、データベース ログインおよびパスワードを作成します。
...
6. プライマリ レプリカで、マスター キーと証明書を作成し、秘密キーを持つ証明書をバックアップします。
...
7. 証明書と秘密キーを Linux サーバー (セカンダリ レプリカ) の `/var/opt/mssql/data` にコピーします。Linux サーバーにファイルをコピーするには `pscp` を使用することができます。  

8. 秘密キーと証明書のグループと所有権を `mssql:mssql` に設定します。
...
9. セカンダリ レプリカで、データベース ログインとパスワードを作成し、マスター キーを作成します。
...
10. セカンダリ レプリカで、`/var/opt/mssql/data` にコピーした証明書を復元します。 
...
12. セカンダリ レプリカで、エンドポイントを作成します。エンドポイントを作成するには、前述のスクリプトをセカンダリ レプリカで繰り返します。  

13. プライマリ レプリカで、`CLUSTER_TYPE = NONE` を使用して可用性グループを作成します。スクリプトの例では `SEEDING_MODE = AUTOMATIC` を使用して可用性グループを作成します。  
...
15. 可用性グループのデータベースを作成します。手順の例では、`<TestDB>` という名前のデータベースを使用しています。自動シード処理を使用している場合は、データ ファイルとログ ファイルの両方に同じパスを設定します。
...
16. データベースの完全なバックアップを作成します。 

17. 自動シード処理を使用していない場合は、セカンダリ レプリカ (Linux) サーバー上のデータベースを復元します。[バックアップと復元を使用して Windows から Linux に SQL Server データベースを移行します](sql-server-linux-migrate-restore-database.md)。セカンダリ レプリカでデータベース `WITH NORECOVERY` を復元します。  

18. データベースを可用性グループに追加します。スクリプトの例を更新します。`<TestDB>` をデータベースの名前に置き換えます。プライマリ レプリカで、データベースを可用性グループに追加する SQL クエリを実行します。
