...
### [ジョブの作成と実行](sql-server-linux-run-sql-server-agent-job.md)
...
### [Windows からの復元](sql-server-linux-migrate-restore-database.md)
...
### [フェールオーバー クラスター インスタンス](sql-server-linux-shared-disk-cluster-configure.md)
...
### [Active Directory 認証](sql-server-linux-active-directory-authentication.md)

## [パフォーマンスの最適化](sql-server-linux-performance-get-started.md)
