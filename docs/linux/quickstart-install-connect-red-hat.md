...
このクイック スタートの最初をインストールする SQL Server 2017 に Red Hat Enterprise Linux (RHEL) 7.3 以降。 次に、**sqlcmd** と接続して最初のデータベースを作成し、クエリを実行します。
...
1. Microsoft SQL Server の Red Hat リポジトリの構成ファイルをダウンロードします。
   ...
   > [!NOTE]
   > これは、累積的な更新プログラム (CU) リポジトリです。 リポジトリ オプションとそれらの相違点の詳細については、[LinuxでSQL Serverのリポジトリを構成する](sql-server-linux-change-repo.md) を参照してください。
...
> [!TIP]
> **Sqlcmd** はSQL Server に接続し、クエリを実行し、管理と開発タスクを実行するツールの 1 つにすぎません。 その他のツールは次のとおりです。
>
