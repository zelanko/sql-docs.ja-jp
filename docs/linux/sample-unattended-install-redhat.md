...
このサンプル Bash スクリプトにより、対話型での入力を必要としない Red Hat Enterprise Linux (RHEL) での SQL Server 2017 をインストールできます。 データベース エンジン、SQL Server エージェント、SQL Server コマンド ライン ツールのインストールの例を紹介し、インストール後の手順を実行します。 必要に応じて、フルテキスト検索をインストールし、管理ユーザーを作成できます。

> [!TIP]
> 無人インストールを必要としない場合、SQL Server をインストールする最も簡単な方法は、 [Red Hat のクイック スタート チュートリアル](quickstart-install-connect-red-hat.md)に従って進めてください。 その他のセットアップについては、次を参照してください。 [Linux 上の SQL Server のインストールのガイダンス](sql-server-linux-setup.md)です。
...
- その他のシステム要件については、[Linux に SQL Server のシステム要件](sql-server-linux-setup.md#system)を参照してください。
...
Bash スクリプトでの最初の手順は、変数を設定しています。これらの変数は、このサンプルのようにスクリプト変数として指定することもできますし、環境変数として指定することもできます。  変数``` MSSQL_SA_PASSWORD ```はSQL Server インストールで**必要**です。その他は、スクリプト用に作成されたカスタム変数です。サンプルスクリプトは、次の手順を実行します。
...
1. ```MSSQL_SA_PASSWORD```でSQL Server を構成し、使用許諾契約書を自動的に同意します。
...
1. 変数```SQL_INSTALL_AGENT```が設定されている場合もしくはデフォルトで、SQL Server エージェントをインストールします。

1. オプションで変数```SQL_INSTALL_FULLTEXT```が設定されている場合、SQL Server フルテキスト検索をインストールします。
...
1. SQL Server がインストールされているようになりました。利用できるようにするために、プロセスを再起動します。

1. すべてのエラー メッセージを非表示にする間に SQL Server が正しくインストールされていることを確認します。

1. ```SQL_INSTALL_USER```と```SQL_INSTALL_USER_PASSWORD```がどちらも設定されている場合、新しいサーバー管理者のユーザーを作成します。

## <a name="next-steps"></a>次の手順

複数回の無人インストールを簡略化するために、適切な環境変数を設定するスタンドアロン Bash スクリプトを作成します。サンプルスクリプトが使用しているいずれの変数も削除することができ、それを独自のBashスクリプトで定義します。
...
さらなるLinux 上の SQL Server に関する詳細については、[SQL Server on Linux の概要](sql-server-linux-overview.md)を参照してください。
