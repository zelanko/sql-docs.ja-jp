...
> [!IMPORTANT]  
> TDE では、クライアントと PDW 間を移動するデータの暗号化は提供されません。 クライアントと SQL Server PDW の間でデータを暗号化する方法の詳細については、[証明書を準備](provision-certificate.md)を参照してください。  
>   
...
使用してデータベース暗号化の状態を確認することができます、 **sys.dm_pdw_nodes_database_encryption_keys**動的管理ビュー。 詳細については、、*カタログ ビューおよび動的管理ビュー*この記事の前半の「を参照してください。  
