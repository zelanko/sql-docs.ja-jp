...
各パラメーターの名前は、モデルの種類に固有です。 RevoScaleR で rxPredict 関数では、パラメーター、たとえば、  _@computeResidualsビット_ロジスティック回帰モデルのスコア付けに残差の計算をサポートするためにします。 パラメーター名とその値を渡す可能性があります、`PREDICT`関数。
