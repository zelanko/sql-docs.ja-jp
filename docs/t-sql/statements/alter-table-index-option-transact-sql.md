...
ON PARTITIONS **(** { \<partition_number_expression> | \<range> } [ **,**...*n* ] **)** **適用対象**: [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] ～ [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)].  
...
-   ON PARTITIONS (2) などのように、パーティションの番号を指定します。  
