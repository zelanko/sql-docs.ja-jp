 ...
 *rule_name*  
 新しいルールの名前を指定します。 ルール名は、[識別子](../../relational-databases/databases/database-identifiers.md)の規則に従っている必要があります。 ルールの所有者名の指定は省略可能です。  
 ...
 *condition_expression* には 1 つの変数が含まれます。 それぞれのローカル変数の前には、アット マーク (**@**) を付けます。 指定する式は、UPDATE または INSERT ステートメントで入力された値を参照します。 ルールを作成するときには、任意の名前または記号で値を表すことができますが、最初の文字はアット マーク (**@**) にする必要があります。  
