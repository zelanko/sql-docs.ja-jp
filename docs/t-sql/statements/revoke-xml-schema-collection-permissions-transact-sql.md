...
## <a name="examples"></a>使用例  
 次の例では、XML スキーマ コレクション `EXECUTE` の `Invoices4` 権限を、ユーザー `Wanida` から取り消します。 XML スキーマ コレクション `Invoices4` は、`AdventureWorks2012` データベースの `Sales` スキーマ内にあります。  
