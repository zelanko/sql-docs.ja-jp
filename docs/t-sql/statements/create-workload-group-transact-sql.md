...
REQUEST_MAX_MEMORY_GRANT_PERCENT = *value*  
1 つの要求にプールから割り当てられる最大メモリ量を指定します。 *value* は、MAX_MEMORY_PERCENT で指定したリソース プールのサイズが基準になります。 *value* は、[!INCLUDE[ssSQL17](../../includes/sssql17-md.md)] までは整数で、[!INCLUDE[sql-server-2019](../../includes/sssqlv15-md.md)] 以降では浮動小数で、既定の設定は 25 です。 *value* の許容範囲は 1 ～ 100 です。  
...
-   *value* を 0 に設定すると、ユーザー定義のワークロード グループで SORT と HASH MATCH 操作を含むクエリが実行されなくなります。  
...
### <a name="1-shows-how-to-create-a-workload-group-named-newreports"></a>1.newReports という名前のワークロード グループを作成する方法    
