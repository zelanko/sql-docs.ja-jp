...
|**lob read-ahead reads**|数**テキスト**、 **ntext**、**イメージ**または大きな値をクエリ用のキャッシュに入れられたページを入力します。|  
