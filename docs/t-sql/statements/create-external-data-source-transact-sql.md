...
PolyBase または Elastic Database クエリ用の外部データ ソースを作成します。 外部データ ソースを使用して接続を確立し、次の 4 つの主なユース ケースをサポートします。
...
> [!NOTE]  
> PolyBase は、SQL Server (2016 以降)、Azure SQL Data Warehouse、および Parallel Data Warehouse でサポートされています。 [エラスティック クエリ][intro_eq]は、Azure SQL Database v12 以降でのみサポートされています。
...
データ ソースのユーザー定義の名前を指定します。 名前は、SQL Server、SQL Database (SQL DB)、および SQL Data Warehouse (SQL DW) のデータベース内で一意にする必要があります。 名前は、Parallel Data Warehouse (PDW) のサーバー内で一意である必要があります。
...
- 外部データ ソースが Cloudera、Hortonworks、Azure Blob Storage、ADLS Gen 1、または ADLS Gen 2 の場合は、HADOOP を使用します。
...
`TYPE` = `SHARD_MAP_MANAGER` の外部データ ソースを作成する方法を示す例については、「[Shard Map Manager の外部データ ソースを作成する](#f-create-a-shard-map-manager-external-data-source)」を参照してください

## <a name="permissions"></a>アクセス許可

SQL Server、Parallel Data Warehouse、SQL Database、および SQL Data Warehouse でデータベースに対する CONTROL 権限が必要です。
...
## <a name="examples-sql-server-2016-and-parallel-data-warehouse"></a>例 :SQL Server (2016 以降) と Parallel Data Warehouse
...
## <a name="examples-sql-server-2016-sql-data-warehouse-and-parallel-data-warehouse"></a>例 :SQL Server (2016 以降)、SQL Data Warehouse、Parallel Data Warehouse
...
## <a name="examples-sql-database"></a>例 :SQL Database

### <a name="f-create-a-shard-map-manager-external-data-source"></a>F. Shard Map Manager の外部データ ソースを作成する
...
### <a name="g-create-an-rdbms-external-data-source"></a>G. RDBMS の外部データ ソースを作成する
...
## <a name="examples-sql-data-warehouse"></a>例 :SQL Data Warehouse

### <a name="h-create-external-data-source-to-reference-azure-data-lake-store-gen-1"></a>H. Azure Data Lake Store Gen 1 を参照する外部データ ソースを作成する
...
### <a name="i-create-external-data-source-to-reference-azure-data-lake-store-adls-gen-2"></a>I. Azure Data Lake Store (ADLS) Gen 2 を参照する外部データ ソースを作成する

ADLS Gen 2 に接続するには、データベース スコープ資格情報のシークレットとしてストレージ アカウント キーが必要です。 現在 Oauth 2.0 のサポートは使用できません。
