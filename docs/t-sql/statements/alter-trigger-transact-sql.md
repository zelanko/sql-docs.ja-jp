 ...
 *schema_name*  
 DML トリガーが属しているスキーマの名前を指定します。 DML トリガーのスコープは、そのトリガーが作成されたテーブルまたはビューのスキーマです。 *schema**_name* は、DML トリガーと、トリガーに対応するテーブルまたはビューが既定のスキーマに属している場合のみ、省略可能です。 DDL トリガーまたはログオン トリガーでは *schema_name* を指定できません。  
