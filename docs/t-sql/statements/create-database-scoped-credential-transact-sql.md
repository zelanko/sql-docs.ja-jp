 ...
 *credential_name*  
 ...
 SECRET **='***secret***'**  
 送信の認証に必要なシークレットを指定します。 `SECRET` は、Azure Blob Storage からファイルをインポートするために必要です。 Azure Blob Storage から SQL DW に読み込むには、シークレットは Azure Storage キーである必要があります。  
>  [!WARNING]
