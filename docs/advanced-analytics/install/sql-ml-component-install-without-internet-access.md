---
title: インターネットにアクセスせずに R 言語と Python のコンポーネントをインストールする
description: ネットワーク ファイアウォールの内側にある分離された SQL Server インスタンス上で、オフラインまたは切断された Machine Learning R および Python をセットアップします。
ms.prod: sql
