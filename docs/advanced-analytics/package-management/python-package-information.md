...
この`pip`モジュールは、既定でインストールされ、インストールされているパッケージを一覧表示するための多くの操作を、標準の Python でサポートされているものに加えてサポートします。 Python コマンドプロンプト`pip`からを実行できますが、から一部の`sp_execute_external_script`pip 関数を呼び出すこともできます。
