...
ms.topic: "article"
helpviewer_keywords: 
  - "クエリ プラン [SQL Server], 移行"
  - "SQL Server のアップグレード, クエリ プランの移行"
  - "プラン ガイド [SQL Server], クエリ プランの移行"
...
---
# データベース互換性モードの変更とクエリ ストアの使用
  SQL Server 2016 では、一部の変更は、データベースの DATABASE_COMPATIBILITY レベルが 130 に変更された後に有効になります。  これは、次の理由のためです。  
...
-   クエリ プロセッサに対する変更は、複雑な影響をもたらす可能性があります。  システムに対する「良い」変更は、ほとんどのユーザーには有益であっても、重要なクエリで許容できない回帰を別の場所で引き起こす可能性があります。  アップグレード プロセスからこのロジックを分離すると、クエリ ストアなどの機能は、プラン選択の回帰を迅速に緩和し、さらには実稼働サーバーで完全に回避できます。  
