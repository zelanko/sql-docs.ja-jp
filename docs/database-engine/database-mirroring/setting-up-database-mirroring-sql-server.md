...
-   [データベース ミラーリングに参加するサーバー インスタンスの準備](#PrepareInstances)  
  
-   [概要: データベース ミラーリングの確立](#EstablishUsingWinAuthentication)  
...
2.  すべてのデータベース ユーザーのログインが、ミラー サーバーに存在するようにします。 詳細については、「[データベース ミラーリングまたは AlwaysOn 可用性グループのログイン アカウントの設定 &#40;SQL Server&#41;](../../database-engine/database-mirroring/set up login accounts - database mirroring always on availability.md)」を参照してください。  
