...
ms.topic: "article"
helpviewer_keywords: 
  - "パートナー [SQL Server], クライアントの接続"
  - "データベース ミラーリング [SQL Server], クライアントの接続"
  - "クライアントの接続 [SQL Server], データベース ミラーリング"
  - "接続 [SQL Server], データベース ミラーリング"
 ...
 **このトピックの内容**  
 ...
 [データベース ミラーリング セッションへの最初の接続](#InitialConnection)  
 イニシャル パートナー名およびフェールオーバー パートナー名の概要、データ アクセス プロバイダーでミラー化されたデータベースに最初に接続する方法、およびミラー化されたデータベースに接続するための接続文字列の属性について説明します。  
  
 [接続再試行アルゴリズム (TCP/IP 接続用)](#RetryAlgorithm)  
 TCP/IP 接続に使用される接続再試行アルゴリズムについて説明します。  
  
 [データベース ミラーリング セッションへの再接続](#Reconnecting)  
 データベース ミラーリング セッションに対して確立された接続がなんらかの理由で失敗した場合のミラー化されたデータベースへの再接続、およびクライアント アプリケーションに対するリダイレクトの影響について説明します。  
  
 [古いフェールオーバー パートナー名の影響](#StalePartnerName)  
...
|-------------------|----------------------|-------------------|------------------------------------------------------------------------------|  
|元のミラー化構成|Partner_A|Partner_B|Partner_A はイニシャル パートナー名としてキャッシュに保存されます。 クライアントは、Partner_A への接続に成功します。 クライアントは、Partner_B というミラー サーバーの名前をダウンロードしてキャッシュに保存し、クライアント指定のフェールオーバー パートナー名を無視します。|  
