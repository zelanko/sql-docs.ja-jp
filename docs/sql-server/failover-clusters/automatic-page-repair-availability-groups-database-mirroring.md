...
helpviewer_keywords: 
  - "ページの自動修復"
  - "可用性グループ [SQL Server]、ページの自動修復"
  - "データベース ミラーリング [SQL Server]、[ページの自動修復"
  - "問題のあるページ [SQL Server]"
...
|------------------|-----------------|---------------------------------------------------------|  
|[823](../Topic/MSSQLSERVER_823.md)|オペレーティング システムがデータの巡回冗長検査 (CRC) を実行し、それに失敗した場合のみ処理が行われます。|ERROR_CRC。 このエラーのオペレーティング システムの値は 23 です。|  
|[824](../Topic/MSSQLSERVER_824.md)|論理エラー。|破損した書き込みや不適切なページ チェックサムなどの論理データ エラー。|  
|829|ページが復元待ちとしてマークされています。|すべて。|  
  
 最近発生した 823 CRC エラーおよび 824 エラーを確認するには、[msdb](../../relational-databases/databases/msdb-database.md) データベースの [suspect_pages](../../relational-databases/system-tables/suspect-pages-transact-sql.md) テーブルを参照してください。  
